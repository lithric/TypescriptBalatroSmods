...
function GET_DISPLAYINFO(
  screenmode: "Windowed" | "Fullscreen" | "Borderless",
  display: number
): number {
  ...
  for (let i = 1; i <= display_count; i++) {
    ...
    if (screenmode === "Fullscreen") {
      for (const [_, v] of ipairs(love.window.getFullscreenModes(i))) {
        let [_w, _h] = [
          v.width * G.SETTINGS.WINDOW.DISPLAYS[i].DPI_scale,
          v.height * G.SETTINGS.WINDOW.DISPLAYS[i].DPI_scale,
        ];
        if (
          _w <= G.SETTINGS.WINDOW.DISPLAYS[i].MONITOR_DIMS.width &&
          _h <= G.SETTINGS.WINDOW.DISPLAYS[i].MONITOR_DIMS.height
        ) {
          G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings[
            G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings.length + 1
          ] = "" + (v.width + (" X " + v.height));
          G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values[
            G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values.length + 1
          ] = { w: v.width, h: v.height };
          if (
            i === G.SETTINGS.WINDOW.selected_display &&
            i === display &&
            current_res_values.w === v.width &&
            current_res_values.h === v.height
          ) {
      ...
      G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings[1] =
        "" +
        (G.SETTINGS.WINDOW.DISPLAYS[i].MONITOR_DIMS.width /
          G.SETTINGS.WINDOW.DISPLAYS[i].DPI_scale +
          (" X " +
            G.SETTINGS.WINDOW.DISPLAYS[i].MONITOR_DIMS.height /
              G.SETTINGS.WINDOW.DISPLAYS[i].DPI_scale));
...
function timer_checkpoint(
  ...
  cp.checkpoint_list[cp.checkpoints].label =
    label + (": " + (collectgarbage("count") - G.PREV_GARB));
      ...
      while (!seed_found) {
        ...
        seed_found = random_string(
          ...
          extra_num +
            ...
            0.41231101 * G.CONTROLLER.cursor_hover.time
        );
...
function pseudorandom_element(_t, seed, args): void {
  ...
  for (const [k, v] of pairs(_t)) {
    ...
    let in_pool_func =
      (args && args.in_pool) ||
      (type(v) === "table" && type(v.in_pool) === "function" && v.in_pool) ||
      (_t === G.P_CARDS &&
        function (c) {
          let initial_deck = (args && args.starting_deck) || false;
          return !(
            (type(SMODS.Ranks[c.value].in_pool) === "function" &&
              !SMODS.Ranks[c.value].in_pool({ initial_deck: initial_deck, suit: c.suit })) ||
            (type(SMODS.Suits[c.suit].in_pool) === "function" &&
              !SMODS.Suits[c.suit].in_pool({ initial_deck: initial_deck, rank: c.value }))
          );
        });
    if (in_pool_func) {
...
function random_string(length, seed): string {
  ...
  for (let i = 1; i <= length; i++) {
    ret =
      ret +
      string.char(
        (math.random() > 0.7 && math.random(string.byte("1"), string.byte("9"))) ||
          (math.random() > 0.45 && math.random(string.byte("A"), string.byte("N"))) ||
          math.random(string.byte("P"), string.byte("Z"))
      );
  }
...
function pseudoseed(key, predict_seed): number {
  ...
  G.GAME.pseudorandom[key] = math.abs(
    tonumber(string.format("%.13f", (2.134453429141 + G.GAME.pseudorandom[key] * 1.72431234) % 1))
  );
...
function HEX(hex: string): HexArray {
  ...
  let color = [
    (tonumber(r, 16) ?? NaN) / 255,
    (tonumber(g, 16) ?? NaN) / 255,
    (tonumber(b, 16) ?? NaN) / 255,
    (tonumber(a, 16) ?? NaN) / 255,
...
function get_blind_main_colour(blind): void {
  ...
  if (blind === "Boss" || blind === "Small" || blind === "Big") {
    ...
    if (
      G.GAME.round_resets.blind_states[blind] === "Defeated" ||
      G.GAME.round_resets.blind_states[blind] === "Skipped"
    ) {
      disabled = true;
    }
  ...
  return (
    ((disabled || !G.P_BLINDS[blind]) && G.C.BLACK) ||
    G.P_BLINDS[blind].boss_colour ||
    (blind === "bl_small" && mix_colours(G.C.BLUE, G.C.BLACK, 0.6)) ||
    (blind === "bl_big" && mix_colours(G.C.ORANGE, G.C.BLACK, 0.6)) ||
    G.C.BLACK
  );
}
...
function mix_colours(C1, C2, proportionC1): HexArray {
  return [
    (C1[1] || 0.5) * proportionC1 + (C2[1] || 0.5) * (1 - proportionC1),
    (C1[2] || 0.5) * proportionC1 + (C2[2] || 0.5) * (1 - proportionC1),
    (C1[3] || 0.5) * proportionC1 + (C2[3] || 0.5) * (1 - proportionC1),
    (C1[4] || 1) * proportionC1 + (C2[4] || 1) * (1 - proportionC1),
  ];
}
...
function modulate_sound(dt): void {
  G.SPLASH_VOL =
    2 * dt * ((G.STATE === G.STATES.SPLASH && 1) || 0) + (G.SPLASH_VOL || 1) * (1 - 2 * dt);
  let desired_track =
    G.video_soundtrack ||
    (G.STATE === G.STATES.SPLASH && "") ||
    SMODS.Sound.get_current_music() ||
    (G.booster_pack_sparkles && !G.booster_pack_sparkles.REMOVED && "music2") ||
    (G.booster_pack_meteors && !G.booster_pack_meteors.REMOVED && "music3") ||
    (G.booster_pack && !G.booster_pack.REMOVED && "music2") ||
    (G.shop && !G.shop.REMOVED && "music4") ||
    (G.GAME.blind && G.GAME.blind.boss && "music5") ||
    "music1";
  G.PITCH_MOD =
    (G.PITCH_MOD || 1) * (1 - dt) +
    dt * ((!G.normal_music_speed && G.STATE === G.STATES.GAME_OVER && 0.5) || 1);
  ...
  if (
    ...
    G.ARGS.score_intensity.earned_score =
      G.GAME.current_round.current_hand.chips * G.GAME.current_round.current_hand.mult;
  }
  G.ARGS.score_intensity.required_score = (G.GAME.blind && G.GAME.blind.chips) || 0;
  G.ARGS.score_intensity.flames = math.min(
    1,
    (((G.STAGE === G.STAGES.RUN && 1) || 0) *
      ((G.ARGS.chip_flames && G.ARGS.chip_flames.real_intensity + G.ARGS.chip_flames.change) ||
        0)) /
      10
  );
  G.ARGS.score_intensity.organ =
    G.video_organ ||
    (G.ARGS.score_intensity.required_score > 0 &&
      math.max(
        math.min(
          ...
          0.1 *
            math.log(
              G.ARGS.score_intensity.earned_score / (G.ARGS.score_intensity.required_score + 1),
              5
            )
        ),
        0
  ...
  G.ARGS.ambient_sounds = G.ARGS.ambient_sounds || {
    ambientFire2: {
      volfunc: function (_prev_volume) {
        return (
          _prev_volume * (1 - dt) +
          dt *
            0.9 *
            ((G.ARGS.score_intensity.flames > 0.3 && 1) || G.ARGS.score_intensity.flames / 0.3)
        );
      },
    },
    ambientFire1: {
      volfunc: function (_prev_volume) {
        return (
          ...
          dt *
            0.8 *
            ((G.ARGS.score_intensity.flames > 0.3 && (G.ARGS.score_intensity.flames - 0.3) / 0.7) ||
              0)
        );
      },
    },
    ambientFire3: {
      volfunc: function (_prev_volume) {
        return (
          ...
          dt *
            0.4 *
            (((G.ARGS.chip_flames && G.ARGS.chip_flames.change) || 0) +
              ((G.ARGS.mult_flames && G.ARGS.mult_flames.change) || 0))
        );
      },
    },
    ambientOrgan1: {
      volfunc: function (_prev_volume) {
        return (
          _prev_volume * (1 - dt) +
          ((dt * 0.6 * (G.SETTINGS.SOUND.music_volume + 100)) / 200) * G.ARGS.score_intensity.organ
        );
      },
    },
  };
  for (const [k, v] of pairs(G.ARGS.ambient_sounds)) {
    ...
    AC[k].per =
      ...
      1;
    AC[k].vol =
      (!G.video_organ && G.STATE === G.STATES.SPLASH && 0) ||
      (AC[k].vol && v.volfunc(AC[k].vol)) ||
      0;
  }
...
function prep_draw(
  ...
  love.graphics.translate(
    moveable.VT.x +
      moveable.VT.w / 2 +
      ((offset && offset.x) || 0) +
      ((moveable.layered_parallax && moveable.layered_parallax.x) ||
        (moveable.parent &&
          moveable.parent.layered_parallax &&
          moveable.parent.layered_parallax.x) ||
        0),
    moveable.VT.y +
      moveable.VT.h / 2 +
      ((offset && offset.y) || 0) +
      ((moveable.layered_parallax && moveable.layered_parallax.y) ||
        (moveable.parent &&
          moveable.parent.layered_parallax &&
          moveable.parent.layered_parallax.y) ||
        0)
  );
...
function alert_no_space(card, area): void {
  ...
  attention_text({
    scale: 0.9,
    text: localize("k_no_space_ex"),
    hold: 0.9,
    align: "cm",
    cover: area,
    cover_padding: 0.1,
    cover_colour: adjust_alpha(G.C.BLACK, 0.7),
  });
...
function number_format(num, e_switch_point): void {
  ...
  if (num >= (e_switch_point || G.E_SWITCH_POINT)) {
    ...
    return (
      sign +
      string.format(
        (fac >= 100 && "%.1fe%i") || (fac >= 10 && "%.2fe%i") || "%.3fe%i",
        mantissa,
        fac
      )
    );
...
function check_and_set_high_score(score, amt): void {
  ...
  if (
    score === "hand" &&
    G.SETTINGS.COMP &&
    (!G.SETTINGS.COMP.score || G.SETTINGS.COMP.score < math.floor(amt))
  ) {
    G.SETTINGS.COMP.score = amt;
    send_score(math.floor(amt));
  }
  if (
    G.PROFILES[G.SETTINGS.profile].high_scores[score] &&
    math.floor(amt) > G.PROFILES[G.SETTINGS.profile].high_scores[score].amt
  ) {
  ...
  for (const [k, v] of pairs(G.jokers.cards)) {
    if (v.config.center_key && v.ability.set === "Joker") {
      if (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key]) {
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].count =
          G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].count + 1;
      } else {
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key] = {
          count: 1,
...
function set_joker_win(): void {
  for (const [k, v] of pairs(G.jokers.cards)) {
    if (v.config.center_key && v.ability.set === "Joker") {
      G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key] = G.PROFILES[
        G.SETTINGS.profile
      ].joker_usage[v.config.center_key] || {
        count: 1,
        order: v.config.center.order,
        wins: {},
        losses: {},
      ...
      if (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key]) {
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins =
          G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins || {};
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins[G.GAME.stake] =
          (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins[G.GAME.stake] ||
            0) + 1;
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins_by_key[
          SMODS.stake_from_index(G.GAME.stake)
        ] =
          (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins_by_key[
            SMODS.stake_from_index(G.GAME.stake)
          ] || 0) + 1;
...
function get_joker_win_sticker(_center: CenterItemParams, index): void {
  if (
    ...
    G.PROFILES[G.SETTINGS.profile].joker_usage[_center.key].wins
  ) {
    let _w = 0;
...
function set_joker_loss(): void {
  for (const [k, v] of pairs(G.jokers.cards)) {
    if (v.config.center_key && v.ability.set === "Joker") {
      if (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key]) {
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses =
          G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses || {};
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses[G.GAME.stake] =
          (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses[G.GAME.stake] ||
            0) + 1;
        G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses_by_key[
          SMODS.stake_from_index(G.GAME.stake)
        ] =
          (G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses_by_key[
            SMODS.stake_from_index(G.GAME.stake)
...
function set_deck_usage(): void {
  if (
    ...
    G.GAME.selected_back.effect &&
    G.GAME.selected_back.effect.center &&
    ...
    if (G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key]) {
      G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].count =
        G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].count + 1;
    } else {
  ...
  if (
    ...
    if (G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key]) {
      G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins[G.GAME.stake] =
        (G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins[G.GAME.stake] || 0) + 1;
      for (
        let i = 1;
        i <=
        ((G.P_CENTER_POOLS["Stake"][G.GAME.stake].unlocked_stake &&
          G.P_STAKES[G.P_CENTER_POOLS["Stake"][G.GAME.stake].unlocked_stake].stake_level - 1) ||
          G.GAME.stake - 1);
        i++
      ) {
        G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins[i] =
          G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins[i] || 1;
      }
    }
...
function get_deck_win_stake(_deck_key): void {
  ...
  if (
    G.PROFILES[G.SETTINGS.profile].deck_usage[_deck_key] &&
    G.PROFILES[G.SETTINGS.profile].deck_usage[_deck_key].wins
  ) {
    let _w = 0;
...
function get_deck_win_sticker(_center): void {
  if (
    ...
    G.PROFILES[G.SETTINGS.profile].deck_usage[_center.key].wins
  ) {
    let _w = -1;
...
function set_deck_loss(): void {
  if (
    ...
    if (G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key]) {
      G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].losses[G.GAME.stake] =
        (G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].losses[G.GAME.stake] || 0) + 1;
    }
    G.save_settings();
  }
}
function set_consumeable_usage(card): void {
  if (card.config.center_key && card.ability.consumeable) {
    if (G.PROFILES[G.SETTINGS.profile].consumeable_usage[card.config.center_key]) {
      G.PROFILES[G.SETTINGS.profile].consumeable_usage[card.config.center_key].count =
        G.PROFILES[G.SETTINGS.profile].consumeable_usage[card.config.center_key].count + 1;
    } else {
    ...
    if (G.GAME.consumeable_usage[card.config.center_key]) {
      G.GAME.consumeable_usage[card.config.center_key].count =
        G.GAME.consumeable_usage[card.config.center_key].count + 1;
    } else {
...
function set_voucher_usage(card): void {
  if (card.config.center_key && card.ability.set === "Voucher") {
    if (G.PROFILES[G.SETTINGS.profile].voucher_usage[card.config.center_key]) {
      G.PROFILES[G.SETTINGS.profile].voucher_usage[card.config.center_key].count =
        G.PROFILES[G.SETTINGS.profile].voucher_usage[card.config.center_key].count + 1;
    } else {
      G.PROFILES[G.SETTINGS.profile].voucher_usage[card.config.center_key] = {
        count: 1,
        order: card.config.center.order,
      };
    }
...
function set_hand_usage(hand): void {
  ...
  if (G.PROFILES[G.SETTINGS.profile].hand_usage[hand]) {
    G.PROFILES[G.SETTINGS.profile].hand_usage[hand].count =
      G.PROFILES[G.SETTINGS.profile].hand_usage[hand].count + 1;
  } else {
...
function set_profile_progress(): void {
  ...
  for (const [_, v] of pairs(G.P_CENTERS)) {
    ...
    if (v.set === "Joker") {
      ...
      G.PROGRESS.joker_stickers.tally =
        G.PROGRESS.joker_stickers.tally + get_joker_win_sticker(v, true);
    }
  }
...
function set_discover_tallies(): void {
  G.DISCOVER_TALLIES = G.DISCOVER_TALLIES || {
    blinds: { tally: 0, of: 0 },
    tags: { tally: 0, of: 0 },
    jokers: { tally: 0, of: 0 },
...
function inc_career_stat(stat, mod): void {
  ...
  G.PROFILES[G.SETTINGS.profile].career_stats[stat] =
    G.PROFILES[G.SETTINGS.profile].career_stats[stat] + (mod || 0);
  G.save_settings();
}
        ...
        for (const [_, center] of pairs(set)) {
          ...
          if (!center.text) {
            ...
            for (const [_, line] of ipairs(
              (type(center.name) === "table" && center.name) || [center.name]
            )) {
...
function convert_save_to_meta(): void {
  if (love.filesystem.getInfo(G.SETTINGS.profile + ("/" + "unlocked_jokers.jkr"))) {
    ...
    if (love.filesystem.getInfo(G.SETTINGS.profile + ("/" + "unlocked_jokers.jkr"))) {
      for (const [line] of string.gmatch(
        (get_compressed(G.SETTINGS.profile + ("/" + "unlocked_jokers.jkr")) || "") + "\\n",
        "([^\\n]*)\\n"
      )) {
        let key = line.gsub("%s+", "");
        if (key && key !== "") {
    ...
    if (love.filesystem.getInfo(G.SETTINGS.profile + ("/" + "discovered_jokers.jkr"))) {
      for (const [line] of string.gmatch(
        (get_compressed(G.SETTINGS.profile + ("/" + "discovered_jokers.jkr")) || "") + "\\n",
        "([^\\n]*)\\n"
      )) {
        let key = line.gsub("%s+", "");
        if (key && key !== "") {
    ...
    if (love.filesystem.getInfo(G.SETTINGS.profile + ("/" + "alerted_jokers.jkr"))) {
      for (const [line] of string.gmatch(
        (get_compressed(G.SETTINGS.profile + ("/" + "alerted_jokers.jkr")) || "") + "\\n",
        "([^\\n]*)\\n"
      )) {
        let key = line.gsub("%s+", "");
        if (key && key !== "") {
...
function card_from_control(control): void {
  ...
  let _card = Card(
    ...
    G.CARD_W,
    G.CARD_H,
    G.P_CARDS[control.s + ("_" + control.r)],
    G.P_CENTERS[control.e || "c_base"],
...
function loc_parse_string(line): void {
  ...
  for (let i = 1; i <= line.length; i++) {
    ...
    if (char === "{") {
      ...
      str_parts[str_it] =
        (str_parts[str_it] || "") + ((control["X"] && char.gsub("%s+", "")) || char);
    } else if (!_c && char === "#" && !_s_gather) {
      _s_gather = true;
      if (str_parts[str_it]) {
        str_it = str_it + 1;
...
function localize(args, misc_cat): void {
  ...
  if (args.type === "other") {
    ...
    let multi_line = {};
    if (loc_target) {
      for (const [_, lines] of ipairs(
        (args.type === "unlocks" && loc_target.unlock_parsed) ||
          (args.type === "name" && loc_target.name_parsed) ||
          (args.type === "text" && loc_target) ||
        ...
        for (const [_, part] of ipairs(lines)) {
          ...
          for (const [_, subpart] of ipairs(part.strings)) {
            assembled_string =
              assembled_string +
              ((type(subpart) === "string" && subpart) ||
                format_ui_value(args.vars[tonumber(subpart[1])]) ||
                "ERROR");
          }
    ...
    if (
      pcall(function () {
        ret_string =
          G.localization.descriptions[args.set || args.node.config.center.set][
            args.key || args.node.config.center.key
          ].name;
      })
    ) {
    } else {
  ...
  if (loc_target) {
    for (const [_, lines] of ipairs(
      (args.type === "unlocks" && loc_target.unlock_parsed) ||
        (args.type === "name" && loc_target.name_parsed) ||
        ((args.type === "text" || args.type === "tutorial" || args.type === "quips") &&
          loc_target) ||
        loc_target.text_parsed
    )) {
      let final_line = {};
      for (const [_, part] of ipairs(lines)) {
        ...
        if (args.type === "name") {
          final_line[final_line.length + 1] = {
            ...
            config: {
              object: DynaText({
                ...
                colours: [
                  (part.control.V && args.vars.colours[tonumber(part.control.V)]) ||
                    (part.control.C && loc_colour(part.control.C)) ||
                    args.text_colour ||
                    G.C.UI.TEXT_LIGHT,
                ],
                bump: true,
                ...
                scale:
                  (0.55 - 0.004 * assembled_string.length) *
                  ((part.control.s && tonumber(part.control.s)) || args.scale || 1),
              }),
            },
          };
        } else if (part.control.E) {
          let [_float, _silent, _pop_in, _bump, _spacing] = [
            undefined,
            true,
            undefined,
            undefined,
          ...
          final_line[final_line.length + 1] = {
            ...
            config: {
              object: DynaText({
                ...
                colours: [
                  (part.control.V && args.vars.colours[tonumber(part.control.V)]) ||
                    loc_colour(part.control.C || undefined),
                ],
                float: _float,
                silent: _silent,
                pop_in: _pop_in,
            ...
            nodes: [
              {
                ...
                config: {
                  ...
                  colour: loc_colour(part.control.C || undefined),
                  scale:
                    0.32 *
                    ((part.control.s && tonumber(part.control.s)) || args.scale || 1) *
                    desc_scale,
          ...
          final_line[final_line.length + 1] = {
            ...
            config: {
              detailed_tooltip:
                (part.control.T && (G.P_CENTERS[part.control.T] || G.P_TAGS[part.control.T])) ||
                undefined,
              text: assembled_string,
              shadow: args.shadow,
              colour:
                (part.control.V && args.vars.colours[tonumber(part.control.V)]) ||
                (!part.control.C && args.text_colour) ||
                loc_colour(part.control.C || undefined, args.default_col),
              scale:
                0.32 *
                ((part.control.s && tonumber(part.control.s)) || args.scale || 1) *
                desc_scale,
  ...
  if (G.P_CENTER_POOLS["Stake"][_stake].shiny) {
    stake_sprite.draw = function (_sprite) {
      ...
      _sprite.ARGS.send_to_shader[1] =
        math.min(_sprite.VT.r * 3, 1) +
        G.TIMERS.REAL / 18 +
        ((_sprite.juice && _sprite.juice.r * 20) || 0) +
        1;
...
function get_front_spriteinfo(_front): void {
      ...
      if (deckSkin) {
        if (deckSkin.outdated) {
          ...
          if (hasRank) {
            let atlas =
              G.ASSET_ATLAS[
                (G.SETTINGS.colour_palettes[_front.suit] === "hc" && deckSkin.hc_atlas) ||
                  deckSkin.lc_atlas
              ];
            if (atlas) {
          ...
          return [
            G.ASSET_ATLAS[
              (G.SETTINGS.colour_palettes[_front.suit] === "hc" && _front.hc_atlas) ||
                _front.lc_atlas ||
                {}
            ] ||
              G.ASSET_ATLAS[_front.atlas] ||
              G.ASSET_ATLAS[
                "cards_" + ((G.SETTINGS.colour_palettes[_front.suit] === "hc" && 2) || 1)
              ],
            _front.pos,
          ];
        } else {
          let palette =
            (deckSkin.palette_map &&
              deckSkin.palette_map[G.SETTINGS.colour_palettes[_front.suit] || ""]) ||
            (deckSkin.palettes || {})[1];
          let hasRank = false;
          ...
          return [
            G.ASSET_ATLAS[(palette.hc_default && _front.hc_atlas) || _front.lc_atlas || {}] ||
              G.ASSET_ATLAS[_front.atlas] ||
              G.ASSET_ATLAS["cards_" + ((palette.hc_default && 2) || 1)],
            _front.pos,
          ];
        }
      }
  ...
  return [
    G.ASSET_ATLAS[(G.SETTINGS.colourblind_option && _front.hc_atlas) || _front.lc_atlas || {}] ||
      G.ASSET_ATLAS[_front.atlas] ||
      G.ASSET_ATLAS["cards_" + ((G.SETTINGS.colourblind_option && 2) || 1)],
    _front.pos,
  ];
}
function get_stake_col(_stake): void {
  G.C.STAKES = G.C.STAKES || [
    G.C.WHITE,
    G.C.RED,
    G.C.GREEN,
    G.C.BLACK,
...
function PLAY_SOUND(args): SoundHandler {
  ...
  let should_stream =
    string.find(args.sound_code, "music") || string.find(args.sound_code, "ambient");
  let s = {
    sound: love.audio.newSource(
      "resources/sounds/" + (args.sound_code + ".ogg"),
      (should_stream && "stream") || "static"
    ),
  };
...
function SET_SFX(s, args): void {
  if (string.find(s.sound_code, "music")) {
    ...
    s.sound.setVolume(
      s.current_volume *
        s.original_volume *
        (args.sound_settings.volume / 100) *
        (args.sound_settings.music_volume / 100)
    );
    ...
    let sound_vol =
      s.original_volume *
      (args.sound_settings.volume / 100) *
      (args.sound_settings.game_sounds_volume / 100);
