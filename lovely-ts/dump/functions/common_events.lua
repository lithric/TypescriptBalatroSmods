...
function update_hand_text(self, config, vals)
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                    ...
                    if vals.StatusText then
                        attention_text(
                            _G,
                            {
                                ...
                                cover_colour = mix_colours(_G, G.C.CHIPS, col, 0.1),
                                emboss = 0.05,
                                align = "cm",
                                cover_align = "cr"
                            }
                    ...
                    if vals.StatusText then
                        attention_text(
                            _G,
                            {
                                ...
                                cover_colour = mix_colours(_G, G.C.MULT, col, 0.1),
                                emboss = 0.05,
                                align = "cm",
                                cover_align = "cl"
                            }
                ...
                if vals.level and G.GAME.current_round.current_hand.hand_level ~= " " .. tostring(localize(_G, "k_lvl")) .. tostring(vals.level) then
                    if vals.level == "" then
                        G.GAME.current_round.current_hand.hand_level = vals.level
                    else
                        G.GAME.current_round.current_hand.hand_level = " " .. tostring(localize(_G, "k_lvl")) .. tostring(vals.level)
                        if type(vals.level) == "number" then
                            G.hand_text_area.hand_level.config.colour = G.C.HAND_LEVELS[math.min(vals.level, 7) + 1]
                        else
                            G.hand_text_area.hand_level.config.colour = G.C.HAND_LEVELS[2]
                        end
                        G.hand_text_area.hand_level:juice_up()
                    end
                end
                if config.sound and not config.modded then
                    play_sound(_G, config.sound, config.pitch or 1, config.volume or 1)
                end
...
function check_for_unlock(self, args)
    ...
    if args.type == "win_challenge" then
        unlock_achievement(_G, "rule_bender")
        local _c = true
        for k, v in pairs(G.CHALLENGES) do
            if not G.PROFILES[G.SETTINGS.profile + 1].challenge_progress.completed[v.id] then
                _c = false
            end
        end
        if _c then
            unlock_achievement(_G, "rule_breaker")
        end
    end
    if false then
        return
    ...
    for k, v in pairs(G.ACHIEVEMENTS) do
        if not v.earned and (v.unlock_condition and type(v.unlock_condition) == "function") and v:unlock_condition(args) then
            unlock_achievement(_G, k)
        end
    end
    if args.type == "career_stat" then
        if args.statname == "c_cards_played" and G.PROFILES[G.SETTINGS.profile + 1].career_stats[args.statname] >= 2500 then
            unlock_achievement(_G, "card_player")
        end
        if args.statname == "c_cards_discarded" and G.PROFILES[G.SETTINGS.profile + 1].career_stats[args.statname] >= 2500 then
            unlock_achievement(_G, "card_discarder")
        end
    end
    if args.type == "ante_up" then
        if args.ante >= 4 then
            unlock_achievement(_G, "ante_up")
        end
        if args.ante >= 8 then
            unlock_achievement(_G, "ante_upper")
        end
    end
    if args.type == "win" then
        unlock_achievement(_G, "heads_up")
        if G.GAME.round <= 12 then
            unlock_achievement(_G, "speedrunner")
        end
        if G.GAME.round_scores.times_rerolled.amt <= 0 then
            unlock_achievement(_G, "you_get_what_you_get")
        end
    end
    if args.type == "win_stake" then
        local highest_win, lowest_win = __TS__Unpack(get_deck_win_stake(_G, nil))
        if highest_win >= G.P_STAKES.stake_red.stake_level then
            unlock_achievement(_G, "low_stakes")
        end
        if highest_win >= G.P_STAKES.stake_black.stake_level then
            unlock_achievement(_G, "mid_stakes")
        end
        if highest_win >= G.P_STAKES.stake_gold.stake_level then
            unlock_achievement(_G, "high_stakes")
        end
        if G.PROGRESS and G.PROGRESS.deck_stakes.tally / G.PROGRESS.deck_stakes.of >= 1 then
            unlock_achievement(_G, "completionist_plus")
        end
        if G.PROGRESS and G.PROGRESS.joker_stickers.tally / G.PROGRESS.joker_stickers.of >= 1 then
            unlock_achievement(_G, "completionist_plus_plus")
        end
    end
    if args.type == "money" then
        if G.GAME.dollars >= 400 then
            unlock_achievement(_G, "nest_egg")
        end
    end
    if args.type == "hand" then
        if args.handname == "Flush" and args.scoring_hand then
            ...
            if _w == args.scoring_hand.length then
                unlock_achievement(_G, "flushed")
            end
        end
        if args.disp_text == "Royal Flush" then
            unlock_achievement(_G, "royale")
        end
    end
    if args.type == "shatter" then
        if args.shattered.length >= 2 then
            unlock_achievement(_G, "shattered")
        end
    end
    ...
    while i <= #G.P_LOCKED do
        ...
        if not custom_check and not card.unlocked and card.unlock_condition and args.type == "career_stat" then
            if args.statname == card.unlock_condition.type and G.PROFILES[G.SETTINGS.profile + 1].career_stats[args.statname] >= card.unlock_condition.extra then
                ret = true
                unlock_card(_G, card)
            end
        end
        if not custom_check and not card.unlocked and card.unlock_condition and card.unlock_condition.type == args.type then
            ...
            if args.type == "interest_streak" and card.unlock_condition.extra <= G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_round_interest_cap_streak then
                ret = true
                unlock_card(_G, card)
            end
            ...
            if args.type == "blank_redeems" then
                if G.PROFILES[G.SETTINGS.profile + 1].voucher_usage.v_blank and G.PROFILES[G.SETTINGS.profile + 1].voucher_usage.v_blank.count >= card.unlock_condition.extra then
                    unlock_card(_G, card)
                end
            end
            if args.type == "modify_deck" then
                if card.unlock_condition.extra and card.unlock_condition.extra.suit then
                    ...
                    for _, v in pairs(G.playing_cards) do
                        if v.base.suit == card.unlock_condition.extra.suit then
                            count = count + 1
                        end
                    end
                ...
                if card.unlock_condition.extra and card.unlock_condition.extra.enhancement then
                    local count = 0
                    for _, v in pairs(G.playing_cards) do
                        if v.ability.name == card.unlock_condition.extra.enhancement then
                            count = count + 1
                        end
                    end
                    if count >= card.unlock_condition.extra.count then
            ...
            if args.type == "win_deck" then
                if card.unlock_condition.deck then
                    if get_deck_win_stake(_G, card.unlock_condition.deck) > 0 then
                        ret = true
                        unlock_card(_G, card)
                    end
                end
            ...
            if args.type == "win_stake" then
                if card.unlock_condition.stake then
                    if get_deck_win_stake(_G) >= card.unlock_condition.stake then
                        ret = true
                        unlock_card(_G, card)
                    end
                end
            ...
            if args.type == "discover_planets" then
                ...
                for k, v in pairs(G.P_CENTERS) do
                    if v.set == "Planet" and v.discovered then
                        count = count + 1
                    end
                end
                if count >= 9 then
            ...
            if args.type == "modify_jokers" and G.jokers then
                if card.unlock_condition.extra.count then
                    ...
                    for _, v in pairs(G.jokers.cards) do
                        if v.ability.set == "Joker" and v.edition and v.edition.polychrome and card.unlock_condition.extra.polychrome then
                            count = count + 1
                        end
                    end
                    if count >= card.unlock_condition.extra.count then
            ...
            if args.type == "round_win" then
                if card.name == "Matador" then
                    if G.GAME.current_round.hands_played == 1 and G.GAME.current_round.discards_left == G.GAME.round_resets.discards and G.GAME.blind:get_type() == "Boss" then
                        ret = true
                        unlock_card(_G, card)
                    end
                end
                if card.name == "Troubadour" then
                    if G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_single_hand_round_streak >= card.unlock_condition.extra then
                        ret = true
                        unlock_card(_G, card)
                    end
                end
            ...
            if args.type == "discard_custom" then
                ...
                if card.name == "Brainstorm" then
                    local eval = evaluate_poker_hand(_G, args.cards)
            ...
            if args.type == "win_custom" then
                if card.name == "Invisible Joker" and G.GAME.max_jokers <= 4 then
                    ret = true
                    unlock_card(_G, card)
                end
...
function unlock_card(self, card)
    if card.unlocked == false then
        if not SMODS.config.seeded_unlocks and (G.GAME.seeded or G.GAME.challenge) then
            return
        end
        if card.unlocked or card.wip then
            return
        end
        G:save_notify(card)
        card.unlocked = true
        if card.set == "Back" then
            discover_card(_G, card)
        end
        table.sort(
        ...
        notify_alert(_G, card.key, card.set)
    end
end
function fetch_achievements(self)
    ...
    if G.STEAM and not G.STEAM.initial_fetch then
        for k, v in pairs(G.ACHIEVEMENTS) do
            ...
            local success, achieved = __TS__Unpack(G.STEAM.userStats:getAchievement(achievement_name))
            if success then
...
function unlock_achievement(self, achievement_name)
    if G.PROFILES[G.SETTINGS.profile + 1].all_unlocked and (G.ACHIEVEMENTS and G.ACHIEVEMENTS[achievement_name] and not G.ACHIEVEMENTS[achievement_name].bypass_all_unlocked and SMODS.config.achievements < 3) or SMODS.config.achievements < 3 and (G.GAME.seeded or G.GAME.challenge) then
        return true
    end
                ...
                func = function(self)
                    if G.STATE ~= G.STATES.HAND_PLAYED then
                        ...
                        if achievement_set then
                            notify_alert(_G, achievement_name)
                            return true
                        end
                        if G.F_NO_ACHIEVEMENTS and not (G.ACHIEVEMENTS[achievement_name] or ({})).mod then
                            return true
                        end
                        if not G.ACHIEVEMENTS then
                            fetch_achievements(_G)
                        ...
                        if G.STEAM then
                            if G.ACHIEVEMENTS[achievement_name] then
                                if not G.ACHIEVEMENTS[achievement_name].earned then
                                    ...
                                    local success, achieved = __TS__Unpack(G.STEAM.userStats:getAchievement(achievement_code))
...
function notify_alert(self, _achievement, _type)
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    ...
                    G.achievement_notification = G.achievement_notification or UIBox(
                        _G,
                        {
                            definition = create_UIBox_notify_alert(_G, _achievement, _type),
    ...
    G.E_MANAGER:add_event(
        __TS__New(
            GameEvent,
            {
                ...
                delay = 0.1,
                func = function(self)
                    play_sound(_G, "highlight1", nil, 0.5)
                    play_sound(_G, "foil2", 0.5, 0.4)
                    return true
                end
            }
        ),
...
function create_unlock_overlay(self, key)
                ...
                GameEvent,
                {
                    ...
                    func = function(self)
                        if not G.OVERLAY_MENU then
                            ...
                            G.FUNCS:overlay_menu({definition = G.P_CENTERS[key].set == "Back" and create_UIBox_deck_unlock(_G, G.P_CENTERS[key]) or create_UIBox_card_unlock(_G, G.P_CENTERS[key])})
                            play_sound(_G, "foil1", 0.7, 0.3)
                            play_sound(_G, "gong", 1.4, 0.15)
                            return true
                        end
                    end
                }
...
function discover_card(self, card)
    if not SMODS.config.seeded_unlocks and (G.GAME.seeded or G.GAME.challenge) then
        return
    end
    card = card or ({})
    if card.discovered or card.wip then
        return
    end
...
function get_current_pool(self, _type, _rarity, _legendary, _append)
    G.ARGS.TEMP_POOL = EMPTY(_G, G.ARGS.TEMP_POOL)
    ...
    if _type == "Joker" then
        ...
        local rarity = _rarity or SMODS:poll_rarity(
            "Joker",
            "rarity" .. tostring(G.GAME.round_resets.ante + (_append or ""))
        )
        _starting_pool, _pool_key = G.P_JOKER_RARITY_POOLS[rarity], "Joker" .. tostring(rarity + (not _legendary and _append or ""))
    elseif SMODS.ObjectTypes[_type] and SMODS.ObjectTypes[_type].rarities then
        ...
        if _legendary and rarities.legendary then
            ...
            rarity = _rarity or SMODS:poll_rarity(
                _type,
                "rarity_" .. tostring(_type + (G.GAME.round_resets.ante + (_append or "")))
            )
        end
    ...
    for k, v in ipairs(_starting_pool) do
        ...
        elseif _type == "Demo" then
            if v.pos and v.config then
                add = true
            end
        elseif _type == "Tag" then
            if (not v.requires or G.P_CENTERS[v.requires] and G.P_CENTERS[v.requires].discovered) and (not v.min_ante or v.min_ante <= G.GAME.round_resets.ante) then
                add = true
            end
        elseif not (G.GAME.used_jokers[v.key] and not pool_opts.allow_duplicates and not ({next(find_joker(_G, "Showman"))})) and (v.unlocked ~= false or v.rarity == 4) then
    ...
    if _pool_size == 0 then
        _pool = EMPTY(_G, G.ARGS.TEMP_POOL)
    ...
    return {
        _pool,
        _pool_key .. tostring(not _legendary and G.GAME.round_resets.ante or "")
    }
end
function get_new_boss(self)
    ...
    for k, v in pairs(G.P_BLINDS) do
        ...
        elseif v.in_pool and type(v.in_pool) == "function" then
            local res, options = __TS__Unpack(v:in_pool())
            if (G.GAME.round_resets.ante % G.GAME.win_ante == 0 and G.GAME.round_resets.ante >= 2) == (v.boss.showdown or false) or (options or ({})).ignore_showdown_check then
    ...
    local _, boss = __TS__Unpack(pseudorandom_element(
        _G,
        eligible_bosses,
        pseudoseed(_G, "boss")
    ))
...
function set_screen_positions(self)
...
function ease_dollars(self, mod, instant)
    local function _mod(self, mod)
        local dollar_UI = G.HUD:get_UIE_by_ID("dollar_text_UI")
        mod = mod or 0
        local text = "+" .. tostring(localize(_G, "$"))
        local col = G.C.MONEY
        if mod < 0 then
            text = "-" .. tostring(localize(_G, "$"))
            col = G.C.RED
        else
            inc_career_stat(_G, "c_dollars_earned", mod)
        end
        G.GAME.dollars = G.GAME.dollars + mod
        check_and_set_high_score(_G, "most_money", G.GAME.dollars)
        check_for_unlock(_G, {type = "money"})
...
function ease_discard(self, mod, instant, silent)
    local function _mod(self, mod)
        if math.abs(math.max(G.GAME.current_round.discards_left, mod)) == 0 then
            return
        end
...
function ease_hands_played(self, mod, instant)
    local function _mod(self, mod)
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                ...
                check_and_set_high_score(_G, "furthest_ante", G.GAME.round_resets.ante)
                ...
                play_sound(_G, "highlight2", 0.685, 0.2)
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                ...
                check_and_set_high_score(_G, "furthest_round", G.GAME.round)
                check_and_set_high_score(_G, "furthest_ante", G.GAME.round_resets.ante)
...
function ease_value(self, ref_table, ref_value, mod, floored, timer_type, not_blockable, delay, ease_type)
    mod = mod or 0
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
...
function ease_background_colour(self, args)
            ...
            if args.special_colour and args.tertiary_colour then
                ...
                if k == "C" and args.special_colour then
                    ...
                    ease_value(
                        ...
                        v,
                        1,
                        args.new_colour[1] * brightness - v[1],
                        false,
                        nil,
                        true,
                        0.6
                    ...
                    ease_value(
                        ...
                        v,
                        2,
                        args.new_colour[2] * brightness - v[2],
                        false,
                        nil,
                        true,
                        0.6
                    ...
                    ease_value(
                        ...
                        v,
                        3,
                        args.new_colour[3] * brightness - v[3],
                        false,
                        nil,
                        true,
                        0.6
    ...
    if args.contrast then
        ease_value(
            ...
            G.C.BACKGROUND,
            "contrast",
            args.contrast - G.C.BACKGROUND.contrast,
            false,
            nil,
            true,
            0.6
...
function ease_colour(self, old_colour, new_colour, delay)
    ease_value(
        _G,
        old_colour,
        1,
        new_colour[1] - old_colour[1],
        false,
        "REAL",
        nil,
        delay
...
function ease_background_colour_blind(self, state, blind_override)
    ...
    if state == G.STATES.SHOP then
        ease_colour(
            _G,
            G.C.DYN_UI.MAIN,
            mix_colours(_G, G.C.RED, G.C.BLACK, 0.9)
        )
    elseif state == G.STATES.TAROT_PACK then
        ease_colour(
            _G,
            G.C.DYN_UI.MAIN,
            mix_colours(_G, G.C.WHITE, G.C.BLACK, 0.9)
        )
    elseif state == G.STATES.SPECTRAL_PACK then
        ease_colour(
            _G,
            G.C.DYN_UI.MAIN,
            mix_colours(_G, G.C.SECONDARY_SET.Spectral, G.C.BLACK, 0.9)
        )
    elseif state == G.STATES.STANDARD_PACK then
        ease_colour(_G, G.C.DYN_UI.MAIN, G.C.RED)
    elseif state == G.STATES.BUFFOON_PACK then
        ease_colour(_G, G.C.DYN_UI.MAIN, G.C.FILTER)
    elseif state == G.STATES.PLANET_PACK then
        ease_colour(
            _G,
            G.C.DYN_UI.MAIN,
            mix_colours(_G, G.C.SECONDARY_SET.Planet, G.C.BLACK, 0.9)
        )
    ...
    if state == G.STATES.TAROT_PACK then
        ease_background_colour(
            _G,
            {
                ...
                special_colour = darken(_G, G.C.BLACK, 0.2),
                contrast = 1.5
            }
        )
    elseif state == G.STATES.SPECTRAL_PACK then
    ...
    elseif state == G.STATES.STANDARD_PACK then
        ease_background_colour(
            _G,
            {
                new_colour = darken(_G, G.C.BLACK, 0.2),
                special_colour = G.C.RED,
                contrast = 3
            }
        )
    elseif state == G.STATES.BUFFOON_PACK then
        ease_background_colour(_G, {new_colour = G.C.FILTER, special_colour = G.C.BLACK, contrast = 2})
    elseif state == G.STATES.PLANET_PACK then
        ease_background_colour(_G, {new_colour = G.C.BLACK, contrast = 3})
    elseif G.GAME.won then
        ease_background_colour(_G, {new_colour = G.C.BLIND.won, contrast = 1})
    elseif blindname == "Small Blind" or blindname == "Big Blind" or blindname == "" then
        ease_background_colour(_G, {new_colour = G.C.BLIND.Small, contrast = 1})
    else
                ...
                if v.boss.showdown then
                    ease_background_colour(
                        _G,
                        {
                            new_colour = G.C.BLUE,
                            special_colour = G.C.RED,
                            tertiary_colour = darken(_G, G.C.BLACK, 0.4),
                            contrast = 3
                        }
                    )
                    return
        ...
        ease_background_colour(
            _G,
            {
                new_colour = lighten(
                    _G,
                    mix_colours(_G, boss_col, G.C.BLACK, 0.3),
                    0.1
                ),
...
function delay(self, time, queue)
    G.E_MANAGER:add_event(
        __TS__New(
            GameEvent,
            {
...
function add_joker(self, joker, edition, silent, eternal)
    ...
    local card = Card(
        ...
        G.P_CARDS.empty,
        G.P_CENTERS[joker],
        {discover = true, bypass_discovery_center = true, bypass_discovery_ui = true, bypass_back = G.GAME.selected_back.pos}
    )
...
function draw_card(self, from, to, percent, dir, sort, card, delay, mute, stay_flipped, vol, discarded_only)
        ...
        GameEvent,
        {
            ...
            func = function(self)
                ...
                if not mute and drawn then
                    ...
                    play_sound(_G, "card1", 0.85 + percent * 0.2 / 100, 0.6 * (vol or 1))
                end
                if sort then
                    to:sort()
                end
...
function highlight_card(self, card, percent, dir)
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                ...
                play_sound(_G, "cardSlide1", 0.85 + percent * 0.2)
                return true
            end
        }
    ))
end
function play_area_status_text(self, text, silent, delay)
...
function level_up_hand(self, card, hand, instant, amount)
    amount = amount or 1
    ...
    if not instant then
        ...
        update_hand_text(_G, {delay = 0}, {mult = G.GAME.hands[hand].mult, StatusText = true})
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                trigger = "after",
        ...
        update_hand_text(_G, {delay = 0}, {chips = G.GAME.hands[hand].chips, StatusText = true})
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                trigger = "after",
        ...
        update_hand_text(_G, {sound = "button", volume = 0.7, pitch = 0.9, delay = 0}, {level = G.GAME.hands[hand].level})
        delay(_G, 1.3)
    end
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            trigger = "immediate",
            func = function(self)
                check_for_unlock(_G, {type = "upgrade_hand", hand = hand, level = G.GAME.hands[hand].level})
                return true
            end
        }
    ))
end
function eval_card(self, card, context)
    ...
    if card.area and area_set[card.area] then
        local jokers, triggered = __TS__Unpack(card:calculate_joker(context))
                ...
                if v.should_alert then
                    if not G[v.alert_uibox_name] then
                        G[v.alert_uibox_name] = UIBox(
                            _G,
                            {
                                definition = create_UIBox_card_alert(_G, {red_bad = true}),
                                config = {
                                    align = "tri",
                                    offset = {x = 0.05, y = -0.05},
                                    major = G.OVERLAY_MENU:get_UIE_by_ID(v.id),
                                    instance_type = "ALERT"
                                }
                            }
                        )
...
function set_main_menu_UI(self)
    G.MAIN_MENU_UI = UIBox(
        _G,
        {
            definition = create_UIBox_main_menu_buttons(_G),
            ...
            func = function(self)
                if not G.F_DISP_USERNAME or type(G.F_DISP_USERNAME) == "string" then
                    G.PROFILE_BUTTON = UIBox(
                        _G,
                        {
                            definition = create_UIBox_profile_button(_G),
...
function card_eval_status_text(self, card, eval_type, amt, percent, dir, extra)
    ...
    elseif eval_type == "chips" then
        ...
        text = localize(
            ...
            {
                type = "variable",
                key = "a_chips" .. (amt < 0 and "_minus" or ""),
                vars = {math.abs(amt)}
            }
        )
        delay = 0.6
    elseif eval_type == "mult" then
        ...
        text = localize(
            ...
            {
                type = "variable",
                key = "a_mult" .. (amt < 0 and "_minus" or ""),
                vars = {math.abs(amt)}
            }
        )
        colour = G.C.MULT
    ...
    elseif eval_type == "x_chips" then
        ...
        text = localize(
            ...
            {
                type = "variable",
                key = "a_xchips" .. (amt < 0 and "_minus" or ""),
                vars = {math.abs(amt)}
            }
        )
        colour = G.C.BLUE
    ...
    elseif eval_type == "x_mult" or eval_type == "h_x_mult" then
        ...
        text = localize(
            ...
            {
                type = "variable",
                key = "a_xmult" .. (amt < 0 and "_minus" or ""),
                vars = {math.abs(amt)}
            }
        )
        colour = G.C.XMULT
    ...
    elseif eval_type == "dollars" then
        sound = "coin3"
        amt = amt
        text = (amt < -0.01 and "-" or "") .. tostring(localize(_G, "$")) .. tostring(math.abs(amt))
        colour = amt < -0.01 and G.C.RED or G.C.MONEY
    elseif eval_type == "swap" then
        sound = "generic1"
        amt = amt
        text = localize(_G, "k_swapped_ex")
        colour = G.C.PURPLE
    ...
    if amt > 0 or amt < 0 then
        if extra and extra.instant then
            ...
            play_sound(_G, sound, 0.8 + percent * 0.2, volume)
    ...
    if extra and extra.playing_cards_created then
        playing_card_joker_effects(_G, extra.playing_cards_created)
    end
end
function add_round_eval_row(self, config)
                ...
                func = function(self)
                    ...
                    if config.name == "blind1" then
                        ...
                        local blind_sprite = AnimatedSprite(
                            ...
                            copy_table(_G, G.GAME.blind.pos)
                        )
                                ...
                                nodes = {{
                                    ...
                                    nodes = {{
                                        ...
                                        config = {object = DynaText(
                                            _G,
                                            {
                                                string = {" " .. tostring(localize(_G, "ph_mr_bones")) .. " "},
                                                colours = {G.C.FILTER},
                                                shadow = true,
                                                pop_in = 0,
                                                scale = 0.5 * scale,
                                ...
                                nodes = {
                                    {
                                        ...
                                        nodes = {{
                                            ...
                                            config = {object = DynaText(
                                                _G,
                                                {
                                                    string = {" " .. tostring(localize(_G, "ph_score_at_least")) .. " "},
                                        ...
                                        nodes = {
                                            ...
                                            }},
                                            {
                                                ...
                                                config = {
                                                    ...
                                                    scale = scale_number(_G, G.GAME.blind.chips, scale, 100000),
                                                    colour = G.C.RED,
                                                    shadow = true
                                                }
                                            }
                        ...
                        table.insert(
                            left_text,
                            {
                                ...
                                config = {object = DynaText(
                                    _G,
                                    {
                                        string = {" " .. tostring(localize(_G, {type = "variable", key = "remaining_hand_money", vars = {G.GAME.modifiers.money_per_hand or 1}}))},
                                        colours = {G.C.UI.TEXT_LIGHT},
                                        shadow = true,
                                        pop_in = 0,
                                        scale = 0.4 * scale,
                        ...
                        table.insert(
                            left_text,
                            {
                                ...
                                config = {object = DynaText(
                                    _G,
                                    {
                                        string = {" " .. tostring(localize(_G, {type = "variable", key = "remaining_discard_money", vars = {G.GAME.modifiers.money_per_discard or 0}}))},
                                        colours = {G.C.UI.TEXT_LIGHT},
                                        shadow = true,
                                        pop_in = 0,
                                        scale = 0.4 * scale,
                        ...
                        table.insert(
                            left_text,
                            {
                                ...
                                config = {object = DynaText(
                                    _G,
                                    {
                                        string = localize(_G, {type = "name_text", set = config.card.config.center.set, key = config.card.config.center.key}),
                                        colours = {G.C.FILTER},
                                        shadow = true,
                                        pop_in = 0,
                                        scale = 0.6 * scale,
                        ...
                        table.insert(
                            left_text,
                            {
                                ...
                                config = {object = DynaText(
                                    _G,
                                    {
                                        string = {" " .. tostring(localize(_G, {type = "variable", key = "interest", vars = {G.GAME.interest_amount, 5, G.GAME.interest_amount * G.GAME.interest_cap / 5}}))},
                                        colours = {G.C.UI.TEXT_LIGHT},
                                        shadow = true,
                                        pop_in = 0,
                                        scale = 0.4 * scale,
                        ...
                        nodes = {
                            {n = G.UIT.C, config = {padding = 0.05, minw = width * 0.55, minh = 0.61, align = "cl"}, nodes = left_text},
                            {
                                ...
                                nodes = {{
                                    ...
                                    config = {
                                        align = "cm",
                                        id = "dollar_" .. tostring(config.name)
                                    },
                                    nodes = {}
                                }}
                            }
                    ...
                    play_sound(_G, "cancel", config.pitch or 1)
                    play_sound(_G, "highlight1", 1.5 * config.pitch or 1, 0.2)
                    if config.card then
                            ...
                            G.round_eval:add_child(
                                {
                                    ...
                                    config = {
                                        align = "cm",
                                        id = "dollar_row_" .. tostring(dollar_row + 1) .. "_" .. tostring(config.name)
                                    },
                                    nodes = {{
                                        n = G.UIT.O,
                                        config = {object = DynaText(
                                            _G,
                                            {
                                                string = {"-" .. tostring(localize(_G, "$") + format_ui_value(_G, -num_dollars))},
                                                colours = {G.C.MONEY},
                                                shadow = true,
                                                pop_in = 0,
                                                scale = 0.65,
                                ...
                                G.round_eval:get_UIE_by_ID("dollar_" .. tostring(config.name))
                            )
                            play_sound(
                                _G,
                                "coin3",
                            ...
                            G.round_eval:add_child(
                                {
                                    ...
                                    nodes = {{
                                        ...
                                        config = {object = DynaText(
                                            _G,
                                            {
                                                string = {localize(_G, "$") + format_ui_value(_G, num_dollars)},
                                                colours = {G.C.MONEY},
                                                shadow = true,
                                                pop_in = 0,
                                                scale = 0.65,
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {
                            ...
                            func = function(self)
                                ...
                                G.round_eval:add_child(
                                    r,
                                    G.round_eval:get_UIE_by_ID("dollar_row_" .. tostring(dollar_row) .. "_" .. tostring(config.name))
                                )
                ...
                func = function(self)
                    UIBox(
                        _G,
                        {
                                    ...
                                    nodes = {
                                        {
                                            n = G.UIT.T,
                                            config = {
                                                text = tostring(localize(_G, "b_cash_out")) .. ": ",
                                                scale = 1,
                                        ...
                                        },
                                        {
                                            n = G.UIT.T,
                                            config = {
                                                text = localize(_G, "$") + format_ui_value(_G, config.dollars),
                                                scale = 1.2 * scale,
                                                colour = G.C.WHITE,
                                                shadow = true,
                                                juice = true
                            ...
                            config = {align = "tmi", offset = {x = 0, y = 0.4}, major = G.round_eval}
                        }
                    )
                    G.GAME.current_round.dollars = config.dollars
                    play_sound(_G, "coin6", config.pitch or 1)
...
function change_shop_size(self, mod)
    ...
    if G.shop_jokers and G.shop_jokers.cards then
        if mod < 0 then
            do
                ...
                while i <= G.GAME.shop.joker_max + 1 do
...
function update_canvas_juice(self, dt)
    ...
    if not G.SETTINGS.rumble then
        G.CURR_VIBRATION = 0
    end
    if G.CONTROLLER.GAMEPAD.object and G.F_RUMBLE then
        G.CONTROLLER.GAMEPAD.object:setVibration(G.CURR_VIBRATION * 0.4 * G.F_RUMBLE, G.CURR_VIBRATION * 0.4 * G.F_RUMBLE)
    end
end
function juice_card_until(self, card, eval_func, first, delay)
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            trigger = "after",
...
function inc_steam_stat(self, stat_name)
    if not G.STEAM then
        return
    end
    local success, current_stat = __TS__Unpack(G.STEAM.userStats:getStatInt(stat_name))
    if success then
        G.STEAM.userStats:setStatInt(stat_name, current_stat + 1)
        G.STEAM.send_control.update_queued = true
    end
end
function unlock_notify(self)
    local _UN = get_compressed(
        _G,
        tostring(G.SETTINGS.profile) .. "/" .. "unlock_notify.jkr"
    )
    if _UN then
        for key in string.gmatch(_UN .. "\\n", "(.-)\\n") do
            create_unlock_overlay(_G, key)
        end
        love.filesystem.remove(tostring(G.SETTINGS.profile) .. "/" .. "unlock_notify.jkr")
    end
end
function get_deck_from_name(self, _name)
    for k, v in pairs(G.P_CENTERS) do
...
function get_next_voucher_key(self, _from_tag)
    local _pool, _pool_key = __TS__Unpack(get_current_pool(_G, "Voucher"))
    if _from_tag then
    ...
    while center == "UNAVAILABLE" do
        ...
        center = pseudorandom_element(
            ...
            pseudoseed(
                _G,
                tostring(_pool_key) .. "_resample" .. tostring(it)
            )
        )
    end
    return center
end
function get_next_tag_key(self, append)
    if G.FORCE_TAG then
        return G.FORCE_TAG
    end
    local _pool, _pool_key = __TS__Unpack(get_current_pool(
        _G,
        "Tag",
        nil,
        nil,
    ...
    local _tag = pseudorandom_element(
        _G,
        _pool,
        pseudoseed(_G, _pool_key)
    )
...
function create_playing_card(self, card_init, area, skip_materialize, silent, colours)
    card_init = card_init or ({})
    card_init.front = card_init.front or pseudorandom_element(
        _G,
        G.P_CARDS,
        pseudoseed(_G, "front")
    )
...
function get_pack(self, _key, _type)
    if not G.GAME.first_shop_buffoon and not G.GAME.banned_keys.p_buffoon_normal_1 then
        ...
        return G.P_CENTERS["p_buffoon_normal_" .. tostring(math.random(1, 2))]
    end
    ...
    local poll = pseudorandom(
        _G,
        pseudoseed(_G, (_key or "pack_generic") + G.GAME.round_resets.ante)
    ) * cume
    for k, v in ipairs(G.P_CENTER_POOLS.Booster) do
...
function poll_edition(self, _key, _mod, _no_neg, _guaranteed)
    _mod = _mod or 1
    local edition_poll = pseudorandom(
        _G,
        pseudoseed(_G, _key or "edition_generic")
    )
    if _guaranteed then
...
function create_card(self, _type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    if not forced_key and soulable and not G.GAME.banned_keys.c_soul then
        for _, v in ipairs(SMODS.Consumable.legendaries) do
            if (_type == v.type.key or _type == v.soul_set) and not (G.GAME.used_jokers[v.key] and not ({next(find_joker(_G, "Showman"))}) and not v.can_repeat_soul) and (not v.in_pool or type(v.in_pool) ~= "function" or v:in_pool()) then
                if pseudorandom(
                    _G,
                    "soul_" .. tostring(v.key + (_type + G.GAME.round_resets.ante))
                ) > 1 - v.soul_rate then
                    forced_key = v.key
                end
            end
        end
        if (_type == "Tarot" or _type == "Spectral" or _type == "Tarot_Planet") and not (G.GAME.used_jokers.c_soul and not ({next(find_joker(_G, "Showman"))})) then
            if pseudorandom(
                _G,
                "soul_" .. tostring(_type + G.GAME.round_resets.ante)
            ) > 0.997 then
                forced_key = "c_soul"
            end
        end
        if (_type == "Planet" or _type == "Spectral") and not (G.GAME.used_jokers.c_black_hole and not ({next(find_joker(_G, "Showman"))})) then
            if pseudorandom(
                _G,
                "soul_" .. tostring(_type + G.GAME.round_resets.ante)
            ) > 0.997 then
    ...
    local front = (_type == "Base" or _type == "Enhanced") and pseudorandom_element(
        _G,
        G.P_CARDS,
        pseudoseed(
            _G,
            "front" .. tostring((key_append or "") + G.GAME.round_resets.ante)
        )
    ) or nil
    local card = Card(
        _G,
        ...
        front,
        center,
        {bypass_discovery_center = area == G.shop_jokers or area == G.pack_cards or area == G.shop_vouchers or G.shop_demo and area == G.shop_demo or area == G.jokers or area == G.consumeables, bypass_discovery_ui = area == G.shop_jokers or area == G.pack_cards or area == G.shop_vouchers or G.shop_demo and area == G.shop_demo, discover = area == G.jokers or area == G.consumeables, bypass_back = G.GAME.selected_back.pos}
    )
    ...
    if _type == "Joker" then
        ...
        if area == G.shop_jokers or area == G.pack_cards then
            local eternal_perishable_poll = pseudorandom(
                _G,
                (area == G.pack_cards and "packetper" or "etperpoll") .. tostring(G.GAME.round_resets.ante)
            )
            ...
            if G.GAME.modifiers.enable_rentals_in_shop and pseudorandom(
                _G,
                (area == G.pack_cards and "packssjr" or "ssjr") .. tostring(G.GAME.round_resets.ante)
            ) > 0.7 and not SMODS.Stickers.rental.should_apply then
        ...
        if not SMODS.bypass_create_card_edition then
            local edition = poll_edition(
                _G,
                "edi" .. tostring((key_append or "") + G.GAME.round_resets.ante)
            )
...
function copy_card(self, other, new_card, card_scale, playing_card, strip_edition)
    local new_card = new_card or Card(
        _G,
        other.T.x,
        other.T.y,
        G.CARD_W * (card_scale or 1),
        G.CARD_H * (card_scale or 1),
        G.P_CARDS.empty,
        G.P_CENTERS.c_base,
    ...
    for k, v in pairs(other.ability) do
        if type(v) == "table" then
            new_card.ability[k] = copy_table(_G, v)
        else
    ...
    if other.seal then
        for k, v in pairs(other.ability.seal or ({})) do
            if type(v) == "table" then
                new_card.ability.seal[k] = copy_table(_G, v)
            else
...
function tutorial_info(self, args)
    ...
    G.OVERLAY_TUTORIAL = G.OVERLAY_TUTORIAL or UIBox(
        _G,
        {
            definition = {
                ...
                nodes = {{
                    ...
                    nodes = {UIBox_button(
                        _G,
                        {
                            label = {tostring(localize(_G, "b_skip")) .. " >"},
            ...
            config = {align = "cm", offset = {x = 0, y = 3.2}, major = G.ROOM_ATTACH, bond = "Weak"}
        }
    )
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    if G.OVERLAY_TUTORIAL and G.OVERLAY_TUTORIAL.step == step and not G.OVERLAY_TUTORIAL.step_complete then
                        ...
                        G.OVERLAY_TUTORIAL.Jimbo = G.OVERLAY_TUTORIAL.Jimbo or Card_Character(_G, pos)
                        ...
                        if args.content then
                            G.OVERLAY_TUTORIAL.content = UIBox(
                                _G,
                                {
                                    ...
                                    config = {align = args.content_config and args.content_config.align or "cm", offset = args.content_config and args.content_config.offset or ({x = 0, y = 0}), major = args.content_config and args.content_config.major or G.OVERLAY_TUTORIAL.Jimbo, bond = "Weak"}
                                }
                            )
                        ...
                        G.OVERLAY_TUTORIAL.Jimbo:say_stuff(2 * (G.localization.misc.tutorial[args.text_key] or ({})).length + 1)
...
function calculate_reroll_cost(self, skip_increment)
...
function reset_idol_card(self)
    ...
    if valid_idol_cards[1] then
        local idol_card = pseudorandom_element(
            _G,
            valid_idol_cards,
            pseudoseed(
                _G,
                "idol" .. tostring(G.GAME.round_resets.ante)
            )
        )
...
function reset_mail_rank(self)
    ...
    if valid_mail_cards[1] then
        local mail_card = pseudorandom_element(
            _G,
            valid_mail_cards,
            pseudoseed(
                _G,
                "mail" .. tostring(G.GAME.round_resets.ante)
            )
        )
...
function reset_ancient_card(self)
    local ancient_suits = {}
    for k, v in ipairs({"Spades", "Hearts", "Clubs", "Diamonds"}) do
        if v ~= G.GAME.current_round.ancient_card.suit then
            ancient_suits[ancient_suits.length + 1] = v
        end
    end
    local ancient_card = pseudorandom_element(
        _G,
        ancient_suits,
        pseudoseed(
            _G,
            "anc" .. tostring(G.GAME.round_resets.ante)
        )
    )
    G.GAME.current_round.ancient_card.suit = ancient_card
end
function reset_castle_card(self)
    ...
    if valid_castle_cards[1] then
        local castle_card = pseudorandom_element(
            ...
            pseudoseed(
                _G,
                "cas" .. tostring(G.GAME.round_resets.ante)
            )
        )
        G.GAME.current_round.castle_card.suit = castle_card.base.suit
    end
end
function reset_blinds(self)
...
function get_type_colour(self, _c, card)
    return _c.unlocked == false and not (card and card.bypass_lock) and G.C.BLACK or _c.unlocked ~= false and (_c.set == "Joker" or _c.consumeable or _c.set == "Voucher") and not _c.discoveredand and not (_c.area ~= G.jokers and _c.area ~= G.consumeables and _c.area or not _c.area) and G.C.JOKER_GREY or card and card.debuff and mix_colours(_G, G.C.RED, G.C.GREY, 0.7) or _c.set == "Joker" and G.C.RARITY[_c.rarity] or _c.set == "Edition" and G.C.DARK_EDITION or _c.set == "Booster" and G.C.BOOSTER or G.C.SECONDARY_SET[_c.set] or ({0, 1, 1, 1})
end
function generate_card_ui(self, _c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end, card)
    if card == nil and card_type then
        card = SMODS.compat_0_9_8.generate_UIBox_ability_table_card
    end
    if _c.specific_vars then
    ...
    if not full_UI_table.name then
        ...
        elseif card_type == "Locked" then
            full_UI_table.name = localize(_G, {type = "name", set = "Other", key = "locked", nodes = {}})
        ...
        elseif specific_vars and (card_type == "Default" or card_type == "Enhanced") then
            if _c.name == "Stone Card" or _c.replace_base_card then
                full_UI_table.name = true
            elseif specific_vars.playing_card then
                ...
                localize(
                    _G,
                    {
                        ...
                        vars = {
                            [1] = localize(_G, specific_vars.value, "ranks"),
                            [2] = localize(_G, specific_vars.suit, "suits_plural"),
        ...
        elseif card_type == "Booster" then
            ...
            if not _c.generate_ui or type(_c.generate_ui) ~= "function" then
                full_UI_table.name = localize(_G, {type = "name", set = _c.set, key = _c.key, nodes = full_UI_table.name})
            end
        end
    ...
    if _c.set == "Other" then
        localize(_G, {type = "other", key = _c.key, nodes = desc_nodes, vars = specific_vars or _c.vars})
    elseif card_type == "Locked" then
        if _c.wip then
            localize(_G, {
                type = "other",
        ...
        elseif _c.demo and specific_vars then
            localize(_G, {type = "other", key = "demo_shop_locked", nodes = desc_nodes, vars = loc_vars})
        elseif _c.demo then
            localize(_G, {type = "other", key = "demo_locked", nodes = desc_nodes, vars = loc_vars})
        else
            local res = {}
            ...
            elseif _c.name == "Mr. Bones" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_losses}
            elseif _c.name == "Acrobat" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_hands_played}
            elseif _c.name == "Sock and Buskin" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_face_cards_played}
            elseif _c.name == "Swashbuckler" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_jokers_sold}
            elseif _c.name == "Troubadour" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Certificate" then
            elseif _c.name == "Smeared Joker" then
                loc_vars = {
                    _c.unlock_condition.extra.count,
                    localize(_G, {type = "name_text", key = _c.unlock_condition.extra.e_key, set = "Enhanced"})
                }
            elseif _c.name == "Throwback" then
            elseif _c.name == "Hanging Chad" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "Rough Gem" then
                loc_vars = {
                    _c.unlock_condition.extra.count,
                    localize(_G, _c.unlock_condition.extra.suit, "suits_singular")
                }
            ...
            elseif _c.name == "Arrowhead" then
                loc_vars = {
                    _c.unlock_condition.extra.count,
                    localize(_G, _c.unlock_condition.extra.suit, "suits_singular")
                }
            elseif _c.name == "Onyx Agate" then
                loc_vars = {
                    _c.unlock_condition.extra.count,
                    localize(_G, _c.unlock_condition.extra.suit, "suits_singular")
                }
            ...
            elseif _c.name == "Showman" then
                loc_vars = {_c.unlock_condition.ante}
            elseif _c.name == "Flower Pot" then
                loc_vars = {_c.unlock_condition.ante}
            elseif _c.name == "Blueprint" then
            elseif _c.name == "Wee Joker" then
                loc_vars = {_c.unlock_condition.n_rounds}
            elseif _c.name == "Merry Andy" then
                loc_vars = {_c.unlock_condition.n_rounds}
            elseif _c.name == "Oops! All 6s" then
                loc_vars = {number_format(_G, _c.unlock_condition.chips)}
            elseif _c.name == "The Idol" then
                loc_vars = {number_format(_G, _c.unlock_condition.chips)}
            elseif _c.name == "Seeing Double" then
                loc_vars = {localize(_G, "ph_4_7_of_clubs")}
            elseif _c.name == "Matador" then
            elseif _c.name == "Hit the Road" then
            elseif _c.name == "The Duo" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "The Trio" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "The Family" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "The Order" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "The Tribe" then
                loc_vars = {localize(_G, _c.unlock_condition.extra, "poker_hands")}
            elseif _c.name == "Stuntman" then
                loc_vars = {number_format(_G, _c.unlock_condition.chips)}
            elseif _c.name == "Invisible Joker" then
            elseif _c.name == "Brainstorm" then
            elseif _c.name == "Satellite" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Shoot the Moon" then
            elseif _c.name == "Driver's License" then
                loc_vars = {_c.unlock_condition.extra.count}
            elseif _c.name == "Cartomancer" then
                loc_vars = {_c.unlock_condition.tarot_count}
            elseif _c.name == "Astronomer" then
            elseif _c.name == "Burnt Joker" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_cards_sold}
            elseif _c.name == "Bootstraps" then
                loc_vars = {_c.unlock_condition.extra.count}
            elseif _c.name == "Overstock Plus" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_shop_dollars_spent}
            elseif _c.name == "Liquidation" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Tarot Tycoon" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_tarots_bought}
            elseif _c.name == "Planet Tycoon" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_planets_bought}
            elseif _c.name == "Glow Up" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Reroll Glut" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_shop_rerolls}
            elseif _c.name == "Omen Globe" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_tarot_reading_used}
            elseif _c.name == "Observatory" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_planetarium_used}
            elseif _c.name == "Nacho Tong" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_cards_played}
            elseif _c.name == "Recyclomancy" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_cards_discarded}
            elseif _c.name == "Money Tree" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_round_interest_cap_streak}
            elseif _c.name == "Antimatter" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].voucher_usage.v_blank and G.PROFILES[G.SETTINGS.profile + 1].voucher_usage.v_blank.count or 0}
            elseif _c.name == "Illusion" then
                loc_vars = {_c.unlock_condition.extra, G.PROFILES[G.SETTINGS.profile + 1].career_stats.c_playing_cards_bought}
            elseif _c.name == "Petroglyph" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Retcon" then
                loc_vars = {_c.unlock_condition.extra}
            elseif _c.name == "Palette" then
                loc_vars = {_c.unlock_condition.extra}
            end
            if _c.rarity and _c.rarity == 4 and specific_vars and not specific_vars.not_hidden then
                localize(_G, {
                    type = "unlocks",
                    key = res.key or "joker_locked_legendary",
                    set = res.set or "Other",
                    nodes = desc_nodes,
                    vars = loc_vars,
                    text_colour = res.text_colour,
                    scale = res.scale
                })
            else
                localize(_G, {
                    type = "unlocks",
                    key = res.key or _c.key,
                    set = res.set or _c.set,
                    nodes = desc_nodes,
                    vars = loc_vars,
                    text_colour = res.text_colour,
                    scale = res.scale
    ...
    elseif _c.generate_ui and type(_c.generate_ui) == "function" then
        ...
        if specific_vars and specific_vars.sticker then
            info_queue[info_queue.length + 1] = {
                key = string.lower(specific_vars.sticker) .. "_sticker",
                set = "Other"
            }
        end
    elseif specific_vars and specific_vars.debuffed then
        localize(_G, {type = "other", key = "debuffed_" .. (specific_vars.playing_card and "playing_card" or "default"), nodes = desc_nodes})
    elseif _c.set == "Joker" then
        if not card then
            local ability = copy_table(_G, cfg)
            ability.set = "Joker"
            ability.name = _c.name
            local ret = {Card:generate_UIBox_ability_table({ability = ability, config = {center = _c}, bypass_lock = true}, true)}
            specific_vars = ret[2]
            if ret[3] then
                desc_nodes[desc_nodes.length + 1] = ret[3]
            end
            main_end = ret[4]
        end
            ...
            if G.jokers and G.jokers.cards then
                for k, v in ipairs(G.jokers.cards) do
                    if v.edition and v.edition.negative and G.localization.descriptions.Other.remove_negative then
                        main_end = {}
                        localize(_G, {type = "other", key = "remove_negative", nodes = main_end, vars = {}})
                        main_end = main_end[1]
                        break
                    end
                end
    ...
    elseif _c.set == "Tag" then
        specific_vars = specific_vars or Tag:get_uibox_table(
            {
                name = _c.name,
                config = cfg,
                ability = {orbital_hand = "[" .. tostring(localize(_G, "k_poker_hand")) .. "]"}
            },
            nil,
            true
        )
        if _c.name == "Negative Tag" then
            info_queue[info_queue.length + 1] = G.P_CENTERS.e_negative
        elseif _c.name == "Foil Tag" then
            info_queue[info_queue.length + 1] = G.P_CENTERS.e_foil
        elseif _c.name == "Holographic Tag" then
            info_queue[info_queue.length + 1] = G.P_CENTERS.e_holo
        elseif _c.name == "Polychrome Tag" then
    ...
    elseif _c.set == "Voucher" then
        ...
        elseif _c.name == "Tarot Merchant" or _c.name == "Tarot Tycoon" then
            loc_vars = {cfg.extra_disp}
        elseif _c.name == "Planet Merchant" or _c.name == "Planet Tycoon" then
            loc_vars = {cfg.extra_disp}
        elseif _c.name == "Hone" or _c.name == "Glow Up" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Reroll Surplus" or _c.name == "Reroll Glut" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Grabber" or _c.name == "Nacho Tong" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Wasteful" or _c.name == "Recyclomancy" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Seed Money" or _c.name == "Money Tree" then
            loc_vars = {cfg.extra / 5}
        elseif _c.name == "Blank" or _c.name == "Antimatter" then
        elseif _c.name == "Hieroglyph" or _c.name == "Petroglyph" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Director's Cut" or _c.name == "Retcon" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Paint Brush" or _c.name == "Palette" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Telescope" or _c.name == "Observatory" then
            loc_vars = {cfg.extra}
        elseif _c.name == "Clearance Sale" or _c.name == "Liquidation" then
            loc_vars = {cfg.extra}
        end
        localize(_G, {
            type = "descriptions",
    ...
    elseif _c.set == "Default" and specific_vars then
        if specific_vars.nominal_chips then
            localize(_G, {type = "other", key = "card_chips", nodes = desc_nodes, vars = {specific_vars.nominal_chips}})
        end
        if specific_vars.bonus_chips then
            localize(_G, {type = "other", key = "card_extra_chips", nodes = desc_nodes, vars = {specific_vars.bonus_chips}})
        end
    elseif _c.set == "Enhanced" then
        ...
        elseif _c.effect == "Stone Card" then
            loc_vars = {specific_vars and specific_vars.bonus_chips or cfg.bonus}
        ...
        if _c.name ~= "Stone Card" and (specific_vars and specific_vars.bonus_chips or cfg.bonus ~= 0 and cfg.bonus) then
            localize(_G, {type = "other", key = "card_extra_chips", nodes = desc_nodes, vars = {specific_vars and specific_vars.bonus_chips or cfg.bonus}})
        end
    elseif _c.set == "Booster" then
        ...
        if not full_UI_table.name then
            full_UI_table.name = localize(_G, {type = "name", set = "Other", key = name_override, nodes = full_UI_table.name})
        end
        localize(_G, {type = "other", key = desc_override, nodes = desc_nodes, vars = loc_vars})
    elseif _c.set == "Spectral" then
        ...
        elseif _c.name == "Talisman" then
            info_queue[info_queue.length + 1] = G.P_SEALS.gold_seal or G.P_SEALS[SMODS.Seal.badge_to_key.gold_seal or ""]
        elseif _c.name == "Deja Vu" then
            info_queue[info_queue.length + 1] = G.P_SEALS.red_seal or G.P_SEALS[SMODS.Seal.badge_to_key.red_seal or ""]
        elseif _c.name == "Trance" then
            info_queue[info_queue.length + 1] = G.P_SEALS.blue_seal or G.P_SEALS[SMODS.Seal.badge_to_key.blue_seal or ""]
        elseif _c.name == "Medium" then
            info_queue[info_queue.length + 1] = G.P_SEALS.purple_seal or G.P_SEALS[SMODS.Seal.badge_to_key.purple_seal or ""]
    ...
    elseif _c.set == "Planet" then
        loc_vars = {
            ...
            [2] = localize(_G, cfg.hand_type, "poker_hands"),
            [3] = G.GAME.hands[cfg.hand_type].l_mult,
            [4] = G.GAME.hands[cfg.hand_type].l_chips,
            colours = {G.GAME.hands[cfg.hand_type].level == 1 and G.C.UI.TEXT_DARK or G.C.HAND_LEVELS[math.min(7, G.GAME.hands[cfg.hand_type].level) + 1]}
        }
        localize(_G, {
            type = "descriptions",
            key = _c.key,
    ...
    elseif _c.set == "Tarot" then
        if _c.name == "The Fool" then
            ...
            local last_tarot_planet = fool_c and localize(_G, {type = "name_text", key = fool_c.key, set = fool_c.set}) or localize(_G, "k_none")
            ...
            main_end = {{
                ...
                nodes = {{
                    ...
                    nodes = {{
                        n = G.UIT.T,
                        config = {
                            text = " " .. tostring(last_tarot_planet) .. " ",
                            colour = G.C.UI.TEXT_LIGHT,
                            scale = 0.3,
                            shadow = true
                        }
        ...
        elseif _c.name == "The Magician" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The High Priestess" then
            loc_vars = {cfg.planets}
        elseif _c.name == "The Empress" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Emperor" then
            loc_vars = {cfg.tarots}
        elseif _c.name == "The Hierophant" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Lovers" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Chariot" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "Justice" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Hermit" then
            loc_vars = {cfg.extra}
        elseif _c.name == "The Wheel of Fortune" then
            ...
            info_queue[info_queue.length + 1] = G.P_CENTERS.e_holo
            info_queue[info_queue.length + 1] = G.P_CENTERS.e_polychrome
        elseif _c.name == "Strength" then
            loc_vars = {cfg.max_highlighted}
        elseif _c.name == "The Hanged Man" then
            loc_vars = {cfg.max_highlighted}
        elseif _c.name == "Death" then
            loc_vars = {cfg.max_highlighted}
        elseif _c.name == "Temperance" then
            ...
            loc_vars = {
                cfg.extra,
                math.min(cfg.extra, _money)
            }
        elseif _c.name == "The Devil" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Tower" then
            loc_vars = {
                cfg.max_highlighted,
                localize(_G, {type = "name_text", set = "Enhanced", key = cfg.mod_conv})
            }
            info_queue[info_queue.length + 1] = G.P_CENTERS[cfg.mod_conv]
        elseif _c.name == "The Star" then
            loc_vars = {
                [1] = cfg.max_highlighted,
                [2] = localize(_G, cfg.suit_conv, "suits_plural"),
    ...
    if not (specific_vars and not specific_vars.sticker and (card_type == "Default" or card_type == "Enhanced")) then
        if desc_nodes == full_UI_table.main and not full_UI_table.name then
            localize(_G, {type = "name", key = _c.key, set = _c.set, nodes = full_UI_table.name})
            if not full_UI_table.name then
                full_UI_table.name = {}
            end
        elseif desc_nodes ~= full_UI_table.main and not desc_nodes.name then
            desc_nodes.name = localize(_G, {type = "name_text", key = name_override or _c.key, set = name_override and "Other" or _c.set})
        end
    end
    if first_pass and not (_c.set == "Edition") and badges then
        for k, v in ipairs(badges) do
            ...
            if G.P_CENTERS["e_" .. tostring(v)] and G.P_CENTERS["e_" .. tostring(v)].set == "Edition" then
                local t = {
                    key = "e_" .. tostring(v),
                    set = "Edition",
                    config = {}
                ...
                if G.P_CENTERS["e_" .. tostring(v)].loc_vars and type(G.P_CENTERS["e_" .. tostring(v)].loc_vars) == "function" then
                    local res = G.P_CENTERS["e_" .. tostring(v)]:loc_vars(info_queue, card) or ({})
                    t.vars = res.vars
            ...
            if sticker then
                ...
                if v == "perishable" then
                    info_queue[info_queue.length + 1] = {key = "perishable", set = "Other", vars = {G.GAME.perishable_rounds or 1, specific_vars.perish_tally or G.GAME.perishable_rounds}}
                end
                if v == "rental" then
                    info_queue[info_queue.length + 1] = {key = "rental", set = "Other", vars = {G.GAME.rental_rate or 1}}
                end
            end
    ...
    for _, v in ipairs(info_queue) do
        generate_card_ui(_G, v, full_UI_table)
    end
