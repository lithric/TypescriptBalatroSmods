...
function get_first_legendary(self, _key)
    local _t, key = __TS__Unpack(pseudorandom_element(
        _G,
        G.P_JOKER_RARITY_POOLS[5],
        pseudoseed(_G, "Joker4", _key)
    ))
    return _t.key
end
function pseudorandom_element(self, _t, seed, args)
    ...
    for k, v in pairs(_t) do
        local keep = true
        local in_pool_func = args and args.in_pool or type(v) == "table" and type(v.in_pool) == "function" and v.in_pool or _t == G.P_CARDS and (function(self, c)
            local initial_deck = args and args.starting_deck or false
            return not (type(SMODS.Ranks[c.value].in_pool) == "function" and not SMODS.Ranks[c.value]:in_pool({initial_deck = initial_deck, suit = c.suit}) or type(SMODS.Suits[c.suit].in_pool) == "function" and not SMODS.Suits[c.suit]:in_pool({initial_deck = initial_deck, rank = c.value}))
        end)
        if in_pool_func then
            keep = in_pool_func(_G, v, args)
        end
        if keep then
    ...
    local key = keys[math.random(keys.length)].k
    return {_t[key], key}
end
function random_string(self, length, seed)
    if seed then
        math.randomseed(seed)
    end
    local ret = ""
    do
        local i = 1
        while i <= length do
            ret = ret .. string.char(math.random() > 0.7 and math.random(
                string.byte("1"),
                string.byte("9")
            ) or (math.random() > 0.45 and math.random(
                string.byte("A"),
                string.byte("N")
            ) or math.random(
                string.byte("P"),
    ...
    if true then
        ...
        do
            ...
            while i <= 1 do
                h = ____bit.bxor(
                    h,
                    ____bit.lshift(h, 7) + ____bit.rshift(h, 3) + string.byte(str, i)
                )
                i = i + -1
            end
        end
        return tonumber(string.format(
            "%.13f",
            math.sqrt(math.abs(h)) % 1
        ))
    end
end
function pseudoseed(self, key, predict_seed)
    if key == "seed" then
        return math.random()
    end
    if predict_seed then
        local _pseed = pseudohash(_G, key + (predict_seed or ""))
        _pseed = math.abs(tonumber(string.format("%.13f", (2.134453429141 + _pseed * 1.72431234) % 1)))
        return (_pseed + (pseudohash(_G, predict_seed) or 0)) / 2
    end
    if not G.GAME.pseudorandom[key] then
        G.GAME.pseudorandom[key] = pseudohash(_G, key + (G.GAME.pseudorandom.seed or ""))
    end
...
function get_flush(self, hand)
    local ret = {}
    local four_fingers = {next(find_joker(_G, "Four Fingers"))}
    local suits = SMODS.Suit.obj_buffer
    if hand.length < 5 - (four_fingers and 1 or 0) then
        return ret
    else
        do
            local j = 1
...
function get_straight(self, hand)
    local ret = {}
    local four_fingers = {next(find_joker(_G, "Four Fingers"))}
    if hand.length > 5 or hand.length < 5 - (four_fingers and 1 or 0) then
        return ret
    else
        local t = {}
        local IDS = {}
        ...
        local can_skip = {next(find_joker(_G, "Shortcut"))}
...
function get_X_same(self, num, hand, or_more)
    local vals = {}
    do
        local i = 1
        while i <= SMODS.Rank.max_id.value do
            vals[i] = {}
            i = i + 1
        end
    end
    do
        ...
        while i <= 1 do
            ...
            if or_more and curr.length >= num or curr.length == num then
...
function get_highest(self, hand)
    local highest = nil
    ...
    if hand.length > 0 then
        return {{highest}}
    else
        return {}
    end
end
function mix_colours(self, C1, C2, proportionC1)
    return {(C1[1] or 0.5) * proportionC1 + (C2[1] or 0.5) * (1 - proportionC1), (C1[2] or 0.5) * proportionC1 + (C2[2] or 0.5) * (1 - proportionC1), (C1[3] or 0.5) * proportionC1 + (C2[3] or 0.5) * (1 - proportionC1), (C1[4] or 1) * proportionC1 + (C2[4] or 1) * (1 - proportionC1)}
end
function find_joker(self, name, non_debuff)
    local jokers = {}
...
function get_joker_win_sticker(self, _center, index)
    if G.PROFILES[G.SETTINGS.profile + 1].joker_usage[_center.key] and G.PROFILES[G.SETTINGS.profile + 1].joker_usage[_center.key].wins then
        local _w = 0
        for k, v in pairs(G.PROFILES[G.SETTINGS.profile + 1].joker_usage[_center.key].wins) do
            _w = math.max(k, _w)
        end
        if index then
            return _w
...
function set_challenge_unlock(self)
    if G.PROFILES[G.SETTINGS.profile + 1].all_unlocked then
        return
    end
    if G.PROFILES[G.SETTINGS.profile + 1].challenges_unlocked then
        ...
        G.PROFILES[G.SETTINGS.profile + 1].challenges_unlocked = math.min(_ch_tot, _ch_comp + 5)
    else
        local deck_wins = 0
        ...
        if deck_wins >= G.CHALLENGE_WINS and not G.PROFILES[G.SETTINGS.profile + 1].challenges_unlocked then
            G.PROFILES[G.SETTINGS.profile + 1].challenges_unlocked = 5
            notify_alert(_G, "b_challenge", "Back")
        end
    end
end
function dec_stop_use(self, _depth)
    if _depth > 0 then
...
function recursive_table_cull(self, t)
    local ret_t = {}
    for k, v in pairs(t) do
        if type(v) == "table" then
            if v.is and v:is(Object) then
                ...
                ret_t[k] = recursive_table_cull(_G, v)
            end
        else
            ret_t[k] = v
        end
...
function loc_colour(self, _c, _default)
...
function loc_parse_string(self, line)
    local parsed_line = {}
    local control = {}
    ...
    do
        ...
        while i <= line.length do
            ...
            if char == "{" then
                if str_parts[1] then
                    parsed_line[parsed_line.length + 1] = {strings = str_parts, control = control or ({})}
                end
                str_parts, str_it = {}, 1
...
function localize(self, args, misc_cat)
    if args and not (type(args) == "table") then
        if misc_cat and G.localization.misc[misc_cat] then
            return G.localization.misc[misc_cat][args] or "ERROR"
        end
            ...
            for _, lines in ipairs(args.type == "unlocks" and loc_target.unlock_parsed or args.type == "name" and loc_target.name_parsed or args.type == "text" and loc_target or loc_target.text_parsed) do
                ...
                for _, part in ipairs(lines) do
                    ...
                    for _, subpart in ipairs(part.strings) do
                        assembled_string = assembled_string .. tostring(type(subpart) == "string" and subpart or format_ui_value(
                            _G,
                            args.vars[tonumber(subpart[1])]
                        ) or "ERROR")
                    end
    ...
    elseif args.type == "name_text" then
        if {pcall(function()
            ret_string = G.localization.descriptions[args.set or args.node.config.center.set][args.key or args.node.config.center.key].name
        end)} then
        else
            ret_string = "ERROR"
        end
        ...
        for _, lines in ipairs(args.type == "unlocks" and loc_target.unlock_parsed or args.type == "name" and loc_target.name_parsed or (args.type == "text" or args.type == "tutorial" or args.type == "quips") and loc_target or loc_target.text_parsed) do
            ...
            for _, part in ipairs(lines) do
                ...
                if args.type == "name" then
                    final_line[final_line.length + 1] = {
                        n = G.UIT.O,
                        config = {object = DynaText(
                            _G,
                            {
                                string = {assembled_string},
                                colours = {part.control.V and args.vars.colours[tonumber(part.control.V)] or part.control.C and loc_colour(_G, part.control.C) or args.text_colour or G.C.UI.TEXT_LIGHT},
                                bump = true,
                                silent = true,
                                pop_in = 0,
                                pop_in_rate = 4,
                ...
                elseif part.control.E then
                    ...
                    final_line[final_line.length + 1] = {
                        ...
                        config = {object = DynaText(
                            ...
                            {
                                string = {assembled_string},
                                colours = {part.control.V and args.vars.colours[tonumber(part.control.V)] or loc_colour(_G, part.control.C or nil)},
                                float = _float,
                                silent = _silent,
                                pop_in = _pop_in,
                                bump = _bump,
                ...
                elseif part.control.X then
                    final_line[final_line.length + 1] = {
                        ...
                        config = {
                            align = "m",
                            colour = loc_colour(_G, part.control.X),
                            r = 0.05,
                            padding = 0.03,
                            res = 0.15
                        },
                        nodes = {{
                            ...
                            config = {
                                ...
                                colour = loc_colour(_G, part.control.C or nil),
                                scale = 0.32 * (part.control.s and tonumber(part.control.s) or args.scale or 1) * desc_scale
                            }
                        }}
                    }
                    ...
                    final_line[final_line.length + 1] = {
                        ...
                        config = {
                            ...
                            text = assembled_string,
                            shadow = args.shadow,
                            colour = part.control.V and args.vars.colours[tonumber(part.control.V)] or not part.control.C and args.text_colour or loc_colour(_G, part.control.C or nil, args.default_col),
                            scale = 0.32 * (part.control.s and tonumber(part.control.s) or args.scale or 1) * desc_scale
                        }
                    }
                end
...
function PLAY_SOUND(self, args)
    ...
    local s = {sound = love.audio.newSource(
        "resources/sounds/" .. tostring(args.sound_code) .. ".ogg",
        should_stream and "stream" or "static"
    )}
...
function SET_SFX(self, s, args)
    if {string.find(s.sound_code, "music")} then
        ...
        local sound_vol = s.original_volume * (args.sound_settings.volume / 100) * (args.sound_settings.game_sounds_volume / 100)
...
function GET_DISPLAYINFO(self, screenmode, display)
    display = display or G.SETTINGS.WINDOW.selected_display or 1
    ...
    do
        ...
        while i <= display_count do
            G.SETTINGS.WINDOW.DISPLAYS[i + 1] = {}
            G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions = {strings = {}, values = {}}
            G.SETTINGS.WINDOW.display_names[i + 1] = "" .. tostring(i)
            local render_w, render_h = love.window.getDesktopDimensions(i)
            local unscaled_dims = love.window.getFullscreenModes(i)[2]
            G.SETTINGS.WINDOW.DISPLAYS[i + 1].DPI_scale = 1
            G.SETTINGS.WINDOW.DISPLAYS[i + 1].MONITOR_DIMS = unscaled_dims
            if screenmode == "Fullscreen" then
                for _, v in ipairs(love.window.getFullscreenModes(i)) do
                    local _w, _h = v.width * G.SETTINGS.WINDOW.DISPLAYS[i + 1].DPI_scale, v.height * G.SETTINGS.WINDOW.DISPLAYS[i + 1].DPI_scale
                    if _w <= G.SETTINGS.WINDOW.DISPLAYS[i + 1].MONITOR_DIMS.width and _h <= G.SETTINGS.WINDOW.DISPLAYS[i + 1].MONITOR_DIMS.height then
                        G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.strings[#G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.strings + 1 + 1] = "" .. tostring(v.width) .. " X " .. tostring(v.height)
                        G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.values[#G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.values + 1 + 1] = {w = v.width, h = v.height}
                        if i == G.SETTINGS.WINDOW.selected_display and i == display and current_res_values.w == v.width and current_res_values.h == v.height then
                            res_option = #G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.values
                        end
                    end
                end
            elseif screenmode == "Windowed" then
                G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.strings[2] = "-"
                G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.values[2] = {w = 1280, h = 720}
            elseif screenmode == "Borderless" then
                G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.strings[2] = "" .. tostring(G.SETTINGS.WINDOW.DISPLAYS[i + 1].MONITOR_DIMS.width / G.SETTINGS.WINDOW.DISPLAYS[i + 1].DPI_scale) .. " X " .. tostring(G.SETTINGS.WINDOW.DISPLAYS[i + 1].MONITOR_DIMS.height / G.SETTINGS.WINDOW.DISPLAYS[i + 1].DPI_scale)
                G.SETTINGS.WINDOW.DISPLAYS[i + 1].screen_resolutions.values[2] = current_res_values
            end
            i = i + 1
        end
    end
...
function timer_checkpoint(self, label, ____type, reset)
    G.PREV_GARB = G.PREV_GARB or 0
    if not G.F_ENABLE_PERF_OVERLAY then
        return
    end
    G.check = G.check or ({draw = {checkpoint_list = {}, checkpoints = 0, last_time = 0}, update = {checkpoint_list = {}, checkpoints = 0, last_time = 0}})
    local cp = G.check[____type]
    if reset then
        cp.last_time = love.timer.getTime()
        cp.checkpoints = 0
        return
    end
    cp.checkpoint_list[cp.checkpoints + 1 + 1] = cp.checkpoint_list[cp.checkpoints + 1 + 1] or ({})
    cp.checkpoints = cp.checkpoints + 1
    cp.checkpoint_list[cp.checkpoints + 1].label = label .. ": " .. tostring(collectgarbage("count") - G.PREV_GARB)
    cp.checkpoint_list[cp.checkpoints + 1].time = love.timer.getTime()
    cp.checkpoint_list[cp.checkpoints + 1].TTC = cp.checkpoint_list[cp.checkpoints + 1].time - cp.last_time
    cp.checkpoint_list[cp.checkpoints + 1].trend = cp.checkpoint_list[cp.checkpoints + 1].trend or ({})
    cp.checkpoint_list[cp.checkpoints + 1].states = cp.checkpoint_list[cp.checkpoints + 1].states or ({})
    table.insert(cp.checkpoint_list[cp.checkpoints + 1].trend, 1, cp.checkpoint_list[cp.checkpoints + 1].TTC)
    table.insert(cp.checkpoint_list[cp.checkpoints + 1].states, 1, G.STATE)
    cp.checkpoint_list[cp.checkpoints + 1].trend[402] = nil
    cp.checkpoint_list[cp.checkpoints + 1].states[402] = nil
    cp.last_time = cp.checkpoint_list[cp.checkpoints + 1].time
    G.PREV_GARB = collectgarbage("count")
    local av = 0
    for k, v in ipairs(cp.checkpoint_list[cp.checkpoints + 1].trend) do
        av = av + v / #cp.checkpoint_list[cp.checkpoints + 1].trend
    end
    cp.checkpoint_list[cp.checkpoints + 1].average = av
end
function boot_timer(self, _label, _next, progress)
    ...
    if G.F_VERBOSE and not _RELEASE_MODE then
        love.graphics.print(
            "LOADING: " .. tostring(_next),
            realw / 2 - 150,
            realh / 2 + 40
        )
    end
...
function interp(self, per, max, min)
    min = min or 0
    if per and max then
        return per * (max - min) + min
    end
end
function remove_all(self, t)
    do
        local i = #t - 1
        while i >= 0 do
            local v = t[i + 1]
...
function Vector_Dist(self, trans1, trans2, mid)
...
function Vector_Len(self, trans1)
    return math.sqrt(trans1.x * trans1.x + trans1.y * trans1.y)
end
function Vector_Sub(self, trans1, trans2)
    return {x = trans1.x - trans2.x, y = trans1.y - trans2.y}
end
function get_index(self, t, val)
    local index = nil
...
function SWAP(self, t, i, j)
    if not t or not i or not j then
        return
    end
    local temp = t[i]
    t[i] = t[j]
...
function pseudoshuffle(self, list, seed)
    if seed then
        math.randomseed(seed)
    end
    if list[2] and list[2].sort_id then
        table.sort(
            list,
            function(a, b)
    ...
    do
        ...
        while i <= 2 do
            ...
            local ____temp_0 = {list[j + 1], list[i + 1]}
            list[i + 1] = ____temp_0[1]
            list[j + 1] = ____temp_0[2]
            i = i + -1
        end
    end
end
function generate_starting_seed(self)
    if G.GAME.stake >= #G.P_CENTER_POOLS.Stake then
        ...
        for k, v in pairs(G.P_JOKER_RARITY_POOLS[4]) do
            local win_ante = get_joker_win_sticker(_G, v, true)
        ...
        if r_tally > 0 and g_tally > 0 then
            ...
            while not seed_found do
                ...
                seed_found = random_string(_G, 8, extra_num + G.CONTROLLER.cursor_hover.T.x * 0.33411983 + G.CONTROLLER.cursor_hover.T.y * 0.874146 + 0.41231101 * G.CONTROLLER.cursor_hover.time)
                if not r_leg[get_first_legendary(_G, seed_found)] then
                    seed_found = nil
                end
            end
            return seed_found
    ...
    return random_string(_G, 8, G.CONTROLLER.cursor_hover.T.x * 0.33411983 + G.CONTROLLER.cursor_hover.T.y * 0.874146 + 0.41231101 * G.CONTROLLER.cursor_hover.time)
end
function pseudorandom(self, seed, min, max)
    if type(seed) == "string" then
        seed = pseudoseed(_G, seed)
    end
    math.randomseed(seed)
    if min and max then
...
function tprint(self, tbl, indent)
    if not indent then
        indent = 0
    end
    ...
    for k, v in pairs(tbl) do
        ...
        elseif type(v) == "table" then
            if indent >= 10 then
                ...
                toprint = toprint .. tostring(v) .. tprint(_G, v, indent + 1) .. ",\\r\\n"
            end
        else
            toprint = toprint .. "\\\"" .. tostring(v) .. "\\\",\\r\\n"
        end
...
function sortingFunction(self, e1, e2)
    return e1.order < e2.order
end
function HEX(self, hex)
    if #hex <= 6 then
        hex = hex .. "FF"
    end
    local _, _, r, g, b, a = string.find(hex, "(%x%x)(%x%x)(%x%x)(%x%x)")
    local color = {
        (tonumber(r, 16) or 0 / 0) / 255,
        (tonumber(g, 16) or 0 / 0) / 255,
        (tonumber(b, 16) or 0 / 0) / 255,
        (tonumber(a, 16) or 0 / 0) / 255
    }
    return color
end
function get_blind_main_colour(self, blind)
    local disabled = false
    blind = blind or ""
    if blind == "Boss" or blind == "Small" or blind == "Big" then
        G.GAME.round_resets.blind_states = G.GAME.round_resets.blind_states or ({})
        if G.GAME.round_resets.blind_states[blind] == "Defeated" or G.GAME.round_resets.blind_states[blind] == "Skipped" then
            disabled = true
        end
        blind = G.GAME.round_resets.blind_choices[blind]
    end
    return (disabled or not G.P_BLINDS[blind]) and G.C.BLACK or G.P_BLINDS[blind].boss_colour or (blind == "bl_small" and mix_colours(_G, G.C.BLUE, G.C.BLACK, 0.6) or blind == "bl_big" and mix_colours(_G, G.C.ORANGE, G.C.BLACK, 0.6)) or G.C.BLACK
end
function evaluate_poker_hand(self, hand)
    local results = {
        ["Flush Five"] = {},
        ["Flush House"] = {},
        ["Five of a Kind"] = {},
    ...
    local parts = {
        _5 = get_X_same(_G, 5, hand),
        _4 = get_X_same(_G, 4, hand),
        _3 = get_X_same(_G, 3, hand),
        _2 = get_X_same(_G, 2, hand),
        _flush = get_flush(_G, hand),
        _straight = get_straight(_G, hand),
        _highest = get_highest(_G, hand)
    }
    ...
    if parts._2.length == 2 or parts._3.length == 1 and parts._2.length == 1 then
        local fh_hand = {}
        local r = parts._2
        local fh_2a = r[1]
        local fh_2b = r[2]
    ...
    for _, _hand in pairs(SMODS.PokerHands) do
        if _hand.composite and type(_hand.composite) == "function" then
            local other_hands
            local ____temp_1 = _hand:composite(parts)
            results[_hand.key] = ____temp_1[1]
            other_hands = ____temp_1[2]
...
function reset_drawhash(self)
    G.DRAW_HASH = EMPTY(_G, G.DRAW_HASH)
end
function nuGC(self, time_budget, memory_ceiling, disable_otherwise)
    time_budget = time_budget or 0.0003
...
function add_to_drawhash(self, obj)
    if obj then
        G.DRAW_HASH[#G.DRAW_HASH + 1 + 1] = obj
    end
end
function mod_chips(self, _chips)
    if G.GAME.modifiers.chips_dollar_cap then
        _chips = math.min(
            _chips,
            math.max(G.GAME.dollars, 0)
        )
    end
    return _chips
end
function mod_mult(self, _mult)
    return _mult
end
function play_sound(self, sound_code, per, vol)
    if G.F_MUTE then
        return
    end
    if sound_code and G.SETTINGS.SOUND.volume > 0.001 then
        ...
        if G.F_SOUND_THREAD then
            ...
            PLAY_SOUND(_G, G.ARGS.play_sound)
        end
    end
end
function modulate_sound(self, dt)
    G.SPLASH_VOL = 2 * dt * (G.STATE == G.STATES.SPLASH and 1 or 0) + (G.SPLASH_VOL or 1) * (1 - 2 * dt)
    local desired_track = G.video_soundtrack or G.STATE == G.STATES.SPLASH and "" or SMODS.Sound:get_current_music() or G.booster_pack_sparkles and not G.booster_pack_sparkles.REMOVED and "music2" or G.booster_pack_meteors and not G.booster_pack_meteors.REMOVED and "music3" or G.booster_pack and not G.booster_pack.REMOVED and "music2" or G.shop and not G.shop.REMOVED and "music4" or G.GAME.blind and G.GAME.blind.boss and "music5" or "music1"
    ...
    G.ARGS.score_intensity.flames = math.min(1, (G.STAGE == G.STAGES.RUN and 1 or 0) * (G.ARGS.chip_flames and G.ARGS.chip_flames.real_intensity + G.ARGS.chip_flames.change or 0) / 10)
    G.ARGS.score_intensity.organ = G.video_organ or G.ARGS.score_intensity.required_score > 0 and math.max(
        math.min(
            0.4,
    ...
    G.ARGS.ambient_sounds = G.ARGS.ambient_sounds or ({
        ambientFire2 = {volfunc = function(self, _prev_volume)
            return _prev_volume * (1 - dt) + dt * 0.9 * (G.ARGS.score_intensity.flames > 0.3 and 1 or G.ARGS.score_intensity.flames / 0.3)
        end},
        ambientFire1 = {volfunc = function(self, _prev_volume)
            return _prev_volume * (1 - dt) + dt * 0.8 * (G.ARGS.score_intensity.flames > 0.3 and (G.ARGS.score_intensity.flames - 0.3) / 0.7 or 0)
        end},
        ambientFire3 = {volfunc = function(self, _prev_volume)
            return _prev_volume * (1 - dt) + dt * 0.4 * ((G.ARGS.chip_flames and G.ARGS.chip_flames.change or 0) + (G.ARGS.mult_flames and G.ARGS.mult_flames.change or 0))
        end},
        ambientOrgan1 = {volfunc = function(self, _prev_volume)
    ...
    if SMODS.Sound.stop_sounds[desired_track] then
        if SMODS.previous_track ~= "" and stop_sound > 0 then
    ...
    if G.F_SOUND_THREAD then
        ...
        if type(in_sync) == "table" and not in_sync[SMODS.previous_track] or in_sync == false then
            should_sync = false
        end
        if type(out_sync) == "table" and not out_sync[desired_track] or out_sync == false then
            should_sync = false
        end
        if SMODS.previous_track and SMODS.previous_track ~= desired_track and not should_sync then
            G.ARGS.push.type = "restart_music"
            G.SOUND_MANAGER.channel:push(G.ARGS.push)
        end
        SMODS.previous_track = desired_track
    else
        MODULATE(_G, G.ARGS.push)
    end
end
function count_of_suit(self, area, suit)
    local num = 0
...
function get_chosen_triangle_from_rect(self, x, y, w, h, vert)
    local scale = 2
    if vert then
        x = x + math.min(
            0.6 * math.sin(G.TIMERS.REAL * 9) * scale + 0.2,
            0
        )
        return {
            x - 3.5 * scale,
...
function point_translate(self, _T, delta)
    _T.x = _T.x + delta.x or 0
    _T.y = _T.y + delta.y or 0
end
function point_rotate(self, _T, angle)
...
function lighten(self, colour, percent, no_tab)
    if no_tab then
        return {colour[1] * (1 - percent) + percent, colour[2] * (1 - percent) + percent, colour[3] * (1 - percent) + percent, colour[4]}
    end
    return {colour[1] * (1 - percent) + percent, colour[2] * (1 - percent) + percent, colour[3] * (1 - percent) + percent, colour[4]}
end
function darken(self, colour, percent, no_tab)
    if no_tab then
        return {colour[1] * (1 - percent), colour[2] * (1 - percent), colour[3] * (1 - percent), colour[4]}
    end
    return {colour[1] * (1 - percent), colour[2] * (1 - percent), colour[3] * (1 - percent), colour[4]}
end
function adjust_alpha(self, colour, new_alpha, no_tab)
    if no_tab then
        return {colour[1], colour[2], colour[3], new_alpha}
    end
    return {colour[1], colour[2], colour[3], new_alpha}
end
function alert_no_space(self, card, area)
    G.CONTROLLER.locks.no_space = true
    attention_text(
        _G,
        {
            scale = 0.9,
            text = localize(_G, "k_no_space_ex"),
            hold = 0.9,
            align = "cm",
            cover = area,
            cover_padding = 0.1,
            cover_colour = adjust_alpha(_G, G.C.BLACK, 0.7)
        }
    )
    card:juice_up(0.3, 0.2)
    do
        local i = 1
        while i <= area.cards.length do
...
function get_blind_amount(self, ante)
    ...
    if not G.GAME.modifiers.scaling or G.GAME.modifiers.scaling == 1 then
        ...
        local a, b, c, d = amounts[9], 1.6, ante - 8, 1 + 0.2 * (ante - 8)
        local amount = math.floor(a * bit.bxor(
            b + bit.bxor(k * c, d),
            c
        ))
        amount = amount - amount % bit.bxor(
            10,
...
function number_format(self, num, e_switch_point)
    ...
    return sign .. tostring(formatted:reverse():gsub("(%d%d%d)", "%1,"):gsub(",$", ""):reverse())
end
function score_number_scale(self, scale, amt)
    G.E_SWITCH_POINT = G.E_SWITCH_POINT or 100000000000
...
function copy_table(self, O)
    local O_type = type(O)
    local copy
    if O_type == "table" then
        copy = {}
        for ____, ____value in __TS__Iterator(next) do
            local k = ____value[1]
            local v = ____value[2]
            copy[copy_table(_G, k)] = copy_table(_G, v)
        end
        setmetatable(
            copy,
            copy_table(
...
function send_score(self, _score)
...
function check_and_set_high_score(self, score, amt)
    if not amt or type(amt) ~= "number" then
        return
    end
    ...
    if G.PROFILES[G.SETTINGS.profile + 1].high_scores[score] and math.floor(amt) > G.PROFILES[G.SETTINGS.profile + 1].high_scores[score].amt then
        if G.GAME.round_scores[score] then
            G.GAME.round_scores[score].high_score = true
        end
        G.PROFILES[G.SETTINGS.profile + 1].high_scores[score].amt = math.floor(amt)
        G:save_settings()
    end
end
function set_joker_usage(self)
    for k, v in pairs(G.jokers.cards) do
        if v.config.center_key and v.ability.set == "Joker" then
            if G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] then
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].count = G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].count + 1
            else
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] = {
                    count = 1,
...
function set_joker_win(self)
    for k, v in pairs(G.jokers.cards) do
        if v.config.center_key and v.ability.set == "Joker" then
            G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] = G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] or ({
                count = 1,
                order = v.config.center.order,
                wins = {},
                losses = {},
            ...
            if G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] then
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins = G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins or ({})
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins[G.GAME.stake] = (G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins[G.GAME.stake] or 0) + 1
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins_by_key[SMODS:stake_from_index(G.GAME.stake)] = (G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].wins_by_key[SMODS:stake_from_index(G.GAME.stake)] or 0) + 1
            end
        end
    end
    G:save_settings()
end
function set_joker_loss(self)
    for k, v in pairs(G.jokers.cards) do
        if v.config.center_key and v.ability.set == "Joker" then
            if G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key] then
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses = G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses or ({})
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses[G.GAME.stake] = (G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses[G.GAME.stake] or 0) + 1
                G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses_by_key[SMODS:stake_from_index(G.GAME.stake)] = (G.PROFILES[G.SETTINGS.profile + 1].joker_usage[v.config.center_key].losses_by_key[SMODS:stake_from_index(G.GAME.stake)] or 0) + 1
            end
        end
    end
    G:save_settings()
end
function set_deck_usage(self)
    if G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.center and G.GAME.selected_back.effect.center.key then
        ...
        if G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] then
            G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].count = G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].count + 1
        else
            G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] = {
                count = 1,
...
function set_deck_win(self)
    if G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.center and G.GAME.selected_back.effect.center.key then
        local deck_key = G.GAME.selected_back.effect.center.key
        if not G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] then
            G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] = {
                count = 1,
                order = G.GAME.selected_back.effect.center.order,
                wins = {},
        ...
        if G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] then
            G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].wins[G.GAME.stake] = (G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].wins[G.GAME.stake] or 0) + 1
            do
                local i = 1
                while i <= (G.P_CENTER_POOLS.Stake[G.GAME.stake].unlocked_stake and G.P_STAKES[G.P_CENTER_POOLS.Stake[G.GAME.stake].unlocked_stake].stake_level - 1 or G.GAME.stake - 1) do
                    G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].wins[i] = G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].wins[i] or 1
                    i = i + 1
                end
            end
        end
...
function get_deck_win_stake(self, _deck_key)
    if not _deck_key then
        local _w, _w_low = 0, nil
        local deck_count = 0
        for _, deck in pairs(G.PROFILES[G.SETTINGS.profile + 1].deck_usage) do
            local deck_won_with = nil
            for k, v in pairs(deck.wins) do
                deck_won_with = true
                _w = math.max(k, _w)
            end
            if deck_won_with then
    ...
    if G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_deck_key] and G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_deck_key].wins then
        local _w = 0
        for k, v in pairs(G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_deck_key].wins) do
            _w = math.max(k, _w)
        end
        return _w
    end
...
function get_deck_win_sticker(self, _center)
    if G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_center.key] and G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_center.key].wins then
        local _w = -1
        for k, v in pairs(G.PROFILES[G.SETTINGS.profile + 1].deck_usage[_center.key].wins) do
            _w = math.max(k, _w)
        end
        if _w > 0 then
            return G.sticker_map[_w + 1]
...
function set_deck_loss(self)
    if G.GAME.selected_back and G.GAME.selected_back.effect and G.GAME.selected_back.effect.center and G.GAME.selected_back.effect.center.key then
        ...
        if G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key] then
            G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].losses[G.GAME.stake] = (G.PROFILES[G.SETTINGS.profile + 1].deck_usage[deck_key].losses[G.GAME.stake] or 0) + 1
        end
        G:save_settings()
    end
end
function set_consumeable_usage(self, card)
    if card.config.center_key and card.ability.consumeable then
        if G.PROFILES[G.SETTINGS.profile + 1].consumeable_usage[card.config.center_key] then
            G.PROFILES[G.SETTINGS.profile + 1].consumeable_usage[card.config.center_key].count = G.PROFILES[G.SETTINGS.profile + 1].consumeable_usage[card.config.center_key].count + 1
        else
            G.PROFILES[G.SETTINGS.profile + 1].consumeable_usage[card.config.center_key] = {count = 1, order = card.config.center.order}
        end
...
function set_voucher_usage(self, card)
    if card.config.center_key and card.ability.set == "Voucher" then
        if G.PROFILES[G.SETTINGS.profile + 1].voucher_usage[card.config.center_key] then
            G.PROFILES[G.SETTINGS.profile + 1].voucher_usage[card.config.center_key].count = G.PROFILES[G.SETTINGS.profile + 1].voucher_usage[card.config.center_key].count + 1
        else
            G.PROFILES[G.SETTINGS.profile + 1].voucher_usage[card.config.center_key] = {count = 1, order = card.config.center.order}
        end
    end
    G:save_settings()
end
function set_hand_usage(self, hand)
    local hand_label = hand
    hand = hand:gsub("%s+", "")
    if G.PROFILES[G.SETTINGS.profile + 1].hand_usage[hand] then
        G.PROFILES[G.SETTINGS.profile + 1].hand_usage[hand].count = G.PROFILES[G.SETTINGS.profile + 1].hand_usage[hand].count + 1
    else
        G.PROFILES[G.SETTINGS.profile + 1].hand_usage[hand] = {count = 1, order = hand_label}
    end
...
function set_profile_progress(self)
    G.PROGRESS = G.PROGRESS or ({joker_stickers = {tally = 0, of = 0}, deck_stakes = {tally = 0, of = 0}, challenges = {tally = 0, of = 0}})
    ...
    for _, v in pairs(G.P_CENTERS) do
        if v.set == "Back" and not v.omit then
            ...
            G.PROGRESS.deck_stakes.tally = G.PROGRESS.deck_stakes.tally + get_deck_win_stake(_G, v.key)
        end
        if v.set == "Joker" then
            G.PROGRESS.joker_stickers.of = G.PROGRESS.joker_stickers.of + #G.P_CENTER_POOLS.Stake
            G.PROGRESS.joker_stickers.tally = G.PROGRESS.joker_stickers.tally + get_joker_win_sticker(_G, v, true)
        end
    end
    for _, v in pairs(G.CHALLENGES) do
        G.PROGRESS.challenges.of = G.PROGRESS.challenges.of + 1
        if G.PROFILES[G.SETTINGS.profile + 1].challenge_progress.completed[v.id] then
            G.PROGRESS.challenges.tally = G.PROGRESS.challenges.tally + 1
        end
    end
    G.PROFILES[G.SETTINGS.profile + 1].progress.joker_stickers = copy_table(_G, G.PROGRESS.joker_stickers)
    G.PROFILES[G.SETTINGS.profile + 1].progress.deck_stakes = copy_table(_G, G.PROGRESS.deck_stakes)
    G.PROFILES[G.SETTINGS.profile + 1].progress.challenges = copy_table(_G, G.PROGRESS.challenges)
end
function set_discover_tallies(self)
    ...
    for _, v in ipairs(SMODS.ConsumableType.ctype_buffer) do
        G.DISCOVER_TALLIES[tostring(v:lower()) .. "s"] = {tally = 0, of = 0}
    end
    ...
    for _, v in pairs(G.P_CENTERS) do
        if not v.omit and not v.no_collection then
            ...
            if v.set and v.consumeable then
                ...
                local tally = G.DISCOVER_TALLIES[tostring(v.set:lower()) .. "s"]
                if tally then
                    tally.of = tally.of + 1
                    if v.discovered then
    ...
    G.PROFILES[G.SETTINGS.profile + 1].high_scores.collection.amt = G.DISCOVER_TALLIES.total.tally
    G.PROFILES[G.SETTINGS.profile + 1].high_scores.collection.tot = G.DISCOVER_TALLIES.total.of
    G.PROFILES[G.SETTINGS.profile + 1].progress.discovered = copy_table(_G, G.DISCOVER_TALLIES.total)
    if check_for_unlock then
        check_for_unlock(_G, {type = "discover_amount", amount = G.DISCOVER_TALLIES.total.tally, planet_count = G.DISCOVER_TALLIES.planets.tally, tarot_count = G.DISCOVER_TALLIES.tarots.tally})
    end
end
function stop_use(self)
    G.GAME.STOP_USE = (G.GAME.STOP_USE or 0) + 1
    dec_stop_use(_G, 6)
end
function inc_career_stat(self, stat, mod)
    if G.GAME.seeded or G.GAME.challenge then
        return
    end
    if not G.PROFILES[G.SETTINGS.profile + 1].career_stats[stat] then
        G.PROFILES[G.SETTINGS.profile + 1].career_stats[stat] = 0
    end
    G.PROFILES[G.SETTINGS.profile + 1].career_stats[stat] = G.PROFILES[G.SETTINGS.profile + 1].career_stats[stat] + (mod or 0)
    G:save_settings()
end
function save_with_action(self, action)
    G.action = action
    save_run(_G)
    G.action = nil
end
function remove_save(self)
    love.filesystem.remove(tostring(G.SETTINGS.profile) .. "/save.jkr")
    G.SAVED_GAME = nil
    G.FILE_HANDLER.run = nil
end
function init_localization(self)
    ...
    for k, v in pairs(G.localization.misc.v_dictionary) do
        if type(v) == "table" then
            ...
            for kk, vv in ipairs(v) do
                G.localization.misc.v_dictionary_parsed[k][kk] = loc_parse_string(_G, vv)
            end
        else
            G.localization.misc.v_dictionary_parsed[k] = loc_parse_string(_G, v)
        end
    end
    ...
    for k, v in pairs(G.localization.misc.v_text) do
        ...
        for kk, vv in ipairs(v) do
            G.localization.misc.v_text_parsed[k][kk] = loc_parse_string(_G, vv)
        end
    end
    ...
    for k, v in pairs(G.localization.misc.tutorial) do
        ...
        for kk, vv in ipairs(v) do
            G.localization.tutorial_parsed[k][kk] = loc_parse_string(_G, vv)
        end
    end
    ...
    for k, v in pairs(G.localization.misc.quips or ({})) do
        ...
        for kk, vv in ipairs(v) do
            G.localization.quips_parsed[k][kk] = loc_parse_string(_G, vv)
        end
    end
                ...
                for _, center in pairs(set) do
                    ...
                    if not center.text then
                        ...
                        for _, line in ipairs(center.text) do
                            center.text_parsed[center.text_parsed.length + 1] = loc_parse_string(_G, line)
                        end
                        center.name_parsed = {}
                        for _, line in ipairs(type(center.name) == "table" and center.name or ({center.name})) do
                            center.name_parsed[center.name_parsed.length + 1] = loc_parse_string(_G, line)
                        end
                        if center.unlock then
                            center.unlock_parsed = {}
                            for _, line in ipairs(center.unlock) do
                                center.unlock_parsed[center.unlock_parsed.length + 1] = loc_parse_string(_G, line)
                            end
                        end
                    end
                end
...
function playing_card_joker_effects(self, cards)
    do
        local i = 1
...
function convert_save_to_meta(self)
    if love.filesystem.getInfo(tostring(G.SETTINGS.profile) .. "/" .. "unlocked_jokers.jkr") then
        local _meta = {unlocked = {}, alerted = {}, discovered = {}}
        if love.filesystem.getInfo(tostring(G.SETTINGS.profile) .. "/" .. "unlocked_jokers.jkr") then
            for line in string.gmatch(
                (get_compressed(
                    _G,
                    tostring(G.SETTINGS.profile) .. "/" .. "unlocked_jokers.jkr"
                ) or "") .. "\\n",
                "([^\\n]*)\\n"
            ) do
        ...
        if love.filesystem.getInfo(tostring(G.SETTINGS.profile) .. "/" .. "discovered_jokers.jkr") then
            for line in string.gmatch(
                (get_compressed(
                    _G,
                    tostring(G.SETTINGS.profile) .. "/" .. "discovered_jokers.jkr"
                ) or "") .. "\\n",
                "([^\\n]*)\\n"
            ) do
                local key = line:gsub("%s+", "")
        ...
        if love.filesystem.getInfo(tostring(G.SETTINGS.profile) .. "/" .. "alerted_jokers.jkr") then
            for line in string.gmatch(
                (get_compressed(
                    _G,
                    tostring(G.SETTINGS.profile) .. "/" .. "alerted_jokers.jkr"
                ) or "") .. "\\n",
                "([^\\n]*)\\n"
            ) do
                local key = line:gsub("%s+", "")
        ...
        love.filesystem.remove(tostring(G.SETTINGS.profile) .. "/" .. "unlocked_jokers.jkr")
        love.filesystem.remove(tostring(G.SETTINGS.profile) .. "/" .. "discovered_jokers.jkr")
        love.filesystem.remove(tostring(G.SETTINGS.profile) .. "/" .. "alerted_jokers.jkr")
        compress_and_save(
            _G,
...
function card_from_control(self, control)
    ...
    local _card = Card(
        ...
        G.CARD_W,
        G.CARD_H,
        G.P_CARDS[tostring(control.s) .. "_" .. tostring(control.r)],
        G.P_CENTERS[control.e or "c_base"],
...
utf8.map = function(self, s, f, no_subs)
    local i = 0
    if no_subs then
        for ____, ____value in __TS__Iterator(s:gmatch("()" .. tostring(utf8.pattern) .. "()")) do
            local b = ____value[1]
            local e = ____value[2]
            i = i + 1
            local c = e - b
...
utf8.chars = function(self, s, no_subs)
    return coroutine.wrap(function()
        return utf8:map(s, coroutine.yield, no_subs)
    end)
end
function get_stake_sprite(self, _stake, _scale)
    _stake = _stake or 1
    _scale = _scale or 1
    local stake_sprite = Sprite(
        ...
        _scale * 1,
        _scale * 1,
        G.ASSET_ATLAS[G.P_CENTER_POOLS.Stake[_stake].atlas],
        G.P_CENTER_POOLS.Stake[_stake].pos
    )
    stake_sprite.states.drag.can = false
    if G.P_CENTER_POOLS.Stake[_stake].shiny then
        stake_sprite.draw = function(self, _sprite)
...
function get_front_spriteinfo(self, _front)
        ...
        if collab then
            ...
            if deckSkin then
                if deckSkin.outdated then
                    ...
                    do
                        local i = 1
                        while i <= deckSkin.ranks.length do
                    ...
                    return {
                        G.ASSET_ATLAS[G.SETTINGS.colour_palettes[_front.suit] == "hc" and _front.hc_atlas or _front.lc_atlas or ({})] or G.ASSET_ATLAS[_front.atlas] or G.ASSET_ATLAS["cards_" .. tostring(G.SETTINGS.colour_palettes[_front.suit] == "hc" and 2 or 1)],
                        _front.pos
                    }
                else
                    ...
                    do
                        local i = 1
                        while i <= palette.ranks.length do
                    ...
                    return {
                        G.ASSET_ATLAS[palette.hc_default and _front.hc_atlas or _front.lc_atlas or ({})] or G.ASSET_ATLAS[_front.atlas] or G.ASSET_ATLAS["cards_" .. tostring(palette.hc_default and 2 or 1)],
                        _front.pos
                    }
                end
            end
    ...
    return {
        G.ASSET_ATLAS[G.SETTINGS.colourblind_option and _front.hc_atlas or _front.lc_atlas or ({})] or G.ASSET_ATLAS[_front.atlas] or G.ASSET_ATLAS["cards_" .. tostring(G.SETTINGS.colourblind_option and 2 or 1)],
        _front.pos
    }
end
function get_stake_col(self, _stake)
...
function get_challenge_int_from_id(self, _id)
...
function get_starting_params(self)
    return {
        dollars = 4,
        hand_size = 8,
        discards = 3,
...
function get_challenge_rule(self, _challenge, _type, _id)
