...
function set_screen_positions(): void {
  ...
  if (G.STAGE === G.STAGES.MAIN_MENU) {
    if (G.STATE === G.STATES.DEMO_CTA) {
      ...
      G.title_top.T.x = G.TILE_W / 2 - G.title_top.T.w / 2;
      G.title_top.T.y =
        G.TILE_H / 2 - G.title_top.T.h / 2 - ((G.debug_splash_size_toggle && 2) || 1.2);
    }
  ...
  for (const [k, v] of pairs(G.C.BACKGROUND)) {
    if (args.new_colour && (k === "C" || k === "L" || k === "D")) {
      if (args.special_colour && args.tertiary_colour) {
        ...
        let brightness =
          (k === "L" && 1.3) || (k === "D" && ((args.special_colour && 0.4) || 0.7)) || 0.9;
...
function ease_background_colour_blind(state, blind_override): void {
  let blindname =
    blind_override ||
    (G.GAME.blind && G.GAME.blind.name !== "" && G.GAME.blind.name) ||
    "Small Blind";
  ...
  if (state === G.STATES.TAROT_PACK) {
    ease_background_colour({
      ...
      special_colour: darken(G.C.BLACK, 0.2),
      contrast: 1.5,
    });
  } else if (state === G.STATES.SPECTRAL_PACK) {
    ease_background_colour({
...
function add_joker(joker, edition, silent, eternal): void {
  ...
  let _T = (_area && _area.T) || {
    x: G.ROOM.T.w / 2 - G.CARD_W / 2,
    y: G.ROOM.T.h / 2 - G.CARD_H / 2,
  };
  let card = Card(_T.x, _T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, G.P_CENTERS[joker], {
    discover: true,
...
function play_area_status_text(text, silent, delay): void {
  ...
  G.E_MANAGER.add_event(
    new GameEvent({
      trigger: (delay === 0 && "immediate") || "before",
      delay: delay,
      func: function () {
        attention_text({
          scale: 0.9,
...
function level_up_hand(card, hand, instant, amount): void {
  ...
  G.GAME.hands[hand].mult = math.max(
    G.GAME.hands[hand].s_mult + G.GAME.hands[hand].l_mult * (G.GAME.hands[hand].level - 1),
    1
  );
  G.GAME.hands[hand].chips = math.max(
    G.GAME.hands[hand].s_chips + G.GAME.hands[hand].l_chips * (G.GAME.hands[hand].level - 1),
    0
  );
  if (!instant) {
    G.E_MANAGER.add_event(
        ...
        if (vals.chips && G.GAME.current_round.current_hand.chips !== vals.chips) {
          let delta =
            (type(vals.chips) === "number" &&
              ...
              vals.chips - G.GAME.current_round.current_hand.chips) ||
            0;
          if (delta < 0) {
            delta = "" + delta;
            col = G.C.RED;
        ...
        if (vals.mult && G.GAME.current_round.current_hand.mult !== vals.mult) {
          let delta =
            (type(vals.mult) === "number" &&
              ...
              vals.mult - G.GAME.current_round.current_hand.mult) ||
            0;
          if (delta < 0) {
            delta = "" + delta;
            col = G.C.RED;
  ...
  if (card.area && area_set[card.area]) {
    ...
    if (jokers || triggered) {
      ...
      if (
        ...
        SMODS.optional_features.post_trigger
      ) {
...
function set_alerts(): void {
  if (G.REFRESH_ALERTS) {
    ...
    let alert_any =
      ...
      alert_seal ||
      alert_tag;
    G.ARGS.set_alerts_alertables = G.ARGS.set_alerts_alertables || [
    ...
    table.insert(G.ARGS.set_alerts_alertables, {
      id: "mods_button",
      alert_uibox_name: "mods_button_alert",
      should_alert: SMODS.mod_button_alert,
    });
...
function card_eval_status_text(card, eval_type, amt, percent, dir, extra): void {
  ...
  if (eval_type === "debuff") {
    ...
    sound =
      (extra.edition && "foil2") ||
      (extra.mult_mod && "multhit1") ||
      (extra.Xmult_mod && "multhit2") ||
      "generic1";
    if (extra.edition) {
          ...
          new GameEvent({
            ...
            func: function () {
              ...
              G.round_eval.add_child(
                spacer,
                G.round_eval.get_UIE_by_ID(
                  (config.bonus && "bonus_round_eval") || "base_round_eval"
                )
              );
              return true;
            },
      ...
      new GameEvent({
        ...
        func: function () {
          ...
          if (config.name === "blind1") {
            ...
            let blind_sprite = AnimatedSprite(
              0,
              0,
              1.2,
              1.2,
            ...
            let blind_sprite = Sprite(
              0,
              0,
              0.7,
              0.7,
              G.ASSET_ATLAS["tags"],
              copy_table(config.pos)
            );
            blind_sprite.define_draw_steps([
              { shader: "dissolve", shadow_height: 0.05 },
              { shader: "dissolve" },
            ...
            table.insert(left_text, {
              n: G.UIT.O,
              config: { w: 0.7, h: 0.7, object: blind_sprite, hover: true, can_collide: false },
            });
            table.insert(left_text, {
              n: G.UIT.O,
              config: {
            ...
            table.insert(left_text, {
              n: G.UIT.T,
              config: {
                text: config.disp || config.dollars,
                scale: 0.8 * scale,
                colour: G.C.BLUE,
                shadow: true,
                juice: true,
                  ...
                  string: [
                    " " +
                      localize({
                        ...
                        vars: [G.GAME.modifiers.money_per_hand || 1],
                      }),
                  ],
                  colours: [G.C.UI.TEXT_LIGHT],
                  shadow: true,
                  ...
                  string: [
                    " " +
                      localize({
                        ...
                        vars: [G.GAME.modifiers.money_per_discard || 0],
                      }),
                  ],
                  colours: [G.C.UI.TEXT_LIGHT],
                  shadow: true,
          ...
          G.round_eval.add_child(
            full_row,
            G.round_eval.get_UIE_by_ID((config.bonus && "bonus_round_eval") || "base_round_eval")
          );
    ...
    if (num_dollars > 60 || num_dollars < -60) {
                  ...
                  nodes: [
                    {
                      ...
                      config: {
                        object: DynaText({
                          string: ["-" + (localize("$") + format_ui_value(-num_dollars))],
                          colours: [G.C.MONEY],
                          shadow: true,
                          pop_in: 0,
                          scale: 0.65,
      ...
      for (let i = 1; i <= dollars_to_loop; i++) {
        G.E_MANAGER.add_event(
          new GameEvent({
            trigger: "before",
            delay: 0.18 - ((num_dollars > 20 && 0.13) || (num_dollars > 9 && 0.1) || 0),
            func: function () {
              if (i % 30 === 1) {
                G.round_eval.add_child(
                  {
              ...
              if (i === 1 && num_dollars < 0) {
                r = {
                  ...
                  config: {
                    text: "-",
                    colour: G.C.RED,
                    scale: (num_dollars < -20 && 0.28) || (num_dollars < -9 && 0.43) || 0.58,
                    shadow: true,
                    hover: true,
                    can_collide: false,
                    juice: true,
              ...
              if (config.name === "blind1") {
                G.GAME.current_round.dollars_to_be_earned =
                  G.GAME.current_round.dollars_to_be_earned.sub(2);
              }
              G.round_eval.add_child(
                r,
...
function update_canvas_juice(dt): void {
  ...
  let shake_amt =
    (((G.SETTINGS.reduced_motion && 0) || 1) * math.max(0, G.SETTINGS.screenshake - 30)) / 100;
  ...
  G.ARGS.eased_cursor_pos.x =
    G.ARGS.eased_cursor_pos.x * (1 - 3 * dt) +
    3 * dt * (shake_amt * G.CURSOR.T.x + ((1 - shake_amt) * G.ROOM.T.w) / 2);
  G.ARGS.eased_cursor_pos.y =
    G.ARGS.eased_cursor_pos.y * (1 - 3 * dt) +
    3 * dt * (shake_amt * G.CURSOR.T.y + ((1 - shake_amt) * G.ROOM.T.h) / 2);
  G.ARGS.eased_cursor_pos.sx =
    G.ARGS.eased_cursor_pos.sx * (1 - 3 * dt) +
    3 *
      dt *
      (shake_amt * G.CONTROLLER.cursor_position.x +
        ((1 - shake_amt) * G.WINDOWTRANS.real_window_w) / 2);
  G.ARGS.eased_cursor_pos.sy =
    G.ARGS.eased_cursor_pos.sy * (1 - 3 * dt) +
    3 *
      dt *
      (shake_amt * G.CONTROLLER.cursor_position.y +
        ((1 - shake_amt) * G.WINDOWTRANS.real_window_h) / 2);
  ...
  G.ROOM.T.r =
    (0.001 * math.sin(0.3 * G.TIMERS.REAL) +
      0.002 * G.ROOM.jiggle * math.sin(39.913 * G.TIMERS.REAL)) *
    shake_amt;
  G.ROOM.T.x =
    G.ROOM_ORIG.x +
    shake_amt *
      (0.015 * math.sin(0.913 * G.TIMERS.REAL) +
        0.01 * G.ROOM.jiggle * shake_amt * math.sin(19.913 * G.TIMERS.REAL) +
        (G.ARGS.eased_cursor_pos.x - 0.5 * (G.ROOM.T.w + G.ROOM_ORIG.x)) * 0.01);
  G.ROOM.T.y =
    G.ROOM_ORIG.y +
    shake_amt *
      (0.015 * math.sin(0.952 * G.TIMERS.REAL) +
  ...
  if (G.CONTROLLER.GAMEPAD.object && G.F_RUMBLE) {
    G.CONTROLLER.GAMEPAD.object.setVibration(
      G.CURR_VIBRATION * 0.4 * G.F_RUMBLE,
      G.CURR_VIBRATION * 0.4 * G.F_RUMBLE
    );
  }
}
...
function check_for_unlock(args): void {
  ...
  for (const [k, v] of pairs(G.ACHIEVEMENTS)) {
    if (
      !v.earned &&
      v.unlock_condition &&
      type(v.unlock_condition) === "function" &&
  ...
  if (args.type === "career_stat") {
    if (
      args.statname === "c_cards_played" &&
      G.PROFILES[G.SETTINGS.profile].career_stats[args.statname] >= 2500
    ) {
      unlock_achievement("card_player");
    }
    if (
      args.statname === "c_cards_discarded" &&
      G.PROFILES[G.SETTINGS.profile].career_stats[args.statname] >= 2500
    ) {
  ...
  while (i <= G.P_LOCKED.length) {
    ...
    if (!custom_check && !card.unlocked && card.unlock_condition && args.type === "career_stat") {
      if (
        args.statname === card.unlock_condition.type &&
        G.PROFILES[G.SETTINGS.profile].career_stats[args.statname] >= card.unlock_condition.extra
      ) {
        ret = true;
        unlock_card(card);
      }
    ...
    if (
      ...
      card.unlock_condition.type === args.type
    ) {
      ...
      if (
        args.type === "min_hand_size" &&
        G.hand &&
        G.hand.config.card_limit <= card.unlock_condition.extra
      ) {
        ret = true;
        unlock_card(card);
      }
      if (
        args.type === "interest_streak" &&
        card.unlock_condition.extra <=
          G.PROFILES[G.SETTINGS.profile].career_stats.c_round_interest_cap_streak
      ) {
        ret = true;
        unlock_card(card);
      }
      ...
      if (args.type === "blank_redeems") {
        if (
          G.PROFILES[G.SETTINGS.profile].voucher_usage["v_blank"] &&
          G.PROFILES[G.SETTINGS.profile].voucher_usage["v_blank"].count >=
            card.unlock_condition.extra
        ) {
          unlock_card(card);
        }
        ...
        if (card.unlock_condition.extra.count) {
          ...
          for (const [_, v] of pairs(G.jokers.cards)) {
            if (
              v.ability.set === "Joker" &&
              v.edition &&
              v.edition.polychrome &&
              card.unlock_condition.extra.polychrome
            ) {
              count = count + 1;
            }
          }
      ...
      if (args.type === "round_win") {
        ...
        if (card.name === "Troubadour") {
          if (
            G.PROFILES[G.SETTINGS.profile].career_stats.c_single_hand_round_streak >=
            card.unlock_condition.extra
          ) {
            ret = true;
            unlock_card(card);
        ...
        if (card.name === "Hanging Chad") {
          if (
            G.GAME.last_hand_played === card.unlock_condition.extra &&
            G.GAME.blind.get_type() === "Boss"
          ) {
            ret = true;
            unlock_card(card);
...
function unlock_achievement(achievement_name): void {
  if (
    (G.PROFILES[G.SETTINGS.profile].all_unlocked &&
      G.ACHIEVEMENTS &&
      G.ACHIEVEMENTS[achievement_name] &&
      !G.ACHIEVEMENTS[achievement_name].bypass_all_unlocked &&
      SMODS.config.achievements < 3) ||
    (SMODS.config.achievements < 3 && (G.GAME.seeded || G.GAME.challenge))
  ) {
    return true;
  }
  G.E_MANAGER.add_event(
    ...
    new GameEvent({
      ...
      func: function () {
        G.achievement_notification.alignment.offset.x =
          G.ROOM.T.x - G.achievement_notification.UIRoot.children[1].children[1].T.w - 0.8;
        return true;
      },
    }),
    "achievement"
        ...
        func: function () {
          if (!G.OVERLAY_MENU) {
            ...
            G.FUNCS.overlay_menu({
              definition:
                (G.P_CENTERS[key].set === "Back" && create_UIBox_deck_unlock(G.P_CENTERS[key])) ||
                create_UIBox_card_unlock(G.P_CENTERS[key]),
            });
...
function create_playing_card(card_init, area, skip_materialize, silent, colours): void {
  ...
  let card = Card(_area.T.x, _area.T.y, G.CARD_W, G.CARD_H, card_init.front, card_init.center, {
...
function get_current_pool(_type, _rarity, _legendary, _append): void {
  ...
  if (_type === "Joker") {
    _rarity =
      (_legendary && 4) ||
      (type(_rarity) === "number" && ((_rarity > 0.95 && 3) || (_rarity > 0.7 && 2) || 1)) ||
      _rarity;
    _rarity = { Common: 1, Uncommon: 2, Rare: 3, Legendary: 4 }[_rarity] || _rarity;
    let rarity =
      _rarity ||
      SMODS.poll_rarity("Joker", "rarity" + (G.GAME.round_resets.ante + (_append || "")));
    [_starting_pool, _pool_key] = [
      G.P_JOKER_RARITY_POOLS[rarity],
      "Joker" + (rarity + ((!_legendary && _append) || "")),
    ];
    ...
    if (_legendary && rarities.legendary) {
      ...
      rarity =
        ...
        SMODS.poll_rarity(
          _type,
          "rarity_" + (_type + (G.GAME.round_resets.ante + (_append || "")))
        );
    }
    [_starting_pool, _pool_key] = [
      SMODS.ObjectTypes[_type].rarity_pools[rarity],
      _type + (rarity + (_append || "")),
    ];
  } else {
  ...
  for (const [k, v] of ipairs(_starting_pool)) {
    ...
    if (_type === "Enhanced") {
      ...
      if (
        (!v.requires || (G.P_CENTERS[v.requires] && G.P_CENTERS[v.requires].discovered)) &&
        (!v.min_ante || v.min_ante <= G.GAME.round_resets.ante)
      ) {
        add = true;
      }
    } else if (
      !(G.GAME.used_jokers[v.key] && !pool_opts.allow_duplicates && !next(find_joker("Showman"))) &&
      (v.unlocked !== false || v.rarity === 4)
    ) {
      if (v.set === "Voucher") {
  ...
  if (_pool_size === 0) {
    ...
    if (
      SMODS.ObjectTypes[_type] &&
      SMODS.ObjectTypes[_type].default &&
      G.P_CENTERS[SMODS.ObjectTypes[_type].default]
    ) {
...
function create_card(
  ...
  if (!forced_key && soulable && !G.GAME.banned_keys["c_soul"]) {
    for (const [_, v] of ipairs(SMODS.Consumable.legendaries)) {
      if (
        (_type === v.type.key || _type === v.soul_set) &&
        !(G.GAME.used_jokers[v.key] && !next(find_joker("Showman")) && !v.can_repeat_soul) &&
        (!v.in_pool || type(v.in_pool) !== "function" || v.in_pool())
      ) {
        if (
          pseudorandom("soul_" + (v.key + (_type + G.GAME.round_resets.ante))) >
          1 - v.soul_rate
        ) {
          forced_key = v.key;
        }
    ...
    if (
      (_type === "Tarot" || _type === "Spectral" || _type === "Tarot_Planet") &&
      !(G.GAME.used_jokers["c_soul"] && !next(find_joker("Showman")))
    ) {
    ...
    if (
      (_type === "Planet" || _type === "Spectral") &&
      !(G.GAME.used_jokers["c_black_hole"] && !next(find_joker("Showman")))
    ) {
      if (pseudorandom("soul_" + (_type + G.GAME.round_resets.ante)) > 0.997) {
  ...
  let front =
    ((_type === "Base" || _type === "Enhanced") &&
      pseudorandom_element(
        G.P_CARDS,
        pseudoseed("front" + ((key_append || "") + G.GAME.round_resets.ante))
      )) ||
    undefined;
  let card = Card(area.T.x + area.T.w / 2, area.T.y, G.CARD_W, G.CARD_H, front, center, {
    bypass_discovery_center:
      area === G.shop_jokers ||
      area === G.pack_cards ||
      area === G.shop_vouchers ||
      (G.shop_demo && area === G.shop_demo) ||
      area === G.jokers ||
      area === G.consumeables,
    bypass_discovery_ui:
      area === G.shop_jokers ||
      ...
      area === G.shop_vouchers ||
      (G.shop_demo && area === G.shop_demo),
    discover: area === G.jokers || area === G.consumeables,
    bypass_back: G.GAME.selected_back.pos,
  });
  ...
  for (const [k, v] of ipairs(SMODS.Sticker.obj_buffer)) {
    ...
    if (
      ...
      type(sticker.should_apply) === "function" &&
      sticker.should_apply(card, center, area)
    ) {
  ...
  if (_type === "Joker") {
    ...
    if (area === G.shop_jokers || area === G.pack_cards) {
      let eternal_perishable_poll = pseudorandom(
        ((area === G.pack_cards && "packetper") || "etperpoll") + G.GAME.round_resets.ante
      );
      if (
        G.GAME.modifiers.enable_eternals_in_shop &&
        eternal_perishable_poll > 0.7 &&
        !SMODS.Stickers["eternal"].should_apply
      ) {
        card.set_eternal(true);
      } else if (
        G.GAME.modifiers.enable_perishables_in_shop &&
        eternal_perishable_poll > 0.4 &&
        eternal_perishable_poll <= 0.7 &&
        !SMODS.Stickers["perishable"].should_apply
      ) {
        card.set_perishable(true);
      }
      if (
        G.GAME.modifiers.enable_rentals_in_shop &&
        pseudorandom(((area === G.pack_cards && "packssjr") || "ssjr") + G.GAME.round_resets.ante) >
          0.7 &&
        !SMODS.Stickers["rental"].should_apply
      ) {
...
function copy_card(other, new_card, card_scale, playing_card, strip_edition): void {
  let new_card =
    ...
    Card(
      other.T.x,
      other.T.y,
      G.CARD_W * (card_scale || 1),
      G.CARD_H * (card_scale || 1),
      G.P_CARDS.empty,
      G.P_CENTERS.c_base,
...
function tutorial_info(args): void {
  ...
  let pos = args.pos || {
    x: attach.major.T.x + attach.major.T.w / 2,
    y: attach.major.T.y + attach.major.T.h / 2,
  };
  ...
  G.E_MANAGER.add_event(
    new GameEvent({
      ...
      func: function () {
        if (
          G.OVERLAY_TUTORIAL &&
          G.OVERLAY_TUTORIAL.step === step &&
          !G.OVERLAY_TUTORIAL.step_complete
        ) {
          ...
          if (args.content) {
            G.OVERLAY_TUTORIAL.content = UIBox({
              ...
              config: {
                align: (args.content_config && args.content_config.align) || "cm",
                offset: (args.content_config && args.content_config.offset) || { x: 0, y: 0 },
                major:
                  (args.content_config && args.content_config.major) || G.OVERLAY_TUTORIAL.Jimbo,
                bond: "Weak",
              },
            });
          ...
          G.OVERLAY_TUTORIAL.Jimbo.say_stuff(
            2 * (G.localization.misc.tutorial[args.text_key] || {}).length + 1
          );
          if (args.snap_to) {
            G.E_MANAGER.add_event(
              new GameEvent({
                ...
                func: function () {
                  if (
                    G.OVERLAY_TUTORIAL &&
                    G.OVERLAY_TUTORIAL.Jimbo &&
                    !G.OVERLAY_TUTORIAL.Jimbo.talking
                  ) {
        ...
        return (
          !G.OVERLAY_TUTORIAL || G.OVERLAY_TUTORIAL.step > step || G.OVERLAY_TUTORIAL.skip_steps
        );
      },
    }),
    "tutorial"
...
function calculate_reroll_cost(skip_increment): void {
  ...
  G.GAME.current_round.reroll_cost =
    (G.GAME.round_resets.temp_reroll_cost || G.GAME.round_resets.reroll_cost) +
...
function reset_idol_card(): void {
  ...
  if (valid_idol_cards[1]) {
    let idol_card = pseudorandom_element(
      valid_idol_cards,
      pseudoseed("idol" + G.GAME.round_resets.ante)
    );
...
function reset_mail_rank(): void {
  ...
  if (valid_mail_cards[1]) {
    let mail_card = pseudorandom_element(
      valid_mail_cards,
      pseudoseed("mail" + G.GAME.round_resets.ante)
    );
...
function reset_ancient_card(): void {
  ...
  let ancient_card = pseudorandom_element(
    ancient_suits,
    pseudoseed("anc" + G.GAME.round_resets.ante)
  );
...
function reset_castle_card(): void {
  ...
  if (valid_castle_cards[1]) {
    let castle_card = pseudorandom_element(
      valid_castle_cards,
      pseudoseed("cas" + G.GAME.round_resets.ante)
    );
...
function get_new_boss(): void {
  ...
  for (const [k, v] of pairs(G.P_BLINDS)) {
    if (!v.boss) {
      ...
      if (
        (G.GAME.round_resets.ante % G.GAME.win_ante === 0 && G.GAME.round_resets.ante >= 2) ===
          (v.boss.showdown || false) ||
      ...
      (math.max(1, G.GAME.round_resets.ante) % G.GAME.win_ante !== 0 ||
        G.GAME.round_resets.ante < 2)
    ) {
      eligible_bosses[k] = true;
    } else if (
      v.boss.showdown &&
      G.GAME.round_resets.ante % G.GAME.win_ante === 0 &&
      G.GAME.round_resets.ante >= 2
    ) {
      eligible_bosses[k] = true;
    }
...
function get_type_colour(_c, card): void {
  return (
    (_c.unlocked === false && !(card && card.bypass_lock) && G.C.BLACK) ||
    (_c.unlocked !== false &&
      (_c.set === "Joker" || _c.consumeable || _c.set === "Voucher") &&
      !_c.discoveredand &&
      !((_c.area !== G.jokers && _c.area !== G.consumeables && _c.area) || !_c.area) &&
      G.C.JOKER_GREY) ||
    (card && card.debuff && mix_colours(G.C.RED, G.C.GREY, 0.7)) ||
    (_c.set === "Joker" && G.C.RARITY[_c.rarity]) ||
    (_c.set === "Edition" && G.C.DARK_EDITION) ||
    (_c.set === "Booster" && G.C.BOOSTER) ||
    G.C.SECONDARY_SET[_c.set] || [0, 1, 1, 1]
  );
}
function generate_card_ui(
  ...
  if (_c.set === "Other") {
    ...
    if (_c.wip) {
      ...
      if (_c.locked_loc_vars && type(_c.locked_loc_vars) === "function") {
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_losses,
        ];
      } else if (_c.name === "Acrobat") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_hands_played,
        ];
      } else if (_c.name === "Sock and Buskin") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_face_cards_played,
        ];
      } else if (_c.name === "Swashbuckler") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_jokers_sold,
        ];
        ...
        loc_vars = [
          ...
          localize({ type: "name_text", key: _c.unlock_condition.extra.e_key, set: "Enhanced" }),
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra.count,
          localize(_c.unlock_condition.extra.suit, "suits_singular"),
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_cards_sold,
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_shop_dollars_spent,
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_tarots_bought,
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_shop_rerolls,
        ];
      } else if (_c.name === "Omen Globe") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_tarot_reading_used,
        ];
      } else if (_c.name === "Observatory") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_planetarium_used,
        ];
      } else if (_c.name === "Nacho Tong") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_cards_played,
        ];
      } else if (_c.name === "Recyclomancy") {
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_cards_discarded,
        ];
        ...
        loc_vars = [
          _c.unlock_condition.extra,
          G.PROFILES[G.SETTINGS.profile].career_stats.c_playing_cards_bought,
        ];
      ...
      if (_c.rarity && _c.rarity === 4 && specific_vars && !specific_vars.not_hidden) {
        localize({
          type: "unlocks",
          key: res.key || "joker_locked_legendary",
          set: res.set || "Other",
          nodes: desc_nodes,
          vars: loc_vars,
    ...
    localize({
      type: "other",
      key: "debuffed_" + ((specific_vars.playing_card && "playing_card") || "default"),
      nodes: desc_nodes,
    });
    ...
    if (!card) {
      ...
      let ret = [
        Card.generate_UIBox_ability_table(
          { ability: ability, config: { center: _c }, bypass_lock: true },
          true
        ),
      ];
      specific_vars = ret[1];
    ...
    if (
      _c.name !== "Stone Card" &&
      ((specific_vars && specific_vars.bonus_chips) || (cfg.bonus !== 0 && cfg.bonus))
    ) {
      localize({
        type: "other",
        key: "card_extra_chips",
        nodes: desc_nodes,
        vars: [(specific_vars && specific_vars.bonus_chips) || cfg.bonus],
      });
    }
    ...
    if (_c.name === "Familiar" || _c.name === "Grim" || _c.name === "Incantation") {
      ...
      info_queue[info_queue.length + 1] =
        G.P_SEALS["gold_seal"] || G.P_SEALS[SMODS.Seal.badge_to_key["gold_seal"] || ""];
    } else if (_c.name === "Deja Vu") {
      info_queue[info_queue.length + 1] =
        G.P_SEALS["red_seal"] || G.P_SEALS[SMODS.Seal.badge_to_key["red_seal"] || ""];
    } else if (_c.name === "Trance") {
      info_queue[info_queue.length + 1] =
        G.P_SEALS["blue_seal"] || G.P_SEALS[SMODS.Seal.badge_to_key["blue_seal"] || ""];
    } else if (_c.name === "Medium") {
      info_queue[info_queue.length + 1] =
        G.P_SEALS["purple_seal"] || G.P_SEALS[SMODS.Seal.badge_to_key["purple_seal"] || ""];
    ...
    loc_vars = {
      ...
      colours: [
        (G.GAME.hands[cfg.hand_type].level === 1 && G.C.UI.TEXT_DARK) ||
          G.C.HAND_LEVELS[math.min(7, G.GAME.hands[cfg.hand_type].level)],
      ],
    };
    localize({
      type: "descriptions",
    ...
    if (_c.name === "The Fool") {
      ...
      let last_tarot_planet =
        (fool_c && localize({ type: "name_text", key: fool_c.key, set: fool_c.set })) ||
        localize("k_none");
      ...
      loc_vars = [
        cfg.max_highlighted,
        localize({ type: "name_text", set: "Enhanced", key: cfg.mod_conv }),
      ];
  ...
  if (
    card_type === "Default" ||
    (card_type === "Enhanced" && !_c.replace_base_card && card && card.base)
  ) {
    if (!_c.no_suit) {
  ...
  if (
    !(
      specific_vars &&
      !specific_vars.sticker &&
      (card_type === "Default" || card_type === "Enhanced")
    )
  ) {
    if (desc_nodes === full_UI_table.main && !full_UI_table.name) {
      ...
      desc_nodes.name = localize({
        type: "name_text",
        key: name_override || _c.key,
        set: (name_override && "Other") || _c.set,
      });
    }
  }
      ...
      if (sticker) {
        ...
        if (v === "perishable") {
          info_queue[info_queue.length + 1] = {
            ...
            set: "Other",
            vars: [
              G.GAME.perishable_rounds || 1,
              specific_vars.perish_tally || G.GAME.perishable_rounds,
            ],
          };
        }
        if (v === "rental") {
