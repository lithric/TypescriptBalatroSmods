...
G.FUNCS.blueprint_compat = function (e) {
  if (
    e.config.ref_table.ability.blueprint_compat !==
    e.config.ref_table.ability.blueprint_compat_check
  ) {
    ...
    e.config.ref_table.ability.blueprint_compat_ui =
      " " + (localize("k_" + e.config.ref_table.ability.blueprint_compat) + " ");
...
G.FUNCS.can_buy_and_use = function (e) {
  if (
    (e.config.ref_table.cost > G.GAME.dollars - G.GAME.bankrupt_at &&
      e.config.ref_table.cost > 0) ||
    !e.config.ref_table.can_use_consumeable()
  ) {
...
G.FUNCS.HUD_blind_debuff_prefix = function (e) {
  if (
    (G.GAME.blind && G.GAME.blind.name === "The Wheel" && !G.GAME.blind.disabled) ||
    e.config.id === "bl_wheel"
  ) {
...
G.FUNCS.HUD_blind_reward = function (e) {
  if (
    ...
    G.GAME.blind &&
    G.GAME.modifiers.no_blind_reward[G.GAME.blind.get_type()]
  ) {
...
G.FUNCS.can_delete_profile = function (e) {
  G.CHECK_PROFILE_DATA =
    G.CHECK_PROFILE_DATA || love.filesystem.getInfo(G.focused_profile + ("/" + "profile.jkr"));
  ...
  if (!_infotip_object.config.set && !G.F_NO_ACHIEVEMENTS) {
    ...
    _infotip_object.config.object = new UIBox({
      definition: overlay_infotip(
        localize((G.F_TROPHIES && "ml_unlock_all_trophies") || "ml_unlock_all_explanation")
      ),
...
G.FUNCS.flash = function (e) {
  if (
    ...
    G.CONTROLLER.text_input_id === e.config.id.sub(1, string.len(G.CONTROLLER.text_input_id))
  ) {
...
G.FUNCS.toggle_button = function (e) {
  e.config.ref_table.ref_table[e.config.ref_table.ref_value] =
    !e.config.ref_table.ref_table[e.config.ref_table.ref_value];
...
G.FUNCS.slider = function (e) {
  ...
  if (
    ...
    (G.CONTROLLER.dragging.target === e || G.CONTROLLER.dragging.target === c)
  ) {
    ...
    rt.ref_table[rt.ref_value] = math.min(
      ...
      math.max(
        rt.min,
        rt.min + ((rt.max - rt.min) * (G.CURSOR.T.x - e.parent.T.x - G.ROOM.T.x)) / e.T.w
      )
    );
...
G.FUNCS.option_cycle = function (e) {
  ...
  e.config.ref_table.current_option_val =
    e.config.ref_table.options[e.config.ref_table.current_option];
    ...
    for (let j = 1; j <= G.your_collection.length; j++) {
      let center =
        G.P_CENTER_POOLS["Joker"][
          i + (j - 1) * 5 + 5 * G.your_collection.length * (args.cycle_config.current_option - 1)
        ];
      if (!center) {
        break;
      }
...
G.FUNCS.your_collection_tarot_page = function (args) {
  ...
  for (let j = 1; j <= G.your_collection.length; j++) {
    for (let i = 1; i <= 4 + j; i++) {
      let center =
        G.P_CENTER_POOLS["Tarot"][i + (j - 1) * 5 + 11 * (args.cycle_config.current_option - 1)];
      if (!center) {
        break;
      }
      let card = new Card(
...
G.FUNCS.your_collection_spectral_page = function (args) {
  ...
  for (let j = 1; j <= G.your_collection.length; j++) {
    for (let i = 1; i <= 3 + j; i++) {
      let center =
        G.P_CENTER_POOLS["Spectral"][i + (j - 1) * 4 + 9 * (args.cycle_config.current_option - 1)];
      if (!center) {
        break;
      }
      let card = new Card(
...
G.FUNCS.your_collection_booster_page = function (args) {
  ...
  for (let j = 1; j <= G.your_collection.length; j++) {
    for (let i = 1; i <= 4; i++) {
      let center =
        G.P_CENTER_POOLS["Booster"][i + (j - 1) * 4 + 8 * (args.cycle_config.current_option - 1)];
      if (!center) {
        break;
      }
      let card = new Card(
...
G.FUNCS.your_collection_voucher_page = function (args) {
  ...
  for (let i = 1; i <= 4; i++) {
    for (let j = 1; j <= G.your_collection.length; j++) {
      let center =
        G.P_CENTER_POOLS["Voucher"][i + (j - 1) * 4 + 8 * (args.cycle_config.current_option - 1)];
      if (!center) {
        break;
      }
      let card = new Card(
...
G.FUNCS.change_vsync = function (args) {
  G.SETTINGS.QUEUED_CHANGE.vsync =
    (G.SETTINGS.WINDOW.vsync === 0 && args.to_key === 1 && 1) ||
    (G.SETTINGS.WINDOW.vsync === 1 && args.to_key === 2 && 0) ||
    undefined;
};
...
G.FUNCS.change_window_cycle_UI = function () {
  if (G.OVERLAY_MENU) {
    ...
    if (swap_node) {
      let [focused_display, focused_screenmode] = [
        G.SETTINGS.QUEUED_CHANGE.selected_display || G.SETTINGS.WINDOW.selected_display,
...
G.FUNCS.change_collab = function (args) {
  G.SETTINGS.CUSTOM_DECK.Collabs[args.cycle_config.curr_suit] =
    G.COLLABS.options[args.cycle_config.curr_suit][args.to_key] || "default";
...
G.FUNCS.text_input = function (e) {
  ...
  if (G.CONTROLLER.text_input_hook === e) {
    ...
    args.current_prompt_text =
      (args.text.ref_table[args.text.ref_value] === "" && args.prompt_text) || "";
  ...
  if (G.CONTROLLER.text_input_hook === e && G.CONTROLLER.HID.controller) {
    if (!OSkeyboard_e.children.controller_keyboard) {
      OSkeyboard_e.children.controller_keyboard = new UIBox({
        definition: create_keyboard_input({
          backspace_key: true,
          return_key: true,
          space_key: false,
        }),
        config: {
          align: "cm",
          offset: { x: 0, y: G.CONTROLLER.text_input_hook.config.ref_table.keyboard_offset || -4 },
          major: e.UIBox,
          parent: OSkeyboard_e,
        },
      });
...
G.FUNCS.text_input_key = function (args) {
  ...
  let corpus =
    ...
    ((hook.config.ref_table.extended_corpus && " 0!$&()<>?:{}+-=,.[]_") || "");
  ...
  if (string.len(text.ref_table[text.ref_value]) > 0 && args.key === "BACKSPACE") {
    ...
    string.len(args.key) === 1 &&
    string.find(corpus, args.key, 1, true)
  ) {
...
function GET_TEXT_FROM_INPUT(): void {
  ...
  for (let i = 1; i <= hook.children.length; i++) {
    if (
      hook.children[i].config &&
      hook.children[i].config.id.sub(1, 8 + string.len(G.CONTROLLER.text_input_id)) ===
        G.CONTROLLER.text_input_id + "_letter_" &&
      hook.children[i].config.text !== ""
    ) {
...
function TRANSPOSE_TEXT_INPUT(amount): void {
  ...
  while (amount !== 0) {
    ...
    let real_letter =
      hook.children[position_child + dir].config.id.sub(
        1,
        8 + string.len(G.CONTROLLER.text_input_id)
      ) ===
        G.CONTROLLER.text_input_id + "_letter_" &&
      hook.children[position_child + dir].config.text !== "";
...
G.FUNCS.can_apply_window_changes = function (e) {
  ...
  if (G.SETTINGS.QUEUED_CHANGE) {
    if (
      G.SETTINGS.QUEUED_CHANGE.screenmode &&
      G.SETTINGS.QUEUED_CHANGE.screenmode !== G.SETTINGS.WINDOW.screenmode
    ) {
      can_apply = true;
      ...
      G.SETTINGS.QUEUED_CHANGE.selected_display &&
      G.SETTINGS.QUEUED_CHANGE.selected_display !== G.SETTINGS.WINDOW.selected_display
    ) {
      can_apply = true;
    }
...
G.FUNCS.apply_window_changes = function (_initial) {
  G.SETTINGS.WINDOW.screenmode =
    (G.SETTINGS.QUEUED_CHANGE && G.SETTINGS.QUEUED_CHANGE.screenmode) ||
    G.SETTINGS.WINDOW.screenmode ||
    "Windowed";
  G.SETTINGS.WINDOW.selected_display =
    (G.SETTINGS.QUEUED_CHANGE && G.SETTINGS.QUEUED_CHANGE.selected_display) ||
    G.SETTINGS.WINDOW.selected_display ||
    1;
  G.SETTINGS.WINDOW.DISPLAYS[G.SETTINGS.WINDOW.selected_display].screen_res = {
    w:
      (G.SETTINGS.QUEUED_CHANGE &&
        ...
        G.SETTINGS.QUEUED_CHANGE.screenres.w) ||
      (G.SETTINGS.screen_res && G.SETTINGS.screen_res.w) ||
      love.graphics.getWidth(),
    h:
      (G.SETTINGS.QUEUED_CHANGE &&
        G.SETTINGS.QUEUED_CHANGE.screenres &&
        G.SETTINGS.QUEUED_CHANGE.screenres.h) ||
      (G.SETTINGS.screen_res && G.SETTINGS.screen_res.h) ||
      love.graphics.getHeight(),
  };
  G.SETTINGS.WINDOW.vsync =
    (G.SETTINGS.QUEUED_CHANGE && G.SETTINGS.QUEUED_CHANGE.vsync) || G.SETTINGS.WINDOW.vsync || 1;
  love.window.updateMode(
      ...
      G.SETTINGS.WINDOW.DISPLAYS[G.SETTINGS.WINDOW.selected_display].screen_res.h,
    {
      ...
      fullscreentype:
        (G.SETTINGS.WINDOW.screenmode === "Borderless" && "desktop") ||
        (G.SETTINGS.WINDOW.screenmode === "Fullscreen" && "exclusive") ||
        undefined,
...
G.FUNCS.change_tab = function (e) {
  ...
  tab_contents.config.object = new UIBox({
    definition: e.config.ref_table.tab_definition_function(
      e.config.ref_table.tab_definition_function_args
    ),
...
G.FUNCS.overlay_menu = function (args) {
  ...
  args.config = {
    align: args.config.align || "cm",
    offset: args.config.offset || { x: 0, y: 10 },
    major: args.config.major || G.ROOM_ATTACH,
    bond: "Weak",
...
G.FUNCS.settings = function (e, instant) {
  ...
  G.FUNCS.overlay_menu({
    ...
    config: { offset: { x: 0, y: (instant && 0) || 10 } },
  });
};
...
G.FUNCS.setup_run = function (e) {
  ...
  G.FUNCS.overlay_menu({
    definition: G.UIDEF.run_setup(
      (e.config.id === "from_game_over" ||
        e.config.id === "from_game_won" ||
        e.config.id === "challenge_list") &&
        e.config.id
    ),
  });
    ...
    new GameEvent({
      ...
      func: function () {
        if (G.E_MANAGER.queues.unlock.length <= 0 && !G.OVERLAY_MENU) {
          ...
          G.FUNCS.overlay_menu({
            definition: G.UIDEF.run_setup(
              (e.config.id === "from_game_over" || e.config.id === "from_game_won") && e.config.id
            ),
          });
          if (e.config.id === "from_game_over" || e.config.id === "from_game_won") {
            G.OVERLAY_MENU.config.no_esc = true;
...
G.FUNCS.change_challenge_description = function (e) {
  if (G.OVERLAY_MENU) {
    ...
    if (desc_area && desc_area.config.oid !== e.config.id) {
      ...
      desc_area.config.object = new UIBox({
...
G.FUNCS.change_challenge_list_page = function (args) {
  ...
  if (G.OVERLAY_MENU) {
    ...
    if (ch_list) {
      ...
      ch_list.config.object = new UIBox({
    ...
    definition: create_UIBox_generic_options({
      ...
      contents: [
        G.UIDEF.challenge_description(
          get_challenge_int_from_id(e.config.id.id || ""),
          undefined,
          true
        ),
      ],
        ...
        func: function () {
          if (G.OVERLAY_MENU && G.OVERLAY_MENU.get_UIE_by_ID("jimbo_spot")) {
            ...
            Jimbo.add_speech_bubble([
              "Having fun?",
              [{ text: "Wishlist Balatro!", type: "GREEN" }],
            ]);
            Jimbo.say_stuff(5);
          }
          return true;
...
G.FUNCS.warn_lang = function (e) {
  ...
  if (_infotip_object.config.set !== e.config.ref_table.label) {
    ...
    _infotip_object.config.object = new UIBox({
      definition: overlay_infotip({
        [1]: e.config.ref_table.warning[1],
        [2]: e.config.ref_table.warning[2],
        [3]: e.config.ref_table.warning[3],
        ...
        nodes: [
          {
            ...
            nodes: [
              simple_text_container("ml_disabled_seed", {
...
G.FUNCS.flame_handler = function (e) {
  ...
  for (let i = 1; i <= 3; i++) {
    G.C.UI_CHIPLICK[i] = math.min(
      math.max((G.C.UI_CHIPS[i] * 0.5 + G.C.YELLOW[i] * 0.5 + 0.1) ^ 2, 0.1),
      1
    );
    G.C.UI_MULTLICK[i] = math.min(
      math.max((G.C.UI_MULT[i] * 0.5 + G.C.YELLOW[i] * 0.5 + 0.1) ^ 2, 0.1),
      1
    );
  }
  G.ARGS.flame_handler = G.ARGS.flame_handler || {
    chips: {
      id: "flame_chips",
      arg_tab: "chip_flames",
      colour: G.C.UI_CHIPS,
  ...
  for (const [k, v] of pairs(G.ARGS.flame_handler)) {
    if (e.config.id === v.id) {
        ...
        e.config.object.define_draw_steps([
          {
            ...
            send: [
              ...
              { name: "image_details", ref_table: e.config.object, ref_value: "image_dims" },
              {
                ...
                ref_table: e.config.object.RETS,
                ref_value: "get_pos_pixel",
              },
      ...
      if (
        ...
        _F.intensity =
          (((G.pack_cards && !G.pack_cards.REMOVED) || G.TAROT_INTERRUPT) && 0) ||
          math.max(0, math.log(G.ARGS.score_intensity.earned_score, 5) - 2);
      } else {
        _F.intensity = 0;
      }
      ...
      _F.intensity_vel =
        (1 - exptime) * (_F.intensity - _F.real_intensity) * G.real_dt * 25 +
        exptime * _F.intensity_vel;
      _F.real_intensity = math.max(0, _F.real_intensity + _F.intensity_vel);
      _F.change =
        (_F.change || 0) * (1 - 4 * G.real_dt) +
        4 * G.real_dt * ((_F.real_intensity < _F.intensity - 0 && 1) || 0) * _F.real_intensity;
    }
  }
};
G.FUNCS.hand_text_UI_set = function (e) {
  if (
    G.GAME.current_round.current_hand.handname !== G.GAME.current_round.current_hand.handname_text
  ) {
...
G.FUNCS.can_select_card = function (e) {
  if (
    e.config.ref_table.ability.set !== "Joker" ||
    (e.config.ref_table.edition && e.config.ref_table.edition.negative) ||
...
G.FUNCS.show_infotip = function (e) {
  if (e.config.ref_table) {
    e.children.info = new UIBox({
      definition: {
        n: G.UIT.ROOT,
        config: { align: "cm", colour: G.C.CLEAR, padding: 0.02 },
        nodes: e.config.ref_table,
      },
      config: ((!e.config.ref_table ||
        !e.config.ref_table[1].config.card_pos ||
        e.config.ref_table[1].config.card_pos > G.ROOM.T.w * 0.4) && {
        offset: { x: -0.03, y: 0 },
        align: "cl",
        parent: e,
...
G.FUNCS.use_card = function (e, mute, nosave) {
  ...
  G.STATE =
    (G.STATE === G.STATES.TAROT_PACK && G.STATES.TAROT_PACK) ||
    (G.STATE === G.STATES.PLANET_PACK && G.STATES.PLANET_PACK) ||
    (G.STATE === G.STATES.SPECTRAL_PACK && G.STATES.SPECTRAL_PACK) ||
    (G.STATE === G.STATES.STANDARD_PACK && G.STATES.STANDARD_PACK) ||
    (G.STATE === G.STATES.SMODS_BOOSTER_OPENED && G.STATES.SMODS_BOOSTER_OPENED) ||
    (G.STATE === G.STATES.BUFFOON_PACK && G.STATES.BUFFOON_PACK) ||
    G.STATES.PLAY_TAROT;
  G.CONTROLLER.locks.use = true;
  if (
    G.booster_pack &&
    !G.booster_pack.alignment.offset.py &&
    (card.ability.consumeable || !(G.GAME.pack_choices && G.GAME.pack_choices > 1))
  ) {
  ...
  if (booster_obj && booster_obj.select_card) {
    let area =
      (type(booster_obj.select_card) === "table" &&
        (booster_obj.select_card[e.config.ref_table.ability.set] || undefined)) ||
      booster_obj.select_card;
    G[area].emplace(card);
          ...
          G.E_MANAGER.add_event(
            new GameEvent({
              ...
              func: function () {
                ...
                G.CONTROLLER.locks.use = false;
                if (
                  (prev_state === G.STATES.TAROT_PACK ||
                    prev_state === G.STATES.PLANET_PACK ||
                    prev_state === G.STATES.SPECTRAL_PACK ||
                    prev_state === G.STATES.STANDARD_PACK ||
                    prev_state === G.STATES.SMODS_BOOSTER_OPENED ||
                    prev_state === G.STATES.BUFFOON_PACK) &&
                  G.booster_pack
                ) {
...
G.FUNCS.shop_voucher_empty = function (e) {
  if (
    ...
    (G.shop_vouchers.cards[1] || G.GAME.current_round.voucher)
  ) {
    e.states.visible = false;
...
G.FUNCS.check_for_buy_space = function (card) {
  if (
    card.ability.set !== "Voucher" &&
    card.ability.set !== "Enhanced" &&
    card.ability.set !== "Default" &&
    !(
      card.ability.set === "Joker" &&
      G.jokers.cards.length <
        G.jokers.config.card_limit + ((card.edition && card.edition.negative && 1) || 0)
    ) &&
    !(
      card.ability.consumeable &&
      G.consumeables.cards.length <
        G.consumeables.config.card_limit + ((card.edition && card.edition.negative && 1) || 0)
    )
  ) {
...
G.FUNCS.blind_choice_handler = function (e) {
  if (
    !e.config.ref_table.run_info &&
    G.blind_select &&
    G.blind_select.VT.y < 10 &&
    e.config.id &&
    G.blind_select_opts[string.lower(e.config.id)]
  ) {
    if (e.UIBox.role.xy_bond !== "Weak") {
      e.UIBox.set_role({ xy_bond: "Weak" });
    }
    if (
      (e.config.ref_table.deck !== "on" && e.config.id === G.GAME.blind_on_deck) ||
      (e.config.ref_table.deck !== "off" && e.config.id !== G.GAME.blind_on_deck)
    ) {
      ...
      if (e.config.id === G.GAME.blind_on_deck) {
        ...
        _border.config.outline_colour =
          (_border.config.outline && _border.config.outline_colour) ||
          get_blind_main_colour(e.config.id);
        ...
        e.config.colour = adjust_alpha(
          (G.GAME.round_resets.blind_states[e.config.id] === "Skipped" &&
            mix_colours(G.C.BLUE, G.C.L_BLACK, 0.1)) ||
            G.C.L_BLACK,
          0.5
        );
        ...
        if (_tag && _tag_container) {
          if (
            G.GAME.round_resets.blind_states[e.config.id] === "Skipped" ||
            G.GAME.round_resets.blind_states[e.config.id] === "Defeated"
          ) {
          ...
          if (G.GAME.round_resets.blind_states[e.config.id] === "Skipped") {
            _blind_choice.children.alert = new UIBox({
              definition: create_UIBox_card_alert({
                text_rot: -0.35,
                no_bg: true,
...
G.FUNCS.hover_tag_proxy = function (e) {
  ...
  if (
    (e.states.hover.is || e.parent.states.hover.is) &&
    e.created_on_pause === G.SETTINGS.paused &&
    !e.parent.children.alert
  ) {
    let _sprite = e.config.ref_table.get_uibox_table();
    e.parent.children.alert = new UIBox({
    ...
    e.parent.children.alert &&
    ((!e.states.collide.is && !e.parent.states.collide.is) ||
      e.created_on_pause !== G.SETTINGS.paused)
  ) {
...
G.FUNCS.skip_blind = function (e) {
  ...
  if (_tag) {
    ...
    let [skipped, skip_to] = [
      ...
      (G.GAME.blind_on_deck === "Small" && "Big") ||
        (G.GAME.blind_on_deck === "Big" && "Boss") ||
        "Boss",
    ];
...
G.FUNCS.reroll_boss_button = function (e) {
  if (
    ...
    (G.GAME.used_vouchers["v_retcon"] ||
      (G.GAME.used_vouchers["v_directors_cut"] && !G.GAME.round_resets.boss_rerolled))
  ) {
    e.config.colour = G.C.RED;
  ...
  G.E_MANAGER.add_event(
    new GameEvent({
      ...
      func: function () {
        ...
        G.blind_select_opts.boss = new UIBox({
          T: [par.T.x, 0, 0, 0],
          definition: {
            n: G.UIT.ROOT,
            config: { align: "cm", colour: G.C.CLEAR },
      ...
      new GameEvent({
        ...
        func: function () {
          ...
          G.GAME.current_round.hands_left = math.max(
            1,
            G.GAME.round_resets.hands + G.GAME.round_bonus.next_hands
          );
...
G.FUNCS.wipe_on = function (message, no_card, timefac, alt_colour) {
        ...
        nodes: [
          {
            ...
            config: {
              object: DynaText({
                string: v || "",
                colours: [
                  (math.min(G.C.BACKGROUND.C[1], G.C.BACKGROUND.C[2]) > 0.5 && G.C.BLACK) ||
                    G.C.WHITE,
                ],
                shadow: true,
                silent: k !== 1,
  ...
  G.screenwipe = new UIBox({
    definition: {
      n: G.UIT.ROOT,
      config: { align: "cm", minw: 0, minh: 0, padding: 0.15, r: 0.1, colour: G.C.CLEAR },
      nodes: [
        {
          n: G.UIT.R,
          config: { align: "cm" },
          nodes: [
            (message && {
              n: G.UIT.R,
              config: { id: "text", align: "cm", padding: 0.7 },
              nodes: message_t,
            }) ||
              undefined,
            (!no_card && {
              n: G.UIT.O,
              config: { object: G.screenwipecard, role: { role_type: "Major" } },
            }) ||
              undefined,
          ],
        },
  ...
  G.screenwipe.children.particles = Particles(0, 0, 0, 0, {
    ...
    attach: G.screenwipe,
    colours: [alt_colour || G.C.BACKGROUND.C],
  });
