...
function graphics_stress(): void {
    ...
    for (let i = 1; i <= 50; i++) {
        ...
        _r[_r.length + 1] = { n: G.UIT.R, config: { align: "cm", minw: 0.05, minh: 0.05, colour: G.C.BLUE, padding: 0.05 }, nodes: _c };
    }
    let uidef = { n: G.UIT.ROOT, config: { align: "cm", colour: G.C.CLEAR }, nodes: _r };
    G.STRESS = new UIBox({ definition: uidef, config: { align: "cm", offset: { x: 0, y: 0 }, major: G.ROOM_ATTACH } });
}
function aprint(text): void {
    if (_RELEASE_MODE) {
        return;
    }
    attention_text({ text: text, scale: 0.8, hold: 5.7, cover: G.deck || G.MAIN_MENU_UI, cover_colour: G.C.RED, align: "cm" });
}
function play_video(): void {
    G.video_control = G.video_control || [{ video: "A3", _s: 0.1, _e: 4.65, track: "music1" }, { video: "E1", _s: 3.69, _e: 6.55 }, { video: "C3", _s: 1.9, _e: 4.3, track: "music3" }, { video: "E5", _s: 5.9, _e: 9.2, track: "music1" }, { video: "C4a", _s: 1.3, _e: 4.5, track: "music2" }, { video: "E4", _s: 4, _e: 7.2, track: "music1" }, { video: "D4", _s: 0.3, _e: 3.2, track: "music4" }, { video: "C2", _s: 2, _e: 4.4, track: "music1" }, { video: "B3", _s: 2.7, _e: 5.3 }, { video: "B4", _s: 21.5, _e: 24.8 }, { video: "D5", _s: 1.2, _e: 3.8, track: "music1" }, { video_organ: 0.1, video: "E2", _s: 1.5, _e: 4.1 }, { video_organ: 0.2, video: "E3", _s: 3.5, _e: 7.5 }, { video_organ: 0.4, video: "D3", _s: 1.9, _e: 4.3, track: "music1" }];
    G.video_volume = 1;
    G.video_volume_real = 0;
    G.E_MANAGER.add_event(new GameEvent({ blocking: false, blockable: false, func: function () {
    ...
    for (const [k, v] of pairs(G.video_control)) {
        ...
        G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 0.29, func: function () {
                v.video_file.pause();
    ...
    for (const [k, v] of pairs(G.video_control)) {
        if (v.text) {
            G.E_MANAGER.add_event(new GameEvent({ trigger: "before", delay: 1.4, func: function () {
        ...
        delay(0.15);
        G.E_MANAGER.add_event(new GameEvent({ blocking: false, trigger: "after", delay: 0.3, func: function () {
                G.screenglitch = false;
                return true;
            } }));
    }
    let flash_col = copy_table(G.C.WHITE);
    G.E_MANAGER.add_event(new GameEvent({ trigger: "before", delay: 0.6, func: function () {
            G.FUNCS.wipe_on(undefined, true, 2, flash_col);
            return true;
        } }));
    G.E_MANAGER.add_event(new GameEvent({ func: function () {
            G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 0.9, blockable: false, func: function () {
                    G.video.pause();
                    G.video = undefined;
