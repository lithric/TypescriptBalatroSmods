...
function win_game(): void {
  if ((!G.GAME.seeded && !G.GAME.challenge) || SMODS.config.seeded_unlocks) {
    ...
    check_and_set_high_score(
      "win_streak",
      G.PROFILES[G.SETTINGS.profile].high_scores.current_streak.amt + 1
    );
    check_and_set_high_score(
      "current_streak",
      G.PROFILES[G.SETTINGS.profile].high_scores.current_streak.amt + 1
  ...
  if ((!G.GAME.seeded && !G.GAME.challenge) || SMODS.config.seeded_unlocks) {
    G.PROFILES[G.SETTINGS.profile].stake = math.max(
      G.PROFILES[G.SETTINGS.profile].stake || 1,
      (G.GAME.stake || 1) + 1
    );
  }
  G.save_progress();
      ...
      func: function () {
        ...
        if (game_over) {
          ...
          G.GAME.unused_discards =
            (G.GAME.unused_discards || 0) + G.GAME.current_round.discards_left;
          ...
          if (G.GAME.blind.get_type() === "Boss") {
            ...
            if (
              ...
              G.GAME.round_resets.ante === G.GAME.modifiers.set_eternal_ante
            ) {
            ...
            if (
              ...
              G.GAME.round_resets.ante === G.GAME.modifiers.set_joker_slots_ante
            ) {
...
G.FUNCS.draw_from_deck_to_hand = function (e) {
  if (
    ...
    G.hand.config.card_limit <= 0 &&
    G.hand.cards.length === 0
  ) {
    G.STATE = G.STATES.GAME_OVER;
    G.STATE_COMPLETE = false;
    return true;
  ...
  if (
    ...
    (G.GAME.current_round.hands_played > 0 || G.GAME.current_round.discards_used > 0)
  ) {
...
G.FUNCS.discard_cards_from_highlighted = function (e, hook) {
  ...
  let highlighted_count = math.min(
    G.hand.highlighted.length,
    G.discard.config.card_limit - G.play.cards.length
  );
  if (highlighted_count > 0) {
    ...
    for (let i = 1; i <= highlighted_count; i++) {
      ...
      SMODS.calculate_context(
        { discard: true, other_card: G.hand.highlighted[i], full_hand: G.hand.highlighted },
        effects
      );
    ...
    G.GAME.round_scores.cards_discarded.amt =
      G.GAME.round_scores.cards_discarded.amt + cards.length;
...
G.FUNCS.evaluate_play = function (e) {
  let [text, disp_text, poker_hands, scoring_hand, non_loc_disp_text] = G.FUNCS.get_poker_hand_info(
    G.play.cards
  );
  ...
  update_hand_text(
    {
      sound: (G.GAME.current_round.current_hand.handname !== disp_text && "button") || undefined,
      volume: 0.4,
      immediate: true,
      nopulse: undefined,
      delay: (G.GAME.current_round.current_hand.handname !== disp_text && 0.4) || 0,
    },
    {
      handname: disp_text,
  ...
  if (!G.GAME.blind.debuff_hand(G.play.cards, poker_hands, text)) {
    ...
    for (const [_, v] of ipairs(SMODS.get_card_areas("playing_cards"))) {
      SMODS.calculate_main_scoring(
        ...
        (v === G.play && scoring_hand) || undefined
      );
      delay(0.3);
    }
    ...
    for (const [_, area] of ipairs(SMODS.get_card_areas("jokers"))) {
      for (const [_, _card] of ipairs(area.cards)) {
        ...
        let [joker_eval, post] = eval_card(_card, {
          cardarea: G.jokers,
          full_hand: G.play.cards,
          scoring_hand: scoring_hand,
          scoring_name: text,
        ...
        if (next(joker_eval)) {
          ...
          if (joker_eval.retriggers) {
            for (let rt = 1; rt <= joker_eval.retriggers.length; rt++) {
              let [rt_eval, rt_post] = eval_card(_card, {
                cardarea: G.jokers,
                full_hand: G.play.cards,
                scoring_hand: scoring_hand,
                scoring_name: text,
        ...
        for (const [_, _area] of ipairs(SMODS.get_card_areas("jokers"))) {
          for (const [_, _joker] of ipairs(_area.cards)) {
            ...
            let [joker_eval, post] = eval_card(_joker, {
              full_hand: G.play.cards,
              scoring_hand: scoring_hand,
              scoring_name: text,
              poker_hands: poker_hands,
        ...
        let deck_effect = G.GAME.selected_back.trigger_effect({
          full_hand: G.play.cards,
          scoring_hand: scoring_hand,
          scoring_name: text,
          poker_hands: poker_hands,
          other_joker: (_card.ability.set === "Joker" && _card) || false,
          other_consumeable: (_card.ability.set !== "Joker" && _card) || false,
        });
        if (deck_effect) {
    ...
    let [nu_chip, nu_mult] = G.GAME.selected_back.trigger_effect({
    ...
    new GameEvent({
      ...
      func: function () {
        update_hand_text(
          ...
          { mult: 0, chips: 0, chip_total: math.floor(hand_chips * mult), level: "", handname: "" }
        );
...
G.FUNCS.evaluate_round = function () {
  ...
  if (G.GAME.current_round.hands_left > 0 && !G.GAME.modifiers.no_extra_hand_money) {
    add_round_eval_row({
      dollars: G.GAME.current_round.hands_left * (G.GAME.modifiers.money_per_hand || 1),
    ...
    if ((!G.GAME.seeded && !G.GAME.challenge) || SMODS.config.seeded_unlocks) {
      if (
        ...
        G.PROFILES[G.SETTINGS.profile].career_stats.c_round_interest_cap_streak =
          G.PROFILES[G.SETTINGS.profile].career_stats.c_round_interest_cap_streak + 1;
      } else {
            ...
            nodes: [
              {
                ...
                config: {
                  object: DynaText({
                    string: [
                      localize({ type: "variable", key: "cashout_hidden", vars: [total_hidden] }),
                    ],
                    colours: [G.C.WHITE],
                    shadow: true,
                    float: false,
...
G.FUNCS.tutorial_controller = function () {
  ...
  G.SETTINGS.tutorial_progress = G.SETTINGS.tutorial_progress || {
  ...
  if (!G.SETTINGS.paused && !G.SETTINGS.tutorial_complete) {
    if (
      ...
      G.blind_select &&
      !G.SETTINGS.tutorial_progress.completed_parts["small_blind"]
    ) {
    ...
    if (
      ...
      !G.SETTINGS.tutorial_progress.completed_parts["second_hand"] &&
      G.SETTINGS.tutorial_progress.hold_parts["big_blind"]
    ) {
    ...
    if (!G.SETTINGS.tutorial_progress.completed_parts["first_hand_section"]) {
      if (
        G.STATE === G.STATES.SELECTING_HAND &&
        !G.SETTINGS.tutorial_progress.completed_parts["first_hand"]
      ) {
      ...
      if (
        G.STATE === G.STATES.SELECTING_HAND &&
        !G.SETTINGS.tutorial_progress.completed_parts["first_hand_2"] &&
        G.SETTINGS.tutorial_progress.hold_parts["first_hand"]
      ) {
      ...
      if (
        G.STATE === G.STATES.SELECTING_HAND &&
        !G.SETTINGS.tutorial_progress.completed_parts["first_hand_3"] &&
        G.SETTINGS.tutorial_progress.hold_parts["first_hand_2"]
      ) {
      ...
      if (
        G.STATE === G.STATES.SELECTING_HAND &&
        !G.SETTINGS.tutorial_progress.completed_parts["first_hand_4"] &&
        G.SETTINGS.tutorial_progress.hold_parts["first_hand_3"]
      ) {
...
G.FUNCS.tutorial_part = function (_part) {
  ...
  if (_part === "small_blind") {
    ...
    step = tutorial_info({
      ...
      attach: {
        major: G.blind_select.UIRoot.children[1].children[1],
        type: "tr",
        offset: { x: 2, y: 4 },
      },
      step: step,
    ...
    step = tutorial_info({
      ...
      snap_to: function () {
        if (
          G.blind_select &&
          G.blind_select.UIRoot &&
          G.blind_select.UIRoot.children[1] &&
          G.blind_select.UIRoot.children[1].children[1] &&
          G.blind_select.UIRoot.children[1].children[1].config.object
        ) {
          return G.blind_select.UIRoot.children[1].children[1].config.object.get_UIE_by_ID(
            "select_blind_button"
          );
        }
      },
    ...
    step = tutorial_info({
      ...
      highlight: [G.blind_select, G.HUD.get_UIE_by_ID("hud_ante")],
      attach: { major: G.HUD, type: "cm", offset: { x: 0, y: -2 } },
      step: step,
      no_button: true,
      snap_to: function () {
        if (
          ...
          G.blind_select.UIRoot &&
          G.blind_select.UIRoot.children[1] &&
          G.blind_select.UIRoot.children[1].children[2] &&
          G.blind_select.UIRoot.children[1].children[2].config.object
        ) {
          return G.blind_select.UIRoot.children[1].children[2].config.object.get_UIE_by_ID(
            "select_blind_button"
          );
        }
      },
    ...
    step = tutorial_info({
      hard_set: true,
      text_key: "fh_4",
      highlight: [G.hand, G.HUD.get_UIE_by_ID("run_info_button")],
    ...
    step = tutorial_info({
      hard_set: true,
      text_key: "s_1",
      highlight: [G.SHOP_SIGN, G.HUD.get_UIE_by_ID("dollar_text_UI").parent.parent.parent],
    ...
    step = tutorial_info({
      text_key: "s_3",
      loc_vars: [G.P_CENTER_POOLS.Joker.length],
      highlight: function () {
        return [
          G.SHOP_SIGN,
          G.HUD.get_UIE_by_ID("dollar_text_UI").parent.parent.parent,
  ...
  G.E_MANAGER.add_event(
    new GameEvent({
      ...
      func: function () {
        if (
          (G.OVERLAY_TUTORIAL.step === step && !G.OVERLAY_TUTORIAL.step_complete) ||
          G.OVERLAY_TUTORIAL.skip_steps
        ) {
