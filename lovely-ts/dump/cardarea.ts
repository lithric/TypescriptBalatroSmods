...
class CardArea extends Moveable {
  ...
  emplace(card, location, stay_flipped) {
    if (card.edition && card.edition.card_limit && this === G.hand) {
      this.config.real_card_limit =
        (this.config.real_card_limit || this.config.card_limit) + card.edition.card_limit;
    ...
    if (
      card.facing === "back" &&
      this.config.type !== "discard" &&
      this.config.type !== "deck" &&
      !stay_flipped
    ) {
      card.flip();
    }
    ...
    if (delta !== 0) {
      G.E_MANAGER.add_event(
        new GameEvent({
          func: function () {
            this.config.real_card_limit =
              (this.config.real_card_limit || this.config.card_limit) + delta;
            this.config.card_limit = math.max(0, this.config.real_card_limit);
            if (
              delta > 0 &&
              this.config.real_card_limit > 1 &&
              this === G.hand &&
              this.cards[1] &&
              (G.STATE === G.STATES.DRAW_TO_HAND || G.STATE === G.STATES.SELECTING_HAND)
            ) {
  ...
  can_highlight(card) {
    if (G.CONTROLLER.HID.controller) {
      ...
      if (
        this.config.type === "hand" ||
        this.config.type === "joker" ||
        this.config.type === "consumeable" ||
        (this.config.type === "shop" && this.config.highlighted_limit > 0)
      ) {
        return true;
      }
    }
  ...
  parse_highlighted() {
    ...
    if (text === "NULL") {
      ...
      if (backwards) {
        update_hand_text(
          { immediate: true, nopulse: undefined, delay: 0 },
  ...
  set_ranks() {
    for (const [k, card] of ipairs(this.cards)) {
      ...
      if (k > 1 && this.config.type === "deck") {
        ...
        this.config.type === "play" ||
        this.config.type === "shop" ||
        this.config.type === "consumeable"
      ) {
        card.states.drag.can = false;
      } else {
  ...
  move(dt) {
    if (this === G.hand) {
      let desired_y =
        ...
        1.9 *
          ((!G.deck_preview &&
            (G.STATE === G.STATES.SELECTING_HAND || G.STATE === G.STATES.DRAW_TO_HAND) &&
            1) ||
            0);
  ...
  update(dt) {
    if (this === G.hand) {
      if (G.GAME.modifiers.minus_hand_size_per_X_dollar) {
        ...
        if (
          math.floor(G.GAME.dollars / G.GAME.modifiers.minus_hand_size_per_X_dollar) !==
          this.config.last_poll_size
        ) {
          this.change_size(
            this.config.last_poll_size -
              math.floor(G.GAME.dollars / G.GAME.modifiers.minus_hand_size_per_X_dollar)
          );
          this.config.last_poll_size = math.floor(
            G.GAME.dollars / G.GAME.modifiers.minus_hand_size_per_X_dollar
          );
        }
      }
  ...
  draw() {
    ...
    this.ARGS.invisible_area_types = this.ARGS.invisible_area_types || {
      discard: 1,
      voucher: 1,
      play: 1,
      consumeable: 1,
    ...
    if (
      this.ARGS.invisible_area_types[this.config.type] ||
      (this.config.type === "hand" &&
        {
          [G.STATES.SHOP]: 1,
      ...
      (this.config.type === "hand" && state === G.STATES.SMODS_BOOSTER_OPENED) ||
      (this.config.type === "deck" && this !== G.deck) ||
      (this.config.type === "shop" && this !== G.shop_vouchers)
    ) {
    } else {
      if (!this.children.area_uibox) {
        let card_count =
          (this !== G.shop_vouchers && {
            n: G.UIT.R,
            config: {
              align: (this === G.jokers && "cl") || (this === G.hand && "cm") || "cr",
              padding: 0.03,
              no_fill: true,
            },
            nodes: [
          ...
          undefined;
        this.children.area_uibox = new UIBox({
          definition: {
            n: G.UIT.ROOT,
            ...
            nodes: [
              {
                ...
                config: {
                  ...
                  mid: true,
                  r: 0.1,
                  colour: (this !== G.shop_vouchers && [0, 0, 0, 0.1]) || undefined,
                  ref_table: this,
                },
                nodes: [
                  (this === G.shop_vouchers && {
                    ...
                    nodes: [
                      ...
                      },
                      {
                        ...
                        nodes: [
                          {
                            n: G.UIT.T,
                            config: { text: "BOSS BLIND", scale: 0.4, colour: G.C.WHITE },
                          },
                        ],
                      },
                      {
                        ...
                        nodes: [
                          {
                            n: G.UIT.T,
                            config: { text: "TO RESTOCK", scale: 0.4, colour: G.C.WHITE },
                          },
                        ],
                      },
                    ],
          ...
          config: { align: "cm", offset: { x: 0, y: 0 }, major: this, parent: this },
        });
      }
      ...
      if (this.config.type === "deck") {
        for (let i = this.cards.length; i <= 1; i += -1) {
          if (this.cards[i] !== G.CONTROLLER.focused.target) {
            if (
              i === 1 ||
              i % (this.config.thin_draw || 9) === 0 ||
              i === this.cards.length ||
              math.abs(this.cards[i].VT.x - this.T.x) > 1 ||
              math.abs(this.cards[i].VT.y - this.T.y) > 1
            ) {
    ...
    if (this === G.deck) {
      if (
        G.CONTROLLER.HID.controller &&
        G.STATE === G.STATES.SELECTING_HAND &&
        !this.children.peek_deck
      ) {
        this.children.peek_deck = new UIBox({
          definition: {
            n: G.UIT.ROOT,
                ...
                nodes: [
                  {
                    ...
                    nodes: [
                      {
                        n: G.UIT.T,
                        config: { text: "PEEK", scale: 0.48, colour: G.C.WHITE, shadow: true },
                      },
                    ],
                  },
                  {
                    ...
                    nodes: [
                      {
                        n: G.UIT.T,
                        config: { text: "DECK", scale: 0.38, colour: G.C.WHITE, shadow: true },
                      },
                    ],
                  },
                ],
          ...
          config: { align: "cl", offset: { x: -0.5, y: 0.1 }, major: this, parent: this },
        });
        this.children.peek_deck.states.collide.can = false;
      } else if (
        (!G.CONTROLLER.HID.controller || G.STATE !== G.STATES.SELECTING_HAND) &&
        this.children.peek_deck
      ) {
      ...
      if (!this.children.view_deck) {
        this.children.view_deck = new UIBox({
          definition: {
            n: G.UIT.ROOT,
            config: { align: "cm", padding: 0.1, r: 0.1, colour: G.C.CLEAR },
            nodes: [
      ...
      if (
        G.deck_preview ||
        this.states.collide.is ||
        (G.buttons && G.buttons.states.collide.is && G.CONTROLLER.HID.controller)
      ) {
  ...
  align_cards() {
    if (
      (this === G.hand || this === G.deck || this === G.discard || this === G.play) &&
      G.view_deck &&
      G.view_deck[1] &&
      G.view_deck[1].cards
    ) {
    ...
    if (this.config.type === "deck") {
      ...
      for (const [k, card] of ipairs(this.cards)) {
        ...
        if (!card.states.drag.is) {
          card.T.x =
            this.T.x +
            0.5 * (this.T.w - card.T.w) +
            this.shadow_parrallax.x *
              deck_height *
              (this.cards.length / ((this === G.deck && 1) || 2) - k) +
            0.9 * this.shuffle_amt * (1 - k * 0.01) * ((k % 2 === 1 && 1) || -0);
          card.T.y =
            this.T.y +
            0.5 * (this.T.h - card.T.h) +
            this.shadow_parrallax.y *
              deck_height *
              (this.cards.length / ((this === G.deck && 1) || 2) - k);
    ...
    if (
      ...
      for (const [k, card] of ipairs(this.cards)) {
        if (!card.states.drag.is) {
          card.T.r =
            (0.4 * (-this.cards.length / 2 - 0.5 + k)) / this.cards.length +
            ((G.SETTINGS.reduced_motion && 0) || 1) * 0.02 * math.sin(2 * G.TIMERS.REAL + card.T.x);
          let max_cards = math.max(this.cards.length, this.config.temp_limit);
          card.T.x =
            this.T.x +
            (this.T.w - this.card_w) *
              ((k - 1) / math.max(max_cards - 1, 1) -
                (0.5 * (this.cards.length - max_cards)) / math.max(max_cards - 1, 1)) +
            0.5 * (this.card_w - card.T.w);
          ...
          card.T.y =
            ...
            ((G.SETTINGS.reduced_motion && 0) || 1) *
              0.1 *
              math.sin(0.666 * G.TIMERS.REAL + card.T.x) +
            (math.abs((1.3 * (-this.cards.length / 2 + k - 0.5)) / this.cards.length) ^ 2) -
            0.3;
          card.T.x = card.T.x + card.shadow_parrallax.x / 30;
        }
      }
    ...
    if (
      ...
      for (const [k, card] of ipairs(this.cards)) {
        if (!card.states.drag.is) {
          card.T.r =
            (0.2 * (-this.cards.length / 2 - 0.5 + k)) / this.cards.length +
            ((G.SETTINGS.reduced_motion && 0) || 1) * 0.02 * math.sin(2 * G.TIMERS.REAL + card.T.x);
          let max_cards = math.max(this.cards.length, this.config.temp_limit);
          card.T.x =
            this.T.x +
          ...
          card.T.y =
            ...
            ((G.SETTINGS.reduced_motion && 0) || 1) *
              0.03 *
              math.sin(0.666 * G.TIMERS.REAL + card.T.x) +
            math.abs((0.5 * (-this.cards.length / 2 + k - 0.5)) / this.cards.length) -
            0.2;
          card.T.x = card.T.x + card.shadow_parrallax.x / 30;
        }
      }
    ...
    if (
      this.config.type === "title" ||
      (this.config.type === "voucher" && this.cards.length === 1)
    ) {
      for (const [k, card] of ipairs(this.cards)) {
        if (!card.states.drag.is) {
          card.T.r =
      ...
      for (const [k, card] of ipairs(this.cards)) {
        if (!card.states.drag.is) {
          card.T.r =
            ...
            ((G.SETTINGS.reduced_motion && 0) || 1) *
              0.02 *
              math.sin(2 * G.TIMERS.REAL + card.T.x + card.T.y) +
            ((k % 2 === 0 && 1) || -1) * 0.08;
          let max_cards = math.max(this.cards.length, this.config.temp_limit);
          card.T.x =
            this.T.x +
          ...
          card.T.x =
            ...
            (this.T.w - this.card_w) *
              ((k - 1) / math.max(max_cards - 1, 1) -
                (0.5 * (this.cards.length - max_cards)) / math.max(max_cards - 1, 1)) +
            0.5 * (this.card_w - card.T.w) +
            ((this.config.card_limit === 1 && 0.5 * (this.T.w - card.T.w)) || 0);
          let highlight_height = G.HIGHLIGHT_H;
          if (!card.highlighted) {
            highlight_height = 0;
          }
    ...
    if (this.config.type === "joker" || this.config.type === "title_2") {
      for (const [k, card] of ipairs(this.cards)) {
        if (!card.states.drag.is) {
          card.T.r =
            (0.1 * (-this.cards.length / 2 - 0.5 + k)) / this.cards.length +
            ((G.SETTINGS.reduced_motion && 0) || 1) * 0.02 * math.sin(2 * G.TIMERS.REAL + card.T.x);
          let max_cards = math.max(this.cards.length, this.config.temp_limit);
          card.T.x =
            this.T.x +
          ...
          if (
            this.cards.length > 2 ||
            (this.cards.length > 1 && this === G.consumeables) ||
            (this.cards.length > 1 && this.config.spread)
          ) {
            card.T.x =
              this.T.x +
              (this.T.w - this.card_w) * ((k - 1) / (this.cards.length - 1)) +
              0.5 * (this.card_w - card.T.w);
          } else if (this.cards.length > 1 && this !== G.consumeables) {
            card.T.x =
              this.T.x +
              (this.T.w - this.card_w) * ((k - 0.5) / this.cards.length) +
              0.5 * (this.card_w - card.T.w);
          } else {
      ...
      table.sort(this.cards, function (a, b) {
        return (
          a.T.x + a.T.w / 2 - 100 * ((a.pinned && !a.ignore_pinned && a.sort_id) || 0) <
          b.T.x + b.T.w / 2 - 100 * ((b.pinned && !b.ignore_pinned && b.sort_id) || 0)
        );
      });
    }
        ...
        if (!card.states.drag.is) {
          ...
          card.T.y =
            ...
            ((!card.highlighted &&
              ((G.SETTINGS.reduced_motion && 0) || 1) *
                0.05 *
                math.sin(2 * 1.666 * G.TIMERS.REAL + card.T.x)) ||
              0);
          card.T.x = card.T.x + card.shadow_parrallax.x / 30;
        }
      }
  ...
  sort(method) {
    ...
    if (this.config.sort === "desc") {
      ...
      table.sort(this.cards, function (a, b) {
        return (
          (a.config.card.order || a.config.center.order) <
          (b.config.card.order || b.config.center.order)
        );
      });
    }
  }
