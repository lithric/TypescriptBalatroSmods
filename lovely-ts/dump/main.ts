...
class Dumper extends SovereignContext {
  ...
  GuessFunctionName(info): string | LuaMultiReturn<string[]> {
    if (this._type(info.source) === "string" && this.string_sub(info.source, 1, 1) === "@") {
      ...
      this._type(info.source) === "string" &&
      this.string_sub(info.source, 1, 6) === "=[love"
    ) {
  ...
  DumpLocals(level: number) {
    ...
    while (name) {
      if (this._type(value) === "number") {
        ...
        if (!info) return;
        let fun_name =
          info.name || this.m_known_functions[value] || this.m_user_known_functions[value];
        if (info.what === "C") {
          this.add_f(
            "%s%s = C %s\\r\\n",
            prefix,
            name,
            (fun_name && "function: " + fun_name) || this._tostring(value)
          );
        } else {
...
class M extends SovereignContext {
  ...
  constructor(SOV: SovereignContext) {
    super(SOV);
  }
  ParseLine(line: string): string | LuaMultiReturn<string[]> {
    this._assert(this._type(line) === "string");
  ...
  stacktrace(
    thread_or_message?: LuaThread | string,
    message_or_level?: string | number,
    level?: number
  ) {
    let thread: LuaThread | undefined = undefined;
    ...
    if (this._type(thread) !== "thread") {
      [thread, message, level] = [
        undefined,
        thread_or_message as string,
        message_or_level as number,
      ];
    } else {
      [thread, message, level] = [
        thread_or_message as LuaThread,
        thread_or_message as string,
        level as number,
      ];
    }
    ...
    while (info) {
      if (info.what === "main") {
        ...
        if (this.string_sub(info.source, 1, 1) === "@") {
          ...
          if (source === "love") {
            dumper.add_f(
              ...
              level_to_show,
              this.string_sub(this.table_concat(props, " "), 2, -2),
              info.currentline
            );
        ...
        let function_name =
          this.m_user_known_functions[info.func] ||
          this.m_known_functions[info.func] ||
          info.name ||
          this._tostring(info.func);
        ...
        let source = info.short_src;
        let function_name =
          this.m_user_known_functions[info.func] || this.m_known_functions[info.func] || info.name;
        if (!source) {
          return;
        }
        if (this.string_sub(source, 2, 7) === "string") {
...
function loadStackTracePlus(): M {
  ...
  for (const [_, name] of SOV._ipairs([
    "assert",
    "collectgarbage",
    "dofile",
    "error",
...
function getDebugInfoForCrash(): void {
  ...
  if (SMODS && SMODS.Mods) {
    ...
    for (const [_, v] of pairs(SMODS.Mods)) {
      if (
        (v.can_load && (!v.meta_mod || v.lovely_only)) ||
        (v.lovely && !v.can_load && !v.disabled)
      ) {
                  ...
                  (v.name +
                    (" by " +
                      (table.concat(v.author, ", ") +
                        (" [ID: " +
                          (v.id +
                            (((v.priority !== 0 && ", Priority: " + v.priority) || "") +
                              (((v.version && v.version !== "0.0.0" && ", Version: " + v.version) ||
                                "") +
                                (((v.lovely && ", Uses Lovely") || "") + "]")))))))))));
          i = i + 1;
...
love.mousemoved = function (x, y, dx, dy, istouch) {
  ...
  G.CONTROLLER.set_HID_flags(
    (G.CONTROLLER.last_touch_time > G.TIMERS.UPTIME - 0.2 && "touch") || "mouse"
  );
};
  ...
  if (G.SETTINGS.crashreports && _RELEASE_MODE && G.F_CRASH_REPORTS) {
    ...
    for (const [l] of string.gmatch(trace, "(.-)\\n")) {
      if (string.match(l, "boot.lua")) {
        ...
        trace = "";
        function_line =
          string.sub(l, string.find(l, "in function") + 12) + (" line:" + function_line);
      }
    ...
    http_channel.push(
      "https://958ha8ong3.execute-api.us-east-2.amazonaws.com/?error=" +
        (httpencode(error) +
          ("&file=" +
            (httpencode(file) +
              ("&function_line=" +
                (httpencode(function_line) +
                  ("&trace=" + (httpencode(trace) + ("&version=" + G.VERSION))))))))
    );
  }
  if (!love.window || !love.graphics || !love.event) {
    return;
...
love.resize = function (w, h) {
  ...
  if (G.ROOM) {
    ...
    if (w / h < G.window_prev.orig_ratio) {
      ...
      G.ROOM.T.y =
        (h / (G.TILESIZE * G.TILESCALE) - (G.ROOM.T.h + G.ROOM_PADDING_H)) / 2 +
        G.ROOM_PADDING_H / 2;
    } else {
      G.ROOM.T.y = G.ROOM_PADDING_H;
      G.ROOM.T.x =
        (w / (G.TILESIZE * G.TILESCALE) - (G.ROOM.T.w + G.ROOM_PADDING_W)) / 2 +
        G.ROOM_PADDING_W / 2;
    }
  ...
  if (love.system.getOS() === "Windows" && false) {
    ...
    let DPI_scale =
      math.floor(
        ((0.5 * unscaled_dims.width) / render_w + (0.5 * unscaled_dims.height) / render_h) * 500 +
          0.5
      ) / 500;
    if (DPI_scale > 1.1) {
      G.CANV_SCALE = 1.5;
      G.AA_CANVAS = love.graphics.newCanvas(
  ...
  G.CANVAS = love.graphics.newCanvas(w * G.CANV_SCALE, h * G.CANV_SCALE, {
    type: "2d",
    readable: true,
  });
...
for (const [_, path] of ipairs([
  ...
  (
    assert(
      load(string.format(NFS.read(SMODS.path + path), '=[SMODS _ "%s"]', path))
