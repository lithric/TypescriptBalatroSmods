...
type HexColorSelection = {
  [P in HexColorName]: P extends SimpleHexColorName
    ? HexArray
    : P extends ComplexHexColorName
      ? P extends "DYN_UI"
        ...
        : P extends "SO_1" | "SO_2" | "SUITS"
          ? { Hearts: HexArray; Diamonds: HexArray; Spades: HexArray; Clubs: HexArray }
          : P extends "UI"
            ? {
                RED: HexArray;
                TEXT_LIGHT: [1, 1, 1, 1];
                TEXT_DARK: HexArray;
                TEXT_INACTIVE: HexArray;
                BACKGROUND_LIGHT: HexArray;
                BACKGROUND_WHITE: [1, 1, 1, 1];
                BACKGROUND_DARK: HexArray;
                BACKGROUND_INACTIVE: HexArray;
                OUTLINE_LIGHT: HexArray;
                OUTLINE_LIGHT_TRANS: HexArray;
                OUTLINE_DARK: HexArray;
                TRANSPARENT_LIGHT: HexArray;
                TRANSPARENT_DARK: HexArray;
                HOVER: HexArray;
              }
            : P extends "SET" | "SECONDARY_SET"
              ? {
                  Default: HexArray;
                  Enhanced: HexArray;
                  Joker: HexArray;
              ...
              : P extends "BLIND"
                ? { Small: HexArray; Big: HexArray; Boss: HexArray; won: HexArray }
                : P extends "BACKGROUND"
                  ? { L: HexArray; D: HexArray; C: HexArray; contrast: number }
                  : P extends "RARITY" | "HAND_LEVELS"
                    ? HexArray[]
                    : never
      : HexArray;
};
...
type EditionCardID = "e_base" | "e_foil" | "e_holo" | "e_polychrome" | "e_negative";
...
type CardUtilityID = "soul" | "undiscovered_joker" | "undiscovered_tarot";

type CardSealID = "Red" | "Blue" | "Gold" | "Purple";
...
type ConsumableCardSet = "Tarot" | "Planet" | "Spectral";

type CardItemSet = "Default" | "Joker" | ConsumableCardSet | "Voucher";

type CardSpecialSet = "Enhanced" | "Edition";

type GameDeckSet = "Back";

type BoosterPackSet = "Booster";

type CenterSet = CardItemSet | CardSpecialSet | GameDeckSet | BoosterPackSet;

type GameSet = CenterSet | "Seal" | "Tag" | "Stake";
...
interface ChallengeParams {
  ...
  id: string;
  rules: {
    custom: { id: string; value?: number | string }[];
    modifiers: { id?: string; value?: number }[];
  };
  ...
  restrictions: {
    banned_cards: { id: string; ids?: string[] }[];
    banned_tags: { id: string }[];
...
type CenterID = CardItemID | CardSpecialID | GameDeckID | BoosterPackID | CardUtilityID;
...
type P_CENTERS = {
  [P1 in
    ...
    | GameDeckID
    | BoosterPackID
    | CardUtilityID]: P1 extends BaseCardID
    ? BaseCardParams
    : P1 extends JokerCardID
      ? JokerCardParams
      : P1 extends TarotCardID
        ? TarotCardParams
        : P1 extends PlanetCardID
          ? PlanetCardParams
          : P1 extends SpectralCardID
            ? SpectralCardParams
            : P1 extends VoucherCardID
              ? VoucherCardParams
              : P1 extends EnhancedCardID
                ? EnhancedCardParams
                : P1 extends EditionCardID
                  ? EditionCardParams
                  : P1 extends GameDeckID
                    ? GameDeckParams
                    : P1 extends BoosterPackID
                      ? BoosterPackParams
                      : P1 extends CardUtilityID
                        ? CardUtilityParams
                        : never;
};
