...
class Back extends LuaObject {
    ...
    generate_UI(other, ui_scale, min_dims, challenge) {
        ...
        if (!back_config.unlocked) {
            ...
            if (G.localization.descriptions.Back[key_override || back_config.key].unlock_parsed) {
                localize({ type: "unlocks", key: key_override || back_config.key, set: "Back", nodes: loc_nodes, vars: loc_args });
            ...
            if (!back_config.unlock_condition) {
                ...
                let other_name = localize({ type: "name_text", set: "Stake", key: G.P_CENTER_POOLS.Stake[back_config.unlock_condition.stake].key });
                loc_args = loc_args || { [1]: other_name, colours: [get_stake_col(back_config.unlock_condition.stake)] };
            ...
            if (back_config.loc_vars && type(back_config.loc_vars) === "function") {
                ...
                loc_args = [localize({ type: "name_text", key: "v_tarot_merchant", set: "Voucher" }), localize({ type: "name_text", key: "v_planet_merchant", set: "Voucher" }), localize({ type: "name_text", key: "v_overstock_norm", set: "Voucher" })];
            }
            ...
            localize({ type: "descriptions", key: key_override || back_config.key, set: "Back", nodes: loc_nodes, vars: loc_args });
        }
        return { n: G.UIT.ROOT, config: { align: "cm", minw: min_dims * 5, minh: min_dims * 2.5, id: this.name, colour: G.C.CLEAR }, nodes: [name_to_check === "Challenge Deck" && UIBox_button({ button: "deck_view_challenge", label: [localize(challenge.id, "challenge_names")], minw: 2.2, minh: 1, scale: 0.6, id: challenge }) || desc_from_rows(loc_nodes, true, min_dims * 5)] };
    };
    change_to(new_back) {
        if (!new_back) {
    ...
    save() {
        let backTable = { name: this.name, pos: this.pos, effect: this.effect, key: this.effect.center.key || "b_red" };
        return backTable;
    };
    trigger_effect(args) {
        if (!args) {
    ...
    apply_to_run() {
        ...
        if (this.effect.config.discards) {
            G.GAME.starting_params.discards = G.GAME.starting_params.discards + this.effect.config.discards;
        }
        ...
        if (this.effect.config.joker_slot) {
            G.GAME.starting_params.joker_slots = G.GAME.starting_params.joker_slots + this.effect.config.joker_slot;
        }
        if (this.effect.config.hand_size) {
            G.GAME.starting_params.hand_size = G.GAME.starting_params.hand_size + this.effect.config.hand_size;
        }
        ...
        if (this.effect.config.consumable_slot) {
            G.GAME.starting_params.consumable_slots = G.GAME.starting_params.consumable_slots + this.effect.config.consumable_slot;
        }
