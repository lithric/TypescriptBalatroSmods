...
Card = __TS__Class()
Card.name = "Card"
__TS__ClassExtends(Card, Moveable)
function Card.prototype.____constructor(self, X, Y, W, H, card, center, params)
    ...
    self:set_cost()
    if __TS__InstanceOf(self, Card) then
        table.insert(G.I.CARD, self)
    end
end
function Card.prototype.update_alert(self)
    if self.ability.set == "Default" and self.config.center and self.config.center.key == "c_base" and self.seal then
        ...
        elseif not G.P_SEALS[self.seal].alerted and not self.children.alert and G.P_SEALS[self.seal].discovered then
            self.children.alert = __TS__New(
                UIBox,
                {
                    definition = create_UIBox_card_alert(_G),
                    config = {align = "tli", offset = {x = 0.1, y = 0.1}, parent = self}
                }
            )
        end
    end
        ...
        if self.area and self.area.config.collection and self.config.center then
            ...
            elseif not self.config.center.alerted and not self.children.alert and self.config.center.discovered then
                self.children.alert = __TS__New(
                    ...
                    {
                        definition = create_UIBox_card_alert(_G),
                        config = {align = self.ability.set == "Voucher" and self.config.center.order % 2 == 1 and "tli" or "tri", offset = {x = self.ability.set == "Voucher" and self.config.center.order % 2 == 1 and 0.1 or -0.1, y = 0.1}, parent = self}
                    }
                )
            end
        end
...
function Card.prototype.set_base(self, card, initial)
    card = card or ({})
    ...
    if self.playing_card and not initial then
        check_for_unlock(_G, {type = "modify_deck"})
    end
end
function Card.prototype.set_sprites(self, _center, _front)
    if _front then
        local _atlas, _pos = __TS__Unpack(get_front_spriteinfo(_G, _front))
        if self.children.front then
    ...
    if _center then
            ...
            if self.children.center then
                ...
                elseif _center.set == "Joker" or _center.consumeable or _center.set == "Voucher" then
                    self.children.center = Sprite(
                        ...
                        self.T.w,
                        self.T.h,
                        G.ASSET_ATLAS[_center[G.SETTINGS.colourblind_option and "hc_atlas" or "lc_atlas"] or _center.atlas or _center.set],
                    ...
                    self.children.center = Sprite(
                        ...
                        self.T.w,
                        self.T.h,
                        G.ASSET_ATLAS[_center.atlas or "centers"],
                        _center.pos
                    )
                end
        ...
        if true then
            self.children.back = Sprite(
                ...
                self.T.w,
                self.T.h,
                G.ASSET_ATLAS[(G.GAME.viewed_back or G.GAME.selected_back) and ((G.GAME.viewed_back or G.GAME.selected_back)[G.SETTINGS.colourblind_option and "hc_atlas" or "lc_atlas"] or (G.GAME.viewed_back or G.GAME.selected_back).atlas) or "centers"],
...
function Card.prototype.set_ability(self, center, initial, delay_sprites)
    ...
    if self.params.discover and not center.discovered then
        unlock_card(_G, center)
        discover_card(_G, center)
    end
    ...
    local new_ability = {
        ...
        extra = copy_table(_G, center.config.extra) or nil,
        extra_value = 0,
    ...
    if not self.ability.name then
        self.ability.name = center.key
    end
    for k, v in pairs(center.config) do
        if k ~= "bonus" then
            if type(v) == "table" then
                self.ability[k] = copy_table(_G, v)
            else
                self.ability[k] = v
            end
        end
    ...
    if self.ability.name == "Gold Card" and self.seal == "Gold" and self.playing_card then
        check_for_unlock(_G, {type = "double_gold"})
    end
    ...
    if self.ability.name == "To Do List" then
        ...
        while not self.ability.to_do_poker_hand do
            self.ability.to_do_poker_hand = pseudorandom_element(
                _G,
                _poker_hands,
                pseudoseed(_G, self.area and self.area.config.type == "title" and "false_to_do" or "to_do")
            )
            if self.ability.to_do_poker_hand == old_hand then
                self.ability.to_do_poker_hand = nil
            end
        end
    end
    ...
    if self.ability.set == "Edition" then
        self.label = self.ability.name
    end
...
function Card.prototype.set_cost(self)
    ...
    self.cost = math.max(
        1,
        math.floor((self.base_cost + self.extra_cost + 0.5) * (100 - G.GAME.discount_percent) / 100)
    )
    ...
    if (self.ability.set == "Planet" or self.ability.set == "Booster" and self.ability.name:find("Celestial")) and find_joker(_G, "Astronomer").length > 0 then
        self.cost = 0
    end
...
function Card.prototype.set_edition(self, edition, immediate, silent)
    self.edition = nil
    if not edition then
        return
    end
    ...
    if self.area and self.area == G.jokers then
        if self.edition then
            if not G.P_CENTERS["e_" .. tostring(self.edition.type)].discovered then
                discover_card(
                    _G,
                    G.P_CENTERS["e_" .. tostring(self.edition.type)]
                )
            end
        else
            if not G.P_CENTERS.e_base.discovered then
                discover_card(_G, G.P_CENTERS.e_base)
            end
        end
    end
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    ...
                    if self.edition.foil then
                        play_sound(_G, "foil1", 1.2, 0.4)
                    end
                    if self.edition.holo then
                        play_sound(_G, "holo1", 1.2 * 1.58, 0.4)
                    end
                    if self.edition.polychrome then
                        play_sound(_G, "polychrome1", 1.2, 0.7)
                    end
                    if self.edition.negative then
                        play_sound(_G, "negative", 1.5, 0.4)
                    end
                    return true
                end
            }
...
function Card.prototype.set_seal(self, _seal, silent, immediate)
    self.seal = nil
    if _seal then
        self.seal = _seal
        self.ability.seal = {}
        for k, v in pairs(G.P_SEALS[_seal].config or ({})) do
            if type(v) == "table" then
                self.ability.seal[k] = copy_table(_G, v)
            else
        ...
        if not silent then
            ...
            if immediate then
                ...
                play_sound(_G, sound.sound, sound.per, sound.vol)
...
function Card.prototype.get_seal(self, bypass_debuff)
    if self.debuff and not bypass_debuff then
        return
    end
    return self.seal
end
function Card.prototype.set_eternal(self, _eternal)
...
function Card.prototype.set_perishable(self, _perishable)
...
function Card.prototype.set_rental(self, _rental)
    self.ability.rental = _rental
    self:set_cost()
end
function Card.prototype.set_debuff(self, should_debuff)
    for _, mod in ipairs(SMODS.mod_list) do
...
function Card.prototype.remove_UI(self)
    self.ability_UIBox_table = nil
    self.config.h_popup = nil
    self.config.h_popup_config = nil
    self.no_ui = true
end
function Card.prototype.change_suit(self, new_suit)
    local new_code = SMODS.Suits[new_suit].card_key
    local new_val = SMODS.Ranks[self.base.value].card_key
    local new_card = G.P_CARDS[tostring(new_code) .. "_" .. tostring(new_val)]
    self:set_base(new_card)
    G.GAME.blind:debuff_card(self)
end
function Card.prototype.add_to_deck(self, from_debuff)
    if not self.config.center.discovered then
        discover_card(_G, self.config.center)
    end
    if not self.added_to_deck then
        ...
        if self.ability.d_size > 0 then
            ...
            ease_discard(_G, self.ability.d_size)
        end
        ...
        if self.ability.name == "Chicot" and G.GAME.blind and G.GAME.blind.boss and not G.GAME.blind.disabled then
            ...
            card_eval_status_text(
                ...
                nil,
                nil,
                {message = localize(_G, "ph_boss_disabled")}
            )
        end
...
function Card.prototype.remove_from_deck(self, from_debuff)
    if self.added_to_deck then
        self.added_to_deck = false
        local obj = self.config.center
...
function Card.prototype.generate_UIBox_unlock_table(self, hidden)
...
function Card.prototype.generate_UIBox_ability_table(self, vars_only)
    local card_type, hide_desc = self.ability.set or "None", nil
    local loc_vars = nil
    local main_start, main_end = nil, nil
    local no_badge = nil
    if not self.bypass_lock and self.config.center.unlocked ~= false and (self.ability.set == "Joker" or self.ability.set == "Edition" or self.ability.consumeable or self.ability.set == "Voucher" or self.ability.set == "Booster") and not self.config.center.discovered and (self.area ~= G.jokers and self.area ~= G.consumeables and self.area or not self.area) then
    ...
    elseif self.ability.set == "Joker" then
        ...
        elseif self.ability.name == "Jolly Joker" or self.ability.name == "Zany Joker" or self.ability.name == "Mad Joker" or self.ability.name == "Crazy Joker" or self.ability.name == "Droll Joker" then
            loc_vars = {
                self.ability.t_mult,
                localize(_G, self.ability.type, "poker_hands")
            }
        ...
        elseif self.ability.name == "Half Joker" then
            loc_vars = {self.ability.extra.mult, self.ability.extra.size}
        elseif self.ability.name == "Fortune Teller" then
            loc_vars = {self.ability.extra, G.GAME.consumeable_usage_total and G.GAME.consumeable_usage_total.tarot or 0}
        elseif self.ability.name == "Steel Joker" then
            loc_vars = {self.ability.extra, 1 + self.ability.extra * (self.ability.steel_tally or 0)}
        ...
        elseif self.ability.name == "Stone Joker" then
            loc_vars = {self.ability.extra, self.ability.extra * (self.ability.stone_tally or 0)}
        elseif self.ability.name == "Drunkard" then
        ...
        elseif self.ability.name == "Credit Card" then
            loc_vars = {self.ability.extra}
        elseif self.ability.name == "Greedy Joker" or self.ability.name == "Lusty Joker" or self.ability.name == "Wrathful Joker" or self.ability.name == "Gluttonous Joker" then
            loc_vars = {
                self.ability.extra.s_mult,
                localize(_G, self.ability.extra.suit, "suits_singular")
            }
        elseif self.ability.name == "Blue Joker" then
            loc_vars = {self.ability.extra, self.ability.extra * (G.deck and G.deck.cards and G.deck.cards.length or 52)}
        ...
        elseif self.ability.name == "Faceless Joker" then
            loc_vars = {self.ability.extra.dollars, self.ability.extra.faces}
        elseif self.ability.name == "Oops! All 6s" then
        elseif self.ability.name == "Juggler" then
        ...
        elseif self.ability.name == "Misprint" then
            local r_mults = {}
            do
                local i = self.ability.extra.min
                while i <= self.ability.extra.max do
                    r_mults[r_mults.length + 1] = tostring(i)
                    i = i + 1
                end
            end
            local loc_mult = " " .. tostring(localize(_G, "k_mult")) .. " "
            main_start = {
                {n = G.UIT.T, config = {text = "  +", colour = G.C.MULT, scale = 0.32}},
                {
                    n = G.UIT.O,
                    config = {object = __TS__New(DynaText, {
                        string = r_mults,
                        colours = {G.C.RED},
                        pop_in_rate = 9999999,
                        silent = true,
                    ...
                    config = {object = __TS__New(
                        DynaText,
                        {
                            string = {
                                ...
                                {
                                    string = "#@" .. tostring((G.deck and G.deck.cards[1] and G.deck.cards[G.deck.cards.length].base.id or 11) + (G.deck and G.deck.cards[1] and G.deck.cards[G.deck.cards.length].base.suit:sub(1, 1) or "D")),
                                    colour = G.C.RED
                                },
                                loc_mult,
                                loc_mult,
        ...
        elseif self.ability.name == "Mystic Summit" then
            loc_vars = {self.ability.extra.mult, self.ability.extra.d_remaining}
        ...
        elseif self.ability.name == "Loyalty Card" then
            loc_vars = {
                ...
                localize(_G, {type = "variable", key = self.ability.loyalty_remaining == 0 and "loyalty_active" or "loyalty_inactive", vars = {self.ability.loyalty_remaining}})
            }
        ...
        elseif self.ability.name == "Abstract Joker" then
            loc_vars = {self.ability.extra, (G.jokers and G.jokers.cards and G.jokers.cards.length or 0) * self.ability.extra}
        ...
        elseif self.ability.name == "Scholar" then
            loc_vars = {self.ability.extra.mult, self.ability.extra.chips}
        ...
        elseif self.ability.name == "To Do List" then
            loc_vars = {
                ...
                localize(_G, self.ability.to_do_poker_hand, "poker_hands")
            }
        ...
        elseif self.ability.name == "Blueprint" then
            ...
            main_end = self.area and self.area == G.jokers and ({{n = G.UIT.C, config = {align = "bm", minh = 0.4}, nodes = {{n = G.UIT.C, config = {
                ref_table = self,
                align = "m",
            ...
            }, nodes = {{n = G.UIT.T, config = {ref_table = self.ability, ref_value = "blueprint_compat_ui", colour = G.C.UI.TEXT_LIGHT, scale = 0.32 * 0.8}}}}}}}) or nil
        ...
        elseif self.ability.name == "Troubadour" then
            loc_vars = {self.ability.extra.h_size, -self.ability.extra.h_plays}
        ...
        elseif self.ability.name == "Merry Andy" then
            loc_vars = {self.ability.d_size, self.ability.h_size}
        elseif self.ability.name == "The Idol" then
            loc_vars = {
                [1] = self.ability.extra,
                [2] = localize(_G, G.GAME.current_round.idol_card.rank, "ranks"),
                [3] = localize(_G, G.GAME.current_round.idol_card.suit, "suits_plural"),
        ...
        elseif self.ability.name == "Seance" then
            loc_vars = {localize(_G, self.ability.extra.poker_hand, "poker_hands")}
        ...
        elseif self.ability.name == "Cloud 9" then
            loc_vars = {self.ability.extra, self.ability.extra * (self.ability.nine_tally or 0)}
        elseif self.ability.name == "Rocket" then
            loc_vars = {self.ability.extra.dollars, self.ability.extra.increase}
                ...
                main_end = {{
                    ...
                    nodes = {{
                        ...
                        nodes = {{
                            n = G.UIT.T,
                            config = {
                                text = " " .. tostring(localize(_G, disableable and "k_active" or "ph_no_boss_active")) .. " ",
        ...
        elseif self.ability.name == "Erosion" then
            loc_vars = {
                self.ability.extra,
                math.max(0, self.ability.extra * (G.playing_cards and G.GAME.starting_deck_size - G.playing_cards.length or 0)),
        ...
        elseif self.ability.name == "Mail-In Rebate" then
            loc_vars = {
                self.ability.extra,
                localize(_G, G.GAME.current_round.mail_card.rank, "ranks")
            }
        ...
        elseif self.ability.name == "Bull" then
            loc_vars = {
                self.ability.extra,
                self.ability.extra * math.max(0, G.GAME.dollars) or 0
            }
        elseif self.ability.name == "Diet Cola" then
            loc_vars = {localize(_G, {type = "name_text", set = "Tag", key = "tag_double", nodes = {}})}
        elseif self.ability.name == "Trading Card" then
            loc_vars = {self.ability.extra}
        elseif self.ability.name == "Flash Card" then
            loc_vars = {self.ability.extra, self.ability.mult}
        elseif self.ability.name == "Popcorn" then
            loc_vars = {self.ability.mult, self.ability.extra}
        ...
        elseif self.ability.name == "Ancient Joker" then
            loc_vars = {
                [1] = self.ability.extra,
                [2] = localize(_G, G.GAME.current_round.ancient_card.suit, "suits_singular"),
        ...
        elseif self.ability.name == "Castle" then
            loc_vars = {
                [1] = self.ability.extra.chip_mod,
                [2] = localize(_G, G.GAME.current_round.castle_card.suit, "suits_singular"),
                [3] = self.ability.extra.chips,
        ...
        elseif self.ability.name == "Stuntman" then
            loc_vars = {self.ability.extra.chip_mod, self.ability.extra.h_size}
        elseif self.ability.name == "Invisible Joker" then
            loc_vars = {self.ability.extra, self.ability.invis_rounds}
        ...
        elseif self.ability.name == "Driver's License" then
            loc_vars = {self.ability.extra, self.ability.driver_tally or "0"}
        elseif self.ability.name == "Burnt Joker" then
        elseif self.ability.name == "Bootstraps" then
            loc_vars = {
                self.ability.extra.mult,
                self.ability.extra.dollars,
                self.ability.extra.mult * math.floor((G.GAME.dollars + (G.GAME.dollar_buffer or 0)) / self.ability.extra.dollars)
            }
        elseif self.ability.name == "Caino" then
            loc_vars = {self.ability.extra, self.ability.caino_xmult}
        elseif self.ability.name == "Triboulet" then
            loc_vars = {self.ability.extra}
        elseif self.ability.name == "Yorick" then
            loc_vars = {self.ability.extra.xmult, self.ability.extra.discards, self.ability.yorick_discards, self.ability.x_mult}
    ...
    if self.edition then
        ...
        elseif self.edition.type == "negative" and (self.ability.set == "Enhanced" or self.ability.set == "Default") then
            ...
            badges[badges.length + 1] = self.edition.type == "holo" and "holographic" or self.edition.type
        end
    end
    if self.seal then
        badges[badges.length + 1] = string.lower(self.seal) .. "_seal"
    end
    ...
    if self.sticker or self.sticker_run and self.sticker_run ~= "NONE" and G.SETTINGS.run_stake_stickers then
        loc_vars = loc_vars or ({})
        loc_vars.sticker = self.sticker or self.sticker_run
    end
    return generate_card_ui(
        _G,
        self.config.center,
...
function Card.prototype.get_nominal(self, mod)
    local mult = 1
    local rank_mult = 1
    if mod == "suit" then
        mult = 10000
...
function Card.prototype.get_id(self)
    if SMODS:has_no_rank(self) and not self.vampired then
        return -math.random(100, 1000000)
    end
    return self.base.id
end
function Card.prototype.is_face(self, from_boss)
    ...
    if id > 0 and rank and rank.face or ({next(find_joker(_G, "Pareidolia"))}) then
        return true
    end
end
function Card.prototype.get_original_rank(self)
    return self.base.original_value
end
function Card.prototype.get_chip_bonus(self)
    if self.debuff then
        return 0
    end
...
function Card.prototype.get_chip_mult(self)
    if self.debuff then
        return 0
    end
    if self.ability.set == "Joker" then
    ...
    if self.ability.effect == "Lucky Card" then
        if pseudorandom(_G, "lucky_mult") < G.GAME.probabilities.normal / 5 then
...
function Card.prototype.get_chip_x_mult(self, context)
    if self.debuff then
        return 0
    end
    if self.ability.set == "Joker" then
...
function Card.prototype.get_chip_h_mult(self)
    if self.debuff then
        return 0
    end
    return self.ability.h_mult
end
function Card.prototype.get_chip_h_x_mult(self)
    if self.debuff then
        return 0
    end
    return self.ability.h_x_mult
end
function Card.prototype.get_edition(self)
    if self.debuff then
        return
    end
    if self.edition then
...
function Card.prototype.get_end_of_round_effect(self, context)
    if self.debuff then
        return {}
    end
    local ret = {}
    ...
    if self.seal == "Blue" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit and not self.ability.extra_enhancement then
        ...
        card_eval_status_text(
            ...
            nil,
            {
                message = localize(_G, "k_plus_planet"),
...
function Card.prototype.get_p_dollars(self)
    if self.debuff then
        return 0
    end
    local ret = 0
    ...
    if self.ability.p_dollars > 0 then
        if self.ability.effect == "Lucky Card" then
            if pseudorandom(_G, "lucky_money") < G.GAME.probabilities.normal / 15 then
                self.lucky_trigger = true
...
function Card.prototype.use_consumeable(self, area, copier)
    stop_use(_G)
    if not copier then
        set_consumeable_usage(_G, self)
    end
    if self.debuff then
        return nil
    end
    ...
    if self.ability.consumeable.mod_conv or self.ability.consumeable.suit_conv then
        ...
        do
            ...
            while i <= G.hand.highlighted.length do
                local percent = 1.15 - (i - 0.999) / (G.hand.highlighted.length - 0.998) * 0.3
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        trigger = "after",
                        ...
                        GameEvent,
                        {
                            ...
                            func = function(self)
                                if G.hand.highlighted[i] ~= rightmost then
                                    copy_card(_G, rightmost, G.hand.highlighted[i])
                                end
                                return true
                            end
                        }
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {
                            ...
                            func = function(self)
                                ...
                                card:set_base(G.P_CARDS[suit_prefix .. tostring(rank_suffix)])
                                return true
                            end
                        }
                    ))
        ...
        do
            local i = 1
            while i <= G.hand.highlighted.length do
                local percent = 0.85 + (i - 0.999) / (G.hand.highlighted.length - 0.998) * 0.3
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        trigger = "after",
                        ...
                        func = function(self)
                            G.hand.highlighted[i]:flip()
                            play_sound(_G, "tarot2", percent, 0.6)
                            G.hand.highlighted[i]:juice_up(0.3, 0.3)
                            return true
                        end
                    }
    ...
    if self.ability.name == "Black Hole" then
        update_hand_text(
            ...
            {
                handname = localize(_G, "k_all_hands"),
                chips = "...",
                mult = "...",
                level = ""
            }
        ...
        update_hand_text(_G, {delay = 0}, {mult = "+", StatusText = true})
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                trigger = "after",
        ...
        update_hand_text(_G, {delay = 0}, {chips = "+", StatusText = true})
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                trigger = "after",
        ...
        update_hand_text(_G, {sound = "button", volume = 0.7, pitch = 0.9, delay = 0}, {level = "+1"})
        delay(_G, 1.3)
        for k, v in pairs(G.GAME.hands) do
            level_up_hand(_G, self, k, true)
        end
    ...
    if self.ability.name == "Cryptid" then
        G.E_MANAGER:add_event(__TS__New(
            ...
            {func = function(self)
                ...
                do
                    local i = 1
                    while i <= self.ability.extra do
                        G.playing_card = G.playing_card and G.playing_card + 1 or 1
                        local _card = copy_card(
                            _G,
                ...
                playing_card_joker_effects(_G, new_cards)
                return true
            end}
        ))
    end
    if self.ability.name == "Sigil" or self.ability.name == "Ouija" then
        ...
        do
            local i = 1
            while i <= G.hand.cards.length do
                local percent = 1.15 - (i - 0.999) / (G.hand.cards.length - 0.998) * 0.3
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        trigger = "after",
        ...
        if self.ability.name == "Sigil" then
            local _suit = pseudorandom_element(
                _G,
                {"S", "H", "D", "C"},
                pseudoseed(_G, "sigil")
            )
                ...
                while i <= G.hand.cards.length do
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            ...
                            local suit_prefix = tostring(_suit) .. "_"
                            local rank_suffix = card.base.id < 10 and tostring(card.base.id) or card.base.id == 10 and "T" or card.base.id == 11 and "J" or card.base.id == 12 and "Q" or card.base.id == 13 and "K" or card.base.id == 14 and "A"
        ...
        do
            local i = 1
            while i <= G.hand.cards.length do
                local percent = 0.85 + (i - 0.999) / (G.hand.cards.length - 0.998) * 0.3
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        trigger = "after",
    ...
    if self.ability.consumeable.hand_type then
        update_hand_text(
            ...
            {sound = "button", volume = 0.7, pitch = 0.8, delay = 0.3},
            {
                handname = localize(_G, self.ability.consumeable.hand_type, "poker_hands"),
        ...
        level_up_hand(_G, used_tarot, self.ability.consumeable.hand_type)
        update_hand_text(_G, {sound = "button", volume = 0.7, pitch = 1.1, delay = 0}, {mult = 0, chips = 0, handname = "", level = ""})
    end
    if self.ability.consumeable.remove_card then
        ...
        if self.ability.name == "The Hanged Man" then
            do
                local i = G.hand.highlighted.length
                while i <= 1 do
                        ...
                        do
                            ...
                            while i <= 1 do
                                ...
                                if SMODS:has_enhancement(card, "m_glass") then
                                    ...
                                    card:start_dissolve(nil, i == G.hand.highlighted.length)
                                end
                                i = i + -1
                            end
                        end
        ...
        elseif self.ability.name == "Familiar" or self.ability.name == "Grim" or self.ability.name == "Incantation" then
            destroyed_cards[destroyed_cards.length + 1] = pseudorandom_element(
                _G,
                G.hand.cards,
                pseudoseed(_G, "random_destroy")
            )
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
            ...
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
                    ...
                    func = function(self)
                        do
                            local i = destroyed_cards.length
                            while i <= 1 do
                                local card = destroyed_cards[i]
                                if SMODS:has_enhancement(card, "m_glass") then
                                    card:shatter()
                                else
                                    card:start_dissolve(nil, i ~= destroyed_cards.length)
                                end
                                i = i + -1
                            end
                        end
            ...
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
                    ...
                    func = function(self)
                            ...
                            while i <= self.ability.extra do
                                ...
                                elseif self.ability.name == "Incantation" then
                                    _rank = pseudorandom_element(
                                        ...
                                        pseudoseed(_G, "incantation_create")
                                    )
                                    _suit = pseudorandom_element(
                                        _G,
                                        {"S", "H", "D", "C"},
                                ...
                                create_playing_card(
                                    _G,
                                    {
                                        front = G.P_CARDS[tostring(_suit) .. "_" .. tostring(_rank)],
                                        center = pseudorandom_element(
                                            _G,
                                            cen_pool,
                                            pseudoseed(_G, "spe_card")
                                        )
                                    },
                                    G.hand,
                                    nil,
                        ...
                        playing_card_joker_effects(_G, cards)
                        return true
                    end
                }
            ))
        elseif self.ability.name == "Immolate" then
            ...
            pseudoshuffle(
                _G,
                temp_hand,
                pseudoseed(_G, "immolate")
            )
            do
                local i = 1
                while i <= self.ability.extra.destroy do
            ...
            delay(_G, 0.5)
            ease_dollars(_G, self.ability.extra.dollars)
        end
        delay(_G, 0.3)
    ...
    if self.ability.name == "The Fool" then
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                ...
                delay = 0.4,
                func = function(self)
                    if G.consumeables.config.card_limit > G.consumeables.cards.length then
                        play_sound(_G, "timpani")
                        local card = create_card(
                            _G,
                            "Tarot_Planet",
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    ...
                    ease_dollars(
                        ...
                        math.max(
                            0,
                            math.min(G.GAME.dollars, self.ability.extra)
                        ),
                        true
                    )
                    return true
        ...
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                ...
                func = function(self)
                    play_sound(_G, "timpani")
                    used_tarot:juice_up(0.3, 0.5)
                    ease_dollars(_G, self.ability.money, true)
                    return true
                end
            }
        ))
    ...
    if self.ability.name == "The Emperor" or self.ability.name == "The High Priestess" then
        do
            local i = 1
            while i <= math.min(self.ability.consumeable.tarots or self.ability.consumeable.planets, G.consumeables.config.card_limit - G.consumeables.cards.length) do
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        trigger = "after",
                        ...
                        func = function(self)
                            if G.consumeables.config.card_limit > G.consumeables.cards.length then
                                ...
                                local card = create_card(
                                    _G,
                                    self.ability.name == "The Emperor" and "Tarot" or self.ability.name == "The High Priestess" and "Planet",
                                    G.consumeables,
                                    nil,
                                    nil,
                                    nil,
                                    nil,
                                    nil,
                                    self.ability.name == "The Emperor" and "emp" or self.ability.name == "The High Priestess" and "pri"
                                )
                                card:add_to_deck()
                                G.consumeables:emplace(card)
                                used_tarot:juice_up(0.3, 0.5)
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    ...
                    local card = create_card(
                        ...
                        nil,
                        nil,
                        self.ability.name == "Judgement" and "jud" or "sou"
                    )
                    card:add_to_deck()
                    G.jokers:emplace(card)
                    if self.ability.name == "The Soul" then
                        check_for_unlock(_G, {type = "spawn_legendary"})
                    end
                    used_tarot:juice_up(0.3, 0.5)
                    return true
                end
            ...
            GameEvent,
            {
                ...
                func = function(self)
                    ...
                    if G.GAME.dollars ~= 0 then
                        ease_dollars(_G, -G.GAME.dollars, true)
                    end
                    return true
                end
            }
    ...
    if self.ability.name == "The Wheel of Fortune" or self.ability.name == "Ectoplasm" or self.ability.name == "Hex" then
        local temp_pool = self.ability.name == "The Wheel of Fortune" and self.eligible_strength_jokers or (self.ability.name == "Ectoplasm" or self.ability.name == "Hex") and self.eligible_editionless_jokers or ({})
        if self.ability.name == "Ectoplasm" or self.ability.name == "Hex" or pseudorandom(_G, "wheel_of_fortune") < G.GAME.probabilities.normal / self.ability.extra then
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
                    trigger = "after",
                    ...
                    func = function(self)
                        ...
                        local eligible_card = pseudorandom_element(
                            _G,
                            temp_pool,
                            pseudoseed(_G, self.ability.name == "The Wheel of Fortune" and "wheel_of_fortune" or self.ability.name == "Ectoplasm" and "ectoplasm" or self.ability.name == "Hex" and "hex")
                        )
                        local edition = nil
                        ...
                        if self.ability.name == "The Wheel of Fortune" or self.ability.name == "Ectoplasm" or self.ability.name == "Hex" then
                            check_for_unlock(_G, {type = "have_edition"})
                        end
...
function Card.prototype.can_use_consumeable(self, any_state, skip_check)
    if not skip_check and (G.play and G.play.cards.length > 0 or G.CONTROLLER.locked or G.GAME.STOP_USE and G.GAME.STOP_USE > 0) then
        return false
    end
    if G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT or any_state then
        ...
        if self.ability.name == "The Wheel of Fortune" then
            if {next(self.eligible_strength_jokers)} then
                return true
            end
        end
        if self.ability.name == "Ankh" then
        ...
        if self.ability.name == "Aura" then
            if G.hand and G.hand.highlighted.length == 1 and G.hand.highlighted[1] and not G.hand.highlighted[1].edition then
                return true
            end
        end
        if self.ability.name == "Ectoplasm" or self.ability.name == "Hex" then
            if {next(self.eligible_editionless_jokers)} then
                return true
            end
        end
        if self.ability.name == "The Emperor" or self.ability.name == "The High Priestess" then
            if G.consumeables.cards.length < G.consumeables.config.card_limit or self.area == G.consumeables then
                return true
            end
        end
        if self.ability.name == "The Fool" then
            if (G.consumeables.cards.length < G.consumeables.config.card_limit or self.area == G.consumeables) and G.GAME.last_tarot_planet and G.GAME.last_tarot_planet ~= "c_fool" then
                return true
            end
        end
        if self.ability.name == "Judgement" or self.ability.name == "The Soul" or self.ability.name == "Wraith" then
            if G.jokers.cards.length < G.jokers.config.card_limit or self.area == G.jokers then
                return true
            else
                return false
            end
        ...
        if G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.TAROT_PACK or G.STATE == G.STATES.SPECTRAL_PACK or G.STATE == G.STATES.PLANET_PACK or G.STATE == G.STATES.SMODS_BOOSTER_OPENED then
            if self.ability.consumeable.max_highlighted then
                if self.ability.consumeable.mod_num >= G.hand.highlighted.length and G.hand.highlighted.length >= (self.ability.consumeable.min_highlighted or 1) then
                    return true
                end
            end
            if (self.ability.name == "Familiar" or self.ability.name == "Grim" or self.ability.name == "Incantation" or self.ability.name == "Immolate" or self.ability.name == "Sigil" or self.ability.name == "Ouija") and G.hand.cards.length > 1 then
                return true
            end
        end
    end
...
function Card.prototype.check_use(self)
    if self.ability.name == "Ankh" then
        if G.jokers.cards.length >= G.jokers.config.card_limit then
            alert_no_space(_G, self, G.jokers)
            return true
        end
    end
end
function Card.prototype.sell_card(self)
    ...
    local eval, post = __TS__Unpack(eval_card(_G, self, {selling_this = true}))
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            trigger = "immediate",
            func = function(self)
                ease_dollars(_G, self.sell_cost)
                self:start_dissolve({G.C.GOLD})
                delay(_G, 0.3)
                inc_career_stat(_G, "c_cards_sold", 1)
                if self.ability.set == "Joker" then
                    inc_career_stat(_G, "c_jokers_sold", 1)
                end
...
function Card.prototype.can_sell_card(self, context)
    if G.play and G.play.cards.length > 0 or G.CONTROLLER.locked or G.GAME.STOP_USE and G.GAME.STOP_USE > 0 then
        return false
    end
    if (G.SETTINGS.tutorial_complete or G.GAME.pseudorandom.seed ~= "TUTORIAL" or G.GAME.round_resets.ante > 1) and self.area and self.area.config.type == "joker" and not self.ability.eternal then
        return true
    end
    return false
end
function Card.prototype.calculate_dollar_bonus(self)
    if self.debuff then
        return
    end
    local obj = self.config.center
...
function Card.prototype.open(self)
    if self.ability.set == "Booster" then
        stop_use(_G)
        ...
        if self.cost > 0 then
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
                    ...
                    delay = 0.2,
                    func = function(self)
                        inc_career_stat(_G, "c_shop_dollars_spent", self.cost)
                        self:juice_up()
                        return true
                    end
                }
            ))
            ease_dollars(_G, -self.cost)
        else
            delay(_G, 0.2)
        end
        G.E_MANAGER:add_event(__TS__New(
                                ...
                                do
                                    ...
                                    while i <= _size do
                                        ...
                                        elseif self.ability.name:find("Arcana") then
                                            if G.GAME.used_vouchers.v_omen_globe and pseudorandom(_G, "omen_globe") > 0.8 then
                                                card = create_card(
                                                    _G,
                                                    "Spectral",
                                                    G.pack_cards,
                                        ...
                                        elseif self.ability.name:find("Standard") then
                                            card = create_card(
                                                ...
                                                pseudorandom(
                                                    ...
                                                    pseudoseed(
                                                        _G,
                                                        "stdset" .. tostring(G.GAME.round_resets.ante)
                                                    )
                                            ...
                                            local edition = poll_edition(
                                                _G,
                                                "standard_edition" .. tostring(G.GAME.round_resets.ante),
                                                edition_rate,
                                                true
                                            )
...
function Card.prototype.redeem(self)
    if self.ability.set == "Voucher" then
        stop_use(_G)
        if not self.config.center.discovered then
            discover_card(_G, self.config.center)
        ...
        G.E_MANAGER:add_event(__TS__New(
            GameEvent,
            {
                ...
                delay = 0.4,
                func = function(self)
                    top_dynatext = __TS__New(
                        DynaText,
                        {
                            string = localize(_G, {type = "name_text", set = self.config.center.set, key = self.config.center.key}),
                            colours = {G.C.WHITE},
                            rotate = 1,
                            shadow = true,
                            bump = true,
                    ...
                    bot_dynatext = __TS__New(
                        DynaText,
                        {
                            string = localize(_G, "k_redeemed_ex"),
                            colours = {G.C.WHITE},
                            rotate = 2,
                            shadow = true,
                            bump = true,
                    ...
                    play_sound(_G, "card1")
                    play_sound(_G, "coin1")
                    self.children.top_disp = __TS__New(UIBox, {definition = {n = G.UIT.ROOT, config = {align = "tm", r = 0.15, colour = G.C.CLEAR, padding = 0.15}, nodes = {{n = G.UIT.O, config = {object = top_dynatext}}}}, config = {align = "tm", offset = {x = 0, y = 0}, parent = self}})
                    self.children.bot_disp = __TS__New(UIBox, {definition = {n = G.UIT.ROOT, config = {align = "tm", r = 0.15, colour = G.C.CLEAR, padding = 0.15}, nodes = {{n = G.UIT.O, config = {object = bot_dynatext}}}}, config = {align = "bm", offset = {x = 0, y = 0}, parent = self}})
                    return true
                end
            }
        ))
        ...
        inc_career_stat(_G, "c_vouchers_bought", 1)
        set_voucher_usage(_G, self)
        check_for_unlock(_G, {type = "run_redeem"})
        G.GAME.current_round.voucher = nil
        self:apply_to_run()
        delay(_G, 0.6)
...
function Card.prototype.apply_to_run(self, center)
    local card_to_save = self and copy_card(_G, self) or __TS__New(
        Card,
        0,
        0,
        G.CARD_W,
    ...
    local ____temp_0 = {G.vouchers.T.x, G.vouchers.T.y}
    card_to_save.VT.x = ____temp_0[1]
    card_to_save.VT.y = ____temp_0[2]
    G.vouchers:emplace(card_to_save)
    local center_table = {name = center and center.name or self and self.ability.name, extra = center and center.config.extra or self and self.ability.extra}
    ...
    if center_table.name == "Grabber" or center_table.name == "Nacho Tong" then
        ...
        ease_hands_played(_G, center_table.extra)
    end
    ...
    if center_table.name == "Wasteful" or center_table.name == "Recyclomancy" then
        ...
        ease_discard(_G, center_table.extra)
    end
    if center_table.name == "Blank" then
        check_for_unlock(_G, {type = "blank_redeems"})
    end
    ...
    if center_table.name == "Hieroglyph" or center_table.name == "Petroglyph" then
        ease_ante(_G, -center_table.extra)
...
function Card.prototype.explode(self, dissolve_colours, explode_time_fac)
    local explode_time = 1.3 * (explode_time_fac or 1) * math.sqrt(G.SETTINGS.GAMESPEED)
    self.dissolve = 0
    self.dissolve_colours = dissolve_colours or ({G.C.WHITE})
    local start_time = G.TIMERS.TOTAL
    local percent = 0
    play_sound(_G, "explosion_buildup1")
    self.juice = {
        scale = 0,
        r = 0,
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                ...
                if G.TIMERS.TOTAL - start_time > 1.5 * explode_time then
                    return true
                end
            end
        }
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                ...
                play_sound(_G, "explosion_release1")
...
function Card.prototype.shatter(self)
    local dissolve_time = 0.7
    self.shattered = true
    self.dissolve = 0
    self.dissolve_colours = {{1, 1, 1, 0.8}}
    self:juice_up()
    local childParts = __TS__New(
        Particles,
        0,
        0,
        0,
    ...
    G.E_MANAGER:add_event(__TS__New(
        GameEvent,
        {
            ...
            func = function(self)
                play_sound(
                    _G,
                    "glass" .. tostring(math.random(1, 6)),
                    math.random() * 0.2 + 0.9,
                    0.5
                )
                play_sound(
    ...
    G.E_MANAGER:add_event(__TS__New(GameEvent, {trigger = "after", blockable = false, delay = 0.51 * dissolve_time}))
end
function Card.prototype.start_dissolve(self, dissolve_colours, silent, dissolve_time_fac, no_juice)
    ...
    G.E_MANAGER:add_event(__TS__New(GameEvent, {trigger = "after", blockable = false, delay = 1.051 * dissolve_time}))
end
function Card.prototype.start_materialize(self, dissolve_colours, silent, timefac)
    local dissolve_time = 0.6 * (timefac or 1)
    self.states.visible = true
    self.states.hover.can = false
    self.dissolve = 1
    self.dissolve_colours = dissolve_colours or self.ability.set == "Joker" and ({G.C.RARITY[self.config.center.rarity]}) or self.ability.set == "Planet" and ({G.C.SECONDARY_SET.Planet}) or self.ability.set == "Tarot" and ({G.C.SECONDARY_SET.Tarot}) or self.ability.set == "Spectral" and ({G.C.SECONDARY_SET.Spectral}) or self.ability.set == "Booster" and ({G.C.BOOSTER}) or self.ability.set == "Voucher" and ({G.C.SECONDARY_SET.Voucher, G.C.CLEAR}) or ({G.C.GREEN})
    self:juice_up()
...
function Card.prototype.calculate_seal(self, context)
    if self.debuff then
        return nil
    end
    local obj = G.P_SEALS[self.seal] or ({})
    ...
    if context.repetition then
        if self.seal == "Red" then
            return {
                message = localize(_G, "k_again_ex"),
                repetitions = 1,
                card = self
            }
        end
    ...
    if context.discard and context.other_card == self then
        if self.seal == "Purple" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
            G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
            ...
            card_eval_status_text(
                ...
                nil,
                {
                    message = localize(_G, "k_plus_tarot"),
                    colour = G.C.PURPLE
                }
            )
            return {nil, true}
...
function Card.prototype.calculate_rental(self)
    if self.ability.rental then
        ease_dollars(_G, -G.GAME.rental_rate)
        card_eval_status_text(_G, self, "dollars", -G.GAME.rental_rate)
    end
end
function Card.prototype.calculate_perishable(self)
    if self.ability.perishable and self.ability.perish_tally > 0 then
        if self.ability.perish_tally == 1 then
            ...
            card_eval_status_text(
                ...
                nil,
                {
                    message = localize(_G, "k_disabled_ex"),
                    colour = G.C.FILTER,
                    delay = 0.45
                }
            )
            ...
            card_eval_status_text(
                ...
                nil,
                {
                    message = localize(_G, {type = "variable", key = "a_remaining", vars = {self.ability.perish_tally}}),
                    colour = G.C.FILTER,
                    delay = 0.45
                }
            )
...
function Card.prototype.calculate_joker(self, context)
    if self.debuff then
        return nil
    end
    local obj = self.config.center
    if self.ability.set ~= "Enhanced" and obj.calculate and type(obj.calculate) == "function" then
        local o, t = __TS__Unpack(obj:calculate(self, context))
        if o or t then
            return {o, t}
        end
    end
    if self.ability.set == "Joker" and not self.debuff then
        if self.ability.name == "Blueprint" then
            ...
            if other_joker and other_joker ~= self and not context.no_blueprint then
                ...
                if context.blueprint > G.jokers.cards.length + 1 then
                    return
                end
        ...
        if context.open_booster then
            if self.ability.name == "Hallucination" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                if pseudorandom(
                    _G,
                    "halu" .. tostring(G.GAME.round_resets.ante)
                ) < G.GAME.probabilities.normal / self.ability.extra then
                    G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {
        ...
        elseif context.selling_this then
            ...
            if self.ability.name == "Invisible Joker" and self.ability.invis_rounds >= self.ability.extra and not context.blueprint then
                local function eval(self, card)
                    return card.ability.loyalty_remaining == 0 and not G.RESET_JIGGLES
                end
                juice_card_until(_G, self, eval, true)
                local jokers = {}
                do
                    local i = 1
                    while i <= G.jokers.cards.length do
                ...
                if jokers.length > 0 then
                    if G.jokers.cards.length <= G.jokers.config.card_limit then
                        card_eval_status_text(
                            ...
                            nil,
                            nil,
                            {message = localize(_G, "k_duplicated_ex")}
                        )
                        local chosen_joker = pseudorandom_element(
                            _G,
                            jokers,
                            pseudoseed(_G, "invisible")
                        )
                        local card = copy_card(
                            _G,
                            chosen_joker,
                        ...
                        card_eval_status_text(
                            ...
                            nil,
                            nil,
                            {message = localize(_G, "k_no_room_ex")}
                        )
                    end
                else
                    card_eval_status_text(
                        ...
                        nil,
                        nil,
                        {message = localize(_G, "k_no_other_jokers")}
                    )
                end
            end
                ...
                G.E_MANAGER:add_event(__TS__New(
                    ...
                    {func = function(self)
                        card_eval_status_text(
                            ...
                            nil,
                            nil,
                            {message = localize(_G, "k_upgrade_ex")}
                        )
                        return true
                    end}
                ))
                ...
                G.E_MANAGER:add_event(__TS__New(
                    ...
                    {func = function(self)
                        card_eval_status_text(
                            ...
                            nil,
                            {
                                message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.mult}}),
                                colour = G.C.MULT
                            }
                        )
                        return true
                ...
                G.E_MANAGER:add_event(__TS__New(
                    ...
                    {func = function(self)
                        card_eval_status_text(
                            ...
                            nil,
                            {
                                message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.x_mult}}),
                                colour = G.C.RED,
                                card = self
                            }
                        )
                ...
                G.E_MANAGER:add_event(__TS__New(
                    ...
                    {func = function(self)
                        card_eval_status_text(
                            ...
                            nil,
                            {
                                message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.extra}}),
                                colour = G.C.RED,
                                delay = 0.45,
                                card = self
                            }
        ...
        elseif context.playing_card_added and not self.getting_sliced then
            if self.ability.name == "Hologram" and not context.blueprint and context.cards and context.cards[1] then
                self.ability.x_mult = self.ability.x_mult + context.cards.length * self.ability.extra
                card_eval_status_text(
                    _G,
                    self,
                    "extra",
        ...
        elseif context.first_hand_drawn then
            if self.ability.name == "Certificate" then
                ...
                playing_card_joker_effects(_G, {true})
                return {nil, true}
            end
        ...
        elseif context.setting_blind and not self.getting_sliced then
            if self.ability.name == "Chicot" and not context.blueprint and context.blind.boss and not self.getting_sliced then
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {func = function(self)
                        G.E_MANAGER:add_event(__TS__New(
            ...
            if self.ability.name == "Madness" and not context.blueprint and not context.blind.boss then
                ...
                local joker_to_destroy = destructable_jokers.length > 0 and pseudorandom_element(
                    _G,
                    destructable_jokers,
                    pseudoseed(_G, "madness")
                ) or nil
            ...
            if self.ability.name == "Burglar" and not (context.blueprint_card or self).getting_sliced then
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {func = function(self)
                        ease_discard(_G, -G.GAME.current_round.discards_left, nil, true)
                        ease_hands_played(_G, self.ability.extra)
                        card_eval_status_text(
                            _G,
                            context.blueprint_card or self,
                            "extra",
                            ...
                            nil,
                            nil,
                            {message = localize(_G, {type = "variable", key = "a_hands", vars = {self.ability.extra}})}
                        )
                        return true
                    end}
                ))
            ...
            if self.ability.name == "Riff-raff" and not (context.blueprint_card or self).getting_sliced and G.jokers.cards.length + G.GAME.joker_buffer < G.jokers.config.card_limit then
                local jokers_to_create = math.min(2, G.jokers.config.card_limit - (G.jokers.cards.length + G.GAME.joker_buffer))
                ...
                card_eval_status_text(
                    ...
                    nil,
                    {
                        message = localize(_G, "k_plus_joker"),
                        colour = G.C.BLUE
                    }
                )
                return {nil, true}
            end
            if self.ability.name == "Cartomancer" and not (context.blueprint_card or self).getting_sliced and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {func = function(self)
            ...
            if self.ability.name == "Ceremonial Dagger" and not context.blueprint then
                ...
                if my_pos and G.jokers.cards[my_pos + 1] and not self.getting_sliced and not G.jokers.cards[my_pos + 1].ability.eternal and not G.jokers.cards[my_pos + 1].getting_sliced then
                    ...
                    card_eval_status_text(
                        ...
                        nil,
                        {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.mult + 2 * sliced_card.sell_cost}}),
                            colour = G.C.RED,
                            no_juice = true
                        }
                    )
            ...
            if self.ability.name == "Marble Joker" and not (context.blueprint_card or self).getting_sliced then
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {func = function(self)
                        local front = pseudorandom_element(
                            _G,
                            G.P_CARDS,
                            pseudoseed(_G, "marb_fr")
                        )
                        G.playing_card = G.playing_card and G.playing_card + 1 or 1
                ...
                card_eval_status_text(
                    ...
                    nil,
                    {
                        message = localize(_G, "k_plus_stone"),
        ...
        elseif context.destroying_card and not context.blueprint then
            if self.ability.name == "Sixth Sense" and context.full_hand.length == 1 and context.full_hand[1]:get_id() == 6 and G.GAME.current_round.hands_played == 0 then
                if G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                    G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {
                    ...
                    card_eval_status_text(
                        ...
                        nil,
                        {
                            message = localize(_G, "k_plus_spectral"),
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            ...
                            card_eval_status_text(
                                ...
                                nil,
                                nil,
                                {message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.caino_xmult + faces * self.ability.extra}})}
                            )
                            return true
                        end}
                    ))
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            ...
                            card_eval_status_text(
                                ...
                                nil,
                                nil,
                                {message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.x_mult + self.ability.extra * glasses}})}
                            )
                            return true
                        end}
                    ))
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            card_eval_status_text(
                                ...
                                nil,
                                nil,
                                {message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.caino_xmult}})}
                            )
                            return true
                        end}
                    ))
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            ...
                            card_eval_status_text(
                                ...
                                nil,
                                nil,
                                {message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.x_mult + self.ability.extra * glass_cards}})}
                            )
                            return true
                        end}
                    ))
                ...
                G.E_MANAGER:add_event(__TS__New(
                    ...
                    {func = function(self)
                        card_eval_status_text(
                            ...
                            nil,
                            nil,
                            {message = localize(_G, {type = "variable", key = "a_mult", vars = {G.GAME.consumeable_usage_total.tarot}})}
                        )
                        return true
                    end}
                ))
        ...
        elseif context.debuffed_hand then
            if self.ability.name == "Matador" then
                if G.GAME.blind.triggered then
                    ease_dollars(_G, self.ability.extra)
                    ...
                    return {
                        message = localize(_G, "$") + self.ability.extra,
                        colour = G.C.MONEY
                    }
                end
            end
        elseif context.pre_discard then
            if self.ability.name == "Burnt Joker" and G.GAME.current_round.discards_used <= 0 and not context.hook then
                local text, disp_text = __TS__Unpack(G.FUNCS:get_poker_hand_info(G.hand.highlighted))
                card_eval_status_text(
                    _G,
                    context.blueprint_card or self,
                    "extra",
                ...
                update_hand_text(
                    ...
                    {sound = "button", volume = 0.7, pitch = 0.8, delay = 0.3},
                    {
                        handname = localize(_G, text, "poker_hands"),
        ...
        elseif context.discard then
            if self.ability.name == "Ramen" and not context.blueprint then
                if self.ability.x_mult - self.ability.extra <= 1 then
                    ...
                    return {
                        card = self,
                        message = localize(_G, "k_eaten_ex"),
                        colour = G.C.FILTER
                    }
                else
                    self.ability.x_mult = self.ability.x_mult - self.ability.extra
                    return {
                        delay = 0.2,
                        card = self,
                        message = localize(_G, {type = "variable", key = "a_xmult_minus", vars = {self.ability.extra}}),
                        colour = G.C.RED
                    }
                end
            end
            ...
            if self.ability.name == "Trading Card" and not context.blueprint and G.GAME.current_round.discards_used <= 0 and context.full_hand.length == 1 then
                ease_dollars(_G, self.ability.extra)
                return {
                    message = localize(_G, "$") + self.ability.extra,
                    colour = G.C.MONEY,
            ...
            if self.ability.name == "Castle" and not context.other_card.debuff and context.other_card:is_suit(G.GAME.current_round.castle_card.suit) and not context.blueprint then
            ...
            if self.ability.name == "Mail-In Rebate" and not context.other_card.debuff and context.other_card:get_id() == G.GAME.current_round.mail_card.id then
                ease_dollars(_G, self.ability.extra)
                return {
                    message = localize(_G, "$") + self.ability.extra,
                    colour = G.C.MONEY,
            ...
            if self.ability.name == "Green Joker" and not context.blueprint and context.other_card == context.full_hand[context.full_hand.length] then
                ...
                if self.ability.mult ~= prev_mult then
                    return {
                        message = localize(_G, {type = "variable", key = "a_mult_minus", vars = {self.ability.extra.discard_sub}}),
                        colour = G.C.RED,
                        card = self
                    }
                end
            end
            if self.ability.name == "Faceless Joker" and context.other_card == context.full_hand[context.full_hand.length] then
                local face_cards = 0
                    ...
                    G.E_MANAGER:add_event(__TS__New(
                        ...
                        {func = function(self)
                            ...
                            card_eval_status_text(
                                ...
                                nil,
                                {
                                    message = localize(_G, "$") + self.ability.extra.dollars,
                                    colour = G.C.MONEY,
                                    delay = 0.45
                                }
                            )
        ...
        elseif context.end_of_round then
            ...
            elseif context.repetition then
                if context.cardarea == G.hand then
                    if self.ability.name == "Mime" and (({next(context.card_effects[1])}) or context.card_effects.length > 1) then
                        return {
                            message = localize(_G, "k_again_ex"),
            ...
            elseif not context.blueprint then
                ...
                if self.ability.name == "Turtle Bean" and not context.blueprint then
                    if self.ability.extra.h_size - self.ability.extra.h_mod <= 0 then
                        ...
                        return {
                            message = localize(_G, {type = "variable", key = "a_handsize_minus", vars = {self.ability.extra.h_mod}}),
                            colour = G.C.FILTER
                        }
                    end
                end
                if self.ability.name == "Invisible Joker" and not context.blueprint then
                    ...
                    return {
                        message = self.ability.invis_rounds < self.ability.extra and tostring(self.ability.invis_rounds) .. "/" .. tostring(self.ability.extra) or localize(_G, "k_active_ex"),
                        colour = G.C.FILTER
                    }
                end
                if self.ability.name == "Popcorn" and not context.blueprint then
                    if self.ability.mult - self.ability.extra <= 0 then
                        ...
                        self.ability.mult = self.ability.mult - self.ability.extra
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult_minus", vars = {self.ability.extra}}),
                            colour = G.C.MULT
                        }
                    end
                end
                if self.ability.name == "To Do List" and not context.blueprint then
                    local _poker_hands = {}
                    for k, v in pairs(G.GAME.hands) do
                        if v.visible and k ~= self.ability.to_do_poker_hand then
                            _poker_hands[_poker_hands.length + 1] = k
                        end
                    end
                    self.ability.to_do_poker_hand = pseudorandom_element(
                ...
                if self.ability.name == "Gros Michel" or self.ability.name == "Cavendish" then
                    if pseudorandom(_G, self.ability.name == "Cavendish" and "cavendish" or "gros_michel") < G.GAME.probabilities.normal / self.ability.extra.odds then
                        G.E_MANAGER:add_event(__TS__New(
                            GameEvent,
                            {func = function(self)
                                play_sound(_G, "tarot1")
                        ...
                        return {message = localize(_G, "k_extinct_ex")}
                    else
                        return {message = localize(_G, "k_safe_ex")}
                    end
                end
                if self.ability.name == "Mr. Bones" and context.game_over and G.GAME.chips / G.GAME.blind.chips >= 0.25 then
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {func = function(self)
                    ...
                    return {
                        message = localize(_G, "k_saved_ex"),
                        saved = true,
                        colour = G.C.RED
                    }
                end
        ...
        elseif context.individual then
            if context.cardarea == G.play then
                ...
                if self.ability.name == "Wee Joker" and context.other_card:get_id() == 2 and not context.blueprint then
                    self.ability.extra.chips = self.ability.extra.chips + self.ability.extra.chip_mod
                    return {
                        extra = {
                            focus = self,
                ...
                if self.ability.name == "Photograph" then
                    ...
                    do
                        local i = 1
                        while i <= context.scoring_hand.length do
                ...
                if self.ability.name == "8 Ball" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                    if context.other_card:get_id() == 8 and pseudorandom(_G, "8ball") < G.GAME.probabilities.normal / self.ability.extra then
                        G.GAME.consumeable_buffer = G.GAME.consumeable_buffer + 1
                        return {
                            extra = {
                                focus = self,
                ...
                if self.ability.name == "The Idol" and context.other_card:get_id() == G.GAME.current_round.idol_card.id and context.other_card:is_suit(G.GAME.current_round.idol_card.suit) then
                    return {x_mult = self.ability.extra, colour = G.C.RED, card = self}
                end
                if self.ability.name == "Scary Face" and context.other_card:is_face() then
                    return {chips = self.ability.extra, card = self}
                end
                if self.ability.name == "Smiley Face" and context.other_card:is_face() then
                    return {mult = self.ability.extra, card = self}
                end
                if self.ability.name == "Golden Ticket" and SMODS:has_enhancement(context.other_card, "m_gold") then
                    G.GAME.dollar_buffer = (G.GAME.dollar_buffer or 0) + self.ability.extra
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {func = function(self)
                ...
                if self.ability.name == "Scholar" and context.other_card:get_id() == 14 then
                    return {chips = self.ability.extra.chips, mult = self.ability.extra.mult, card = self}
                end
                if self.ability.name == "Walkie Talkie" and (context.other_card:get_id() == 10 or context.other_card:get_id() == 4) then
                    return {chips = self.ability.extra.chips, mult = self.ability.extra.mult, card = self}
                end
                if self.ability.name == "Business Card" and context.other_card:is_face() and pseudorandom(_G, "business") < G.GAME.probabilities.normal / self.ability.extra then
                ...
                if self.ability.name == "Fibonacci" and (context.other_card:get_id() == 2 or context.other_card:get_id() == 3 or context.other_card:get_id() == 5 or context.other_card:get_id() == 8 or context.other_card:get_id() == 14) then
                    return {mult = self.ability.extra, card = self}
                end
                ...
                if self.ability.name == "Odd Todd" and (context.other_card:get_id() <= 10 and context.other_card:get_id() >= 0 and context.other_card:get_id() % 2 == 1 or context.other_card:get_id() == 14) then
                    return {chips = self.ability.extra, card = self}
                end
                if self.ability.effect == "Suit Mult" and context.other_card:is_suit(self.ability.extra.suit) then
                    return {mult = self.ability.extra.s_mult, card = self}
                end
                if self.ability.name == "Rough Gem" and context.other_card:is_suit("Diamonds") then
                    G.GAME.dollar_buffer = (G.GAME.dollar_buffer or 0) + self.ability.extra
                    G.E_MANAGER:add_event(__TS__New(
                        GameEvent,
                        {func = function(self)
                ...
                if self.ability.name == "Onyx Agate" and context.other_card:is_suit("Clubs") then
                    return {mult = self.ability.extra, card = self}
                end
                if self.ability.name == "Arrowhead" and context.other_card:is_suit("Spades") then
                    return {chips = self.ability.extra, card = self}
                end
                if self.ability.name == "Bloodstone" and context.other_card:is_suit("Hearts") and pseudorandom(_G, "bloodstone") < G.GAME.probabilities.normal / self.ability.extra.odds then
                    return {x_mult = self.ability.extra.Xmult, card = self}
                end
                if self.ability.name == "Ancient Joker" and context.other_card:is_suit(G.GAME.current_round.ancient_card.suit) then
                    return {x_mult = self.ability.extra, card = self}
                end
                if self.ability.name == "Triboulet" and (context.other_card:get_id() == 12 or context.other_card:get_id() == 13) then
                    return {x_mult = self.ability.extra, colour = G.C.RED, card = self}
                end
            end
            if context.cardarea == G.hand then
                if self.ability.name == "Shoot the Moon" and context.other_card:get_id() == 12 then
                    if context.other_card.debuff then
                        return {
                            message = localize(_G, "k_debuffed"),
                            colour = G.C.RED,
                            card = self
                        }
                    else
                ...
                if self.ability.name == "Baron" and context.other_card:get_id() == 13 then
                    if context.other_card.debuff then
                        return {
                            message = localize(_G, "k_debuffed"),
                            colour = G.C.RED,
                ...
                if self.ability.name == "Reserved Parking" and context.other_card:is_face() and pseudorandom(_G, "parking") < G.GAME.probabilities.normal / self.ability.extra.odds then
                    if context.other_card.debuff then
                        return {
                            message = localize(_G, "k_debuffed"),
                            colour = G.C.RED,
        ...
        elseif context.repetition then
            if context.cardarea == G.play then
                if self.ability.name == "Sock and Buskin" and context.other_card:is_face() then
                    return {
                        message = localize(_G, "k_again_ex"),
                        repetitions = self.ability.extra,
                        card = self
                ...
                if self.ability.name == "Hanging Chad" and context.other_card == context.scoring_hand[1] then
                    return {
                        message = localize(_G, "k_again_ex"),
                        repetitions = self.ability.extra,
                        card = self
                ...
                if self.ability.name == "Hack" and (context.other_card:get_id() == 2 or context.other_card:get_id() == 3 or context.other_card:get_id() == 4 or context.other_card:get_id() == 5) then
                    return {
                        message = localize(_G, "k_again_ex"),
                        repetitions = self.ability.extra,
                        card = self
        ...
        elseif context.other_joker then
            if self.ability.name == "Baseball Card" and (context.other_joker.config.center.rarity == 2 or context.other_joker.config.center.rarity == "Uncommon") and self ~= context.other_joker then
                ...
                return {
                    message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.extra}}),
            ...
            if context.cardarea == G.jokers then
                if context.before then
                    ...
                    if self.ability.name == "Space Joker" and pseudorandom(_G, "space") < G.GAME.probabilities.normal / self.ability.extra then
                        return {
                            card = self,
                            level_up = true,
                            message = localize(_G, "k_level_up_ex")
                        }
                    end
                    if self.ability.name == "Square Joker" and context.full_hand.length == 4 and not context.blueprint then
                        self.ability.extra.chips = self.ability.extra.chips + self.ability.extra.chip_mod
                        return {
                            message = localize(_G, "k_upgrade_ex"),
                            colour = G.C.CHIPS,
                    ...
                    if self.ability.name == "Vampire" and not context.blueprint then
                        ...
                        if enhanced.length > 0 then
                            self.ability.x_mult = self.ability.x_mult + self.ability.extra * enhanced.length
                            return {
                                message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.x_mult}}),
                                colour = G.C.MULT,
                                card = self
                    ...
                    if self.ability.name == "DNA" and G.GAME.current_round.hands_played == 0 then
                        if context.full_hand.length == 1 then
                            G.playing_card = G.playing_card and G.playing_card + 1 or 1
                            local _card = copy_card(
                                _G,
                                context.full_hand[1],
                            ...
                            return {
                                message = localize(_G, "k_copied_ex"),
                                colour = G.C.CHIPS,
                                card = self,
                    ...
                    if self.ability.name == "Green Joker" and not context.blueprint then
                        ...
                        return {
                            card = self,
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.extra.hand_add}})
                        }
                    end
                elseif context.after then
                    if self.ability.name == "Ice Cream" and not context.blueprint then
                        if self.ability.extra.chips - self.ability.extra.chip_mod <= 0 then
                            ...
                            return {
                                message = localize(_G, "k_melted_ex"),
                                colour = G.C.CHIPS
                            }
                        else
                            self.ability.extra.chips = self.ability.extra.chips - self.ability.extra.chip_mod
                            return {
                                message = localize(_G, {type = "variable", key = "a_chips_minus", vars = {self.ability.extra.chip_mod}}),
                                colour = G.C.CHIPS
                            }
                        end
                    end
                    if self.ability.name == "Seltzer" and not context.blueprint then
                        if self.ability.extra - 1 <= 0 then
                            ...
                            return {
                                message = localize(_G, "k_drank_ex"),
                                colour = G.C.FILTER
                            }
                        else
                ...
                elseif context.joker_main then
                    if self.ability.name == "Loyalty Card" then
                        ...
                        if context.blueprint then
                            if self.ability.loyalty_remaining == self.ability.extra.every then
                                return {
                                    message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.extra.Xmult}}),
                    ...
                    if self.ability.t_mult > 0 and ({next(context.poker_hands[self.ability.type])}) then
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.t_mult}}),
                            mult_mod = self.ability.t_mult
                        }
                    end
                    if self.ability.t_chips > 0 and ({next(context.poker_hands[self.ability.type])}) then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {self.ability.t_chips}}),
                            chip_mod = self.ability.t_chips
                        }
                    end
                    if self.ability.name == "Half Joker" and context.full_hand.length <= self.ability.extra.size then
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.extra.mult}}),
                    ...
                    if self.ability.name == "Abstract Joker" then
                        ...
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {x * self.ability.extra}}),
                    ...
                    if self.ability.name == "Misprint" then
                        local temp_Mult = pseudorandom(_G, "misprint", self.ability.extra.min, self.ability.extra.max)
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {temp_Mult}}),
                            mult_mod = temp_Mult
                        }
                    end
                    if self.ability.name == "Banner" and G.GAME.current_round.discards_left > 0 then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {G.GAME.current_round.discards_left * self.ability.extra}}),
                            chip_mod = G.GAME.current_round.discards_left * self.ability.extra
                        }
                    end
                    if self.ability.name == "Stuntman" then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {self.ability.extra.chip_mod}}),
                    ...
                    if self.ability.name == "Supernova" then
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {G.GAME.hands[context.scoring_name].played}}),
                    ...
                    if self.ability.name == "Vagabond" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                    ...
                    if self.ability.name == "Superposition" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                        local aces = 0
                        do
                            local i = 1
                            while i <= context.scoring_hand.length do
                    ...
                    if self.ability.name == "Seance" and G.consumeables.cards.length + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
                    ...
                    if self.ability.name == "Seeing Double" then
                        ...
                        do
                            ...
                            while i <= context.scoring_hand.length do
                                if SMODS:has_any_suit(context.scoring_hand[i]) then
                                    if context.scoring_hand[i]:is_suit("Clubs") and suits.Clubs == 0 then
                                        suits.Clubs = suits.Clubs + 1
                                    elseif context.scoring_hand[i]:is_suit("Diamonds") and suits.Diamonds == 0 then
                                        suits.Diamonds = suits.Diamonds + 1
                                    elseif context.scoring_hand[i]:is_suit("Spades") and suits.Spades == 0 then
                                        suits.Spades = suits.Spades + 1
                                    elseif context.scoring_hand[i]:is_suit("Hearts") and suits.Hearts == 0 then
                                        suits.Hearts = suits.Hearts + 1
                                    end
                                end
                                i = i + 1
                    ...
                    if self.ability.name == "Wee Joker" then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {self.ability.extra.chips}}),
                    ...
                    if self.ability.name == "Blue Joker" and G.deck.cards.length > 0 then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {self.ability.extra * G.deck.cards.length}}),
                            chip_mod = self.ability.extra * G.deck.cards.length,
                            colour = G.C.CHIPS
                        }
                    end
                    if self.ability.name == "Erosion" and G.GAME.starting_deck_size - G.playing_cards.length > 0 then
                        return {
                            message = localize(_G, {type = "variable", key = "a_mult", vars = {self.ability.extra * (G.GAME.starting_deck_size - G.playing_cards.length)}}),
                            mult_mod = self.ability.extra * (G.GAME.starting_deck_size - G.playing_cards.length),
                            colour = G.C.MULT
                        }
                    end
                    ...
                    if self.ability.name == "Stone Joker" and self.ability.stone_tally > 0 then
                        return {
                            message = localize(_G, {type = "variable", key = "a_chips", vars = {self.ability.extra * self.ability.stone_tally}}),
                    ...
                    if self.ability.name == "Steel Joker" and self.ability.steel_tally > 0 then
                        return {
                            message = localize(_G, {type = "variable", key = "a_xmult", vars = {1 + self.ability.extra * self.ability.steel_tally}}),
                        ...
                        return {
                            message = localize(
                                _G,
                                {
                                    type = "variable",
                                    key = "a_chips",
                                    vars = {self.ability.extra * math.max(0, G.GAME.dollars + (G.GAME.dollar_buffer or 0))}
                                }
                            ),
                    ...
                    if self.ability.name == "Joker Stencil" then
                        if G.jokers.config.card_limit - G.jokers.cards.length > 0 then
                            return {
                                message = localize(_G, {type = "variable", key = "a_xmult", vars = {self.ability.x_mult}}),
                                Xmult_mod = self.ability.x_mult
                            }
                        ...
                        return {
                            message = localize(
                                _G,
                                {
                                    type = "variable",
                                    key = "a_mult",
                                    vars = {self.ability.extra.mult * math.floor((G.GAME.dollars + (G.GAME.dollar_buffer or 0)) / self.ability.extra.dollars)}
                                }
                            ),
...
function Card.prototype.is_suit(self, suit, bypass_debuff, flush_calc)
    if flush_calc then
        ...
        if ({next(find_joker(_G, "Smeared Joker"))}) and (self.base.suit == "Hearts" or self.base.suit == "Diamonds") == (suit == "Hearts" or suit == "Diamonds") then
            return true
        end
...
function Card.prototype.set_card_area(self, area)
    self.area = area
    self.parent = area
    self.layered_parallax = area.layered_parallax
end
function Card.prototype.remove_from_area(self)
    self.area = nil
    self.parent = nil
    self.layered_parallax = {x = 0, y = 0}
end
function Card.prototype.align(self)
...
function Card.prototype.flip(self)
...
function Card.prototype.update(self, dt)
    ...
    if self.ability.set == "Joker" and not self.sticker_run then
        self.sticker_run = get_joker_win_sticker(_G, self.config.center) or "NONE"
    end
    ...
    if G.STAGE == G.STAGES.RUN then
        ...
        if self.ability.name == "Joker Stencil" then
            self.ability.x_mult = G.jokers.config.card_limit - G.jokers.cards.length
            do
                local i = 1
                while i <= G.jokers.cards.length do
        ...
        if self.ability.name == "The Wheel of Fortune" then
            self.eligible_strength_jokers = EMPTY(_G, self.eligible_strength_jokers)
            for k, v in pairs(G.jokers.cards) do
        ...
        if self.ability.name == "Ectoplasm" or self.ability.name == "Hex" then
            self.eligible_editionless_jokers = EMPTY(_G, self.eligible_editionless_jokers)
            for k, v in pairs(G.jokers.cards) do
                if v.ability.set == "Joker" and not v.edition then
...
function Card.prototype.hard_set_T(self, X, Y, W, H)
    ...
    if self.children.front then
        self.children.front:hard_set_T(x, y, w, h)
    end
    self.children.back:hard_set_T(x, y, w, h)
    self.children.center:hard_set_T(x, y, w, h)
end
function Card.prototype.move(self, dt)
    Moveable:move(self, dt)
    if self.children.h_popup then
        self.children.h_popup:set_alignment(self:align_h_popup())
    end
end
function Card.prototype.align_h_popup(self)
    local focused_ui = self.children.focused_ui and true or false
    local popup_direction = (self.children.buy_button or self.area and self.area.config.view_deck or self.area and self.area.config.type == "shop") and "cl" or self.T.y < G.CARD_H * 0.8 and "bm" or "tm"
    return {
        ...
        r_bond = "Weak",
        wh_bond = "Weak",
        offset = {x = popup_direction ~= "cl" and 0 or focused_ui and -0.05 or self.ability.consumeable and 0 or self.ability.set == "Voucher" and 0 or -0.05, y = focused_ui and (popup_direction == "tm" and (self.area and self.area == G.hand and -0.08 or -0.15) or popup_direction == "bm" and 0.12 or 0) or popup_direction == "tm" and -0.13 or popup_direction == "bm" and 0.1 or 0},
        type = popup_direction
    }
end
function Card.prototype.hover(self)
...
function Card.prototype.stop_hover(self)
    Node:stop_hover(self)
end
function Card.prototype.juice_up(self, scale, rot_amount)
...
function Card.prototype.draw(self, layer)
    layer = layer or "both"
    self.hover_tilt = 1
    ...
    if not self.no_shadow and G.SETTINGS.GRAPHICS.shadows == "On" and ((layer == "shadow" or layer == "both") and (self.ability.effect ~= "Glass Card" and not self.greyed and self:should_draw_shadow()) and (self.area and self.area ~= G.discard and self.area.config.type ~= "deck" or not self.area or self.states.drag.is)) then
        self.shadow_height = 0 * (0.08 + 0.4 * math.sqrt(bit.bxor(self.velocity.x, 2))) + ((self.highlighted and self.area == G.play or self.states.drag.is) and 0.35 or self.area and self.area.config.type == "title_2" and 0.04 or 0.1)
    ...
    if layer == "card" or layer == "both" then
        ...
        if self.states.focus.is then
            local ____temp_1 = {G.CONTROLLER.cursor_position.x + self.tilt_var.dx * self.T.w * G.TILESCALE * G.TILESIZE, G.CONTROLLER.cursor_position.y + self.tilt_var.dy * self.T.h * G.TILESCALE * G.TILESIZE}
            self.tilt_var.mx = ____temp_1[1]
            self.tilt_var.my = ____temp_1[2]
        ...
        elseif self.states.hover.is then
            local ____temp_2 = {G.CONTROLLER.cursor_position.x, G.CONTROLLER.cursor_position.y}
            self.tilt_var.mx = ____temp_2[1]
            self.tilt_var.my = ____temp_2[2]
        ...
        elseif self.sprite_facing == "front" then
            ...
            if not self.config.center.discovered and (self.ability.consumeable or self.config.center.unlocked) and not self.config.center.demo and not self.bypass_discovery_center then
                ...
                if self.config.center.undiscovered and not self.config.center.undiscovered.no_overlay or not (SMODS.UndiscoveredSprites[self.ability.set] and SMODS.UndiscoveredSprites[self.ability.set].no_overlay) then
                    ...
                    if SMODS.UndiscoveredSprites[self.ability.set] and SMODS.UndiscoveredSprites[self.ability.set].overlay_sprite then
                        SMODS.UndiscoveredSprites[self.ability.set].overlay_sprite:draw_shader(
                            "dissolve",
                            nil,
                            nil,
                            nil,
            ...
            if true then
                ...
                if self.ability.eternal then
                    ...
                    G.shared_sticker_eternal:draw_shader(
                        "dissolve",
                        nil,
                        nil,
                        nil,
                ...
                if self.ability.perishable then
                    ...
                    G.shared_sticker_perishable:draw_shader(
                        "dissolve",
                        nil,
                        nil,
                        nil,
                ...
                if self.ability.rental then
                    ...
                    G.shared_sticker_rental:draw_shader(
                        "dissolve",
                        nil,
                        nil,
                        nil,
                ...
                if self.ability.name == "The Soul" and (self.config.center.discovered or self.bypass_discovery_center) then
                    local scale_mod = 0.05 + 0.05 * math.sin(1.8 * G.TIMERS.REAL) + 0.07 * math.sin((G.TIMERS.REAL - math.floor(G.TIMERS.REAL)) * math.pi * 14) * bit.bxor(
                        1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)),
                        3
                    )
                    local rotate_mod = 0.1 * math.sin(1.219 * G.TIMERS.REAL) + 0.07 * math.sin(G.TIMERS.REAL * math.pi * 5) * bit.bxor(
                        1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)),
                        2
                    )
                ...
                if self.config.center.soul_pos and (self.config.center.discovered or self.bypass_discovery_center) then
                    local scale_mod = 0.07 + 0.02 * math.sin(1.8 * G.TIMERS.REAL) + 0 * math.sin((G.TIMERS.REAL - math.floor(G.TIMERS.REAL)) * math.pi * 14) * bit.bxor(
                        1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)),
                        3
                    )
                    local rotate_mod = 0.05 * math.sin(1.219 * G.TIMERS.REAL) + 0 * math.sin(G.TIMERS.REAL * math.pi * 5) * bit.bxor(
                        1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL)),
                        2
                    )
        ...
        elseif self.sprite_facing == "back" then
            ...
            if self.area and self.area.config.type == "deck" and self.rank > 3 then
                ...
                self.back_overlay[1] = 0.5 + (self.area.cards.length - self.rank) % 7 / 50
                self.back_overlay[2] = 0.5 + (self.area.cards.length - self.rank) % 7 / 50
                self.back_overlay[3] = 0.5 + (self.area.cards.length - self.rank) % 7 / 50
        ...
        for k, v in pairs(self.children) do
            if not v.custom_draw and k ~= "focused_ui" and k ~= "front" and k ~= "back" and k ~= "soul_parts" and k ~= "center" and k ~= "floating_sprite" and k ~= "shadow" and k ~= "use_button" and k ~= "buy_button" and k ~= "buy_and_use_button" and k ~= "debuff" and k ~= "price" and k ~= "particles" and k ~= "h_popup" then
                v:draw()
            end
        end
        if (layer == "card" or layer == "both") and self.area == G.hand then
...
function Card.prototype.release(self, dragged)
    if dragged:is(Card) and self.area then
        self.area:release(dragged)
    end
end
function Card.prototype.highlight(self, is_higlighted)
    ...
    if self.ability.consumeable or self.ability.set == "Joker" or self.area and self.area == G.pack_cards then
        if self.highlighted and self.area and self.area.config.type ~= "shop" then
            ...
            self.children.use_button = __TS__New(
                UIBox,
                {
                    definition = G.UIDEF:use_and_sell_buttons(self),
                    config = {align = (self.area == G.jokers or self.area == G.consumeables) and "cr" or "bmi", offset = (self.area == G.jokers or self.area == G.consumeables) and ({x = x_off - 0.4, y = 0}) or ({x = 0, y = 0.65}), parent = self}
                }
            )
    ...
    if self.ability.consumeable or self.ability.set == "Joker" then
        if not self.highlighted and self.area and self.area.config.type == "joker" and (G.jokers.cards.length >= G.jokers.config.card_limit or self.edition and self.edition.negative) then
...
function Card.prototype.click(self)
    if self.area and self.area:can_highlight(self) then
        if self.area == G.hand and G.STATE == G.STATES.HAND_PLAYED then
            return
        end
...
function Card.prototype.save(self)
    cardTable = {
        ...
        save_fields = {center = self.config.center_key, card = self.config.card_key},
        params = self.params,
        no_ui = self.no_ui,
...
function Card.prototype.load(self, cardTable, other_card)
    local scale = 1
    self.config = {}
...
function Card.prototype.remove(self)
    self.removed = true
    if self.area then
