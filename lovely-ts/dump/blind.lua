...
Blind = __TS__Class()
Blind.name = "Blind"
__TS__ClassExtends(Blind, Moveable)
function Blind.prototype.____constructor(self, X, Y, W, H)
    ...
    self.colour = copy_table(_G, G.C.BLACK)
    self.dark_colour = darken(_G, self.colour, 0.2)
    self.children.animatedSprite = __TS__New(
        AnimatedSprite,
        self.T.x,
        self.T.y,
        self.T.w,
    ...
    if __TS__InstanceOf(self, Blind) then
        table.insert(G.I.CARD, self)
    end
end
function Blind.prototype.change_colour(self, blind_col)
    blind_col = blind_col or get_blind_main_colour(_G, self.config.blind.key or "")
    local dark_col = mix_colours(_G, blind_col, G.C.BLACK, 0.4)
    ease_colour(_G, G.C.DYN_UI.MAIN, blind_col)
    ease_colour(_G, G.C.DYN_UI.DARK, dark_col)
    if not self.boss and self.name then
        blind_col = darken(_G, G.C.BLACK, 0.05)
        dark_col = lighten(_G, G.C.BLACK, 0.07)
    else
        dark_col = mix_colours(_G, blind_col, G.C.BLACK, 0.2)
    end
    ease_colour(_G, G.C.DYN_UI.BOSS_MAIN, blind_col)
    ease_colour(_G, G.C.DYN_UI.BOSS_DARK, dark_col)
end
function Blind.prototype.set_text(self)
    if self.config.blind then
        if self.disabled then
            self.loc_name = self.name == "" and self.name or localize(_G, {type = "name_text", key = self.config.blind.key, set = "Blind"})
            self.loc_debuff_text = ""
            EMPTY(_G, self.loc_debuff_lines)
        else
            local loc_vars = nil
            if self.name == "The Ox" then
                loc_vars = {localize(_G, G.GAME.current_round.most_played_poker_hand, "poker_hands")}
            end
            ...
            if loc_target then
                ...
                for k, v in ipairs(loc_target) do
                    self.loc_debuff_text = tostring(self.loc_debuff_text) .. tostring(v) .. (k <= loc_target.length and " " or "")
...
function Blind.prototype.set_blind(self, blind, reset, silent)
    if not reset then
        if blind then
            self.in_blind = true
        end
        ...
        self.chips = get_blind_amount(_G, G.GAME.round_resets.ante) * self.mult * G.GAME.starting_params.ante_scaling
        self.chip_text = number_format(_G, self.chips)
        if not blind then
            self.chips = 0
        end
        G.GAME.current_round.dollars_to_be_earned = self.dollars > 0 and string.rep(
            localize(_G, "$"),
            self.dollars
        ) .. "" or ""
    ...
    if self.name == "The Water" and not reset then
        ...
        ease_discard(_G, -self.discards_sub)
    end
    if self.name == "The Needle" and not reset then
        self.hands_sub = G.GAME.round_resets.hands - 1
        ease_hands_played(_G, -self.hands_sub)
    end
    if self.name == "The Manacle" and not reset then
        G.hand:change_size(-1)
    end
    if self.name == "Amber Acorn" and not reset and G.jokers.cards.length > 0 then
        G.jokers:unhighlight_all()
        for k, v in ipairs(G.jokers.cards) do
            v:flip()
        end
        if G.jokers.cards.length > 1 then
            G.E_MANAGER:add_event(__TS__New(
                GameEvent,
                {
                    trigger = "after",
...
function Blind.prototype.alert_debuff(self, first)
                    ...
                    if G.STATE == G.STATES.SELECTING_HAND then
                        G.E_MANAGER:add_event(__TS__New(
                            GameEvent,
                            {
                                ...
                                blockable = false,
                                func = function(self)
                                    play_sound(_G, "whoosh1", 0.55, 0.62)
                                    do
                                        local i = 1
                                        while i <= 4 do
                                            ...
                                            G.E_MANAGER:add_event(__TS__New(
                                                GameEvent,
                                                {
                                                    ...
                                                    func = function(self)
                                                        ...
                                                        play_sound(_G, "cancel", 0.7 + 0.05 * i, 0.7)
                                                        return true
                                                    end
                                                }
                                            ))
                                    ...
                                    local hold_time = G.SETTINGS.GAMESPEED * (self.loc_debuff_text.length * 0.035 + 1.3)
...
function Blind.prototype.get_loc_debuff_text(self)
    local obj = self.config.blind
    ...
    if self.config.blind.name == "The Mouth" and self.only_hand then
        disp_text = tostring(disp_text) .. " [" .. tostring(localize(_G, self.only_hand, "poker_hands")) .. "]"
    end
    return disp_text
end
function Blind.prototype.defeat(self, silent)
    local dissolve_time = 1.3
    local extra_time = 0
    self.dissolve = 0
    self.dissolve_colours = {G.C.BLACK, G.C.RED}
    self:juice_up()
    self.children.particles = Particles(
        _G,
        0,
        0,
        0,
    ...
    if not silent then
        do
            local i = 1
            while i <= math.min(self.sound_pings or 3, 7) do
                ...
                G.E_MANAGER:add_event(__TS__New(
                    GameEvent,
                    {
                        ...
                        func = function(self)
                            play_sound(_G, "cancel", 0.8 - 0.05 * i, 1.7)
                            if i == math.min((self.sound_pings or 3) + 1, 6) then
                                play_sound(_G, "whoosh2", 0.7, 0.42)
                            end
                            return true
                        end
                    }
...
function Blind.prototype.get_type(self)
...
function Blind.prototype.disable(self)
    self.disabled = true
    for k, v in ipairs(G.jokers.cards) do
        if v.facing == "back" then
            v:flip()
...
function Blind.prototype.wiggle(self)
...
function Blind.prototype.juice_up(self, _a, _b)
    self.children.animatedSprite:juice_up(_a or 0.2, _b or 0.2)
end
function Blind.prototype.hover(self)
...
function Blind.prototype.stop_hover(self)
    self.hovering = false
    self.hover_tilt = 0
    Node:stop_hover(self)
end
function Blind.prototype.draw(self)
    if not self.states.visible then
        return
    end
    self.tilt_var = self.tilt_var or ({})
    local ____temp_0 = {G.CONTROLLER.cursor_position.x, G.CONTROLLER.cursor_position.y}
    self.tilt_var.mx = ____temp_0[1]
    self.tilt_var.my = ____temp_0[2]
...
function Blind.prototype.press_play(self)
    if self.disabled then
        return
    end
    local obj = self.config.blind
                ...
                do
                    ...
                    while i <= 2 do
                        if G.hand.cards[i] then
                            local selected_card, card_key = __TS__Unpack(pseudorandom_element(
                                _G,
                                _cards,
                                pseudoseed(_G, "hook")
                            ))
...
function Blind.prototype.modify_hand(self, cards, poker_hands, text, mult, hand_chips)
    if self.disabled then
        return {mult, hand_chips, false}
    end
    local obj = self.config.blind
    if obj.modify_hand and type(obj.modify_hand) == "function" then
        return obj:modify_hand(
            cards,
            poker_hands,
            text,
            mult,
    ...
    if self.name == "The Flint" then
        ...
        return {
            math.max(
                math.floor(mult * 0.5 + 0.5),
                1
            ),
            math.max(
                math.floor(hand_chips * 0.5 + 0.5),
                0
            ),
            true
        }
...
function Blind.prototype.debuff_hand(self, cards, hand, handname, check)
    if self.disabled then
        return
    end
    local obj = self.config.blind
    ...
    if self.debuff then
        ...
        if self.debuff.h_size_ge and cards.length < self.debuff.h_size_ge then
            self.triggered = true
            return true
        end
        if self.debuff.h_size_le and cards.length > self.debuff.h_size_le then
            self.triggered = true
            return true
        end
...
function Blind.prototype.drawn_to_hand(self)
    if not self.disabled then
        ...
        if self.name == "Cerulean Bell" then
            ...
            if not any_forced then
                ...
                local forced_card = pseudorandom_element(
                    _G,
                    G.hand.cards,
                    pseudoseed(_G, "cerulean_bell")
                )
        ...
        if self.name == "Crimson Heart" and self.prepped and G.jokers.cards[1] then
            ...
            local _card = pseudorandom_element(
                _G,
                jokers,
                pseudoseed(_G, "crimson_heart")
            )
            if _card then
...
function Blind.prototype.stay_flipped(self, area, card)
    if not self.disabled then
        local obj = self.config.blind
        ...
        if area == G.hand then
            if self.name == "The Wheel" and pseudorandom(
                _G,
                pseudoseed(_G, "wheel")
            ) < G.GAME.probabilities.normal / 7 then
                return true
            end
...
function Blind.prototype.debuff_card(self, card, from_blind)
    local obj = self.config.blind
...
function Blind.prototype.move(self, dt)
    Moveable:move(self, dt)
    self:align()
end
function Blind.prototype.change_dim(self, w, h)
...
function Blind.prototype.align(self)
    for k, v in pairs(self.children) do
        if k == "animatedSprite" then
...
function Blind.prototype.save(self)
    local blindTable = {
...
function Blind.prototype.load(self, blindTable)
    ...
    if self.dollars > 0 then
        G.GAME.current_round.dollars_to_be_earned = string.rep(
            localize(_G, "$"),
            self.dollars
        ) .. ""
        G.HUD_blind:get_UIE_by_ID("dollars_to_be_earned").config.object:pop_in(0)
