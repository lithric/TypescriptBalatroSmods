...
class Particles extends Moveable {
  ...
  update(dt) {
    ...
    while (
      G.TIMERS[this.timer_type] > this.last_real_time + this.timer &&
      (this.particles.length < this.max || this.pulsed < this.pulse_max) &&
      added_this_frame < 20
    ) {
      this.last_real_time = this.last_real_time + this.timer;
      let new_offset = {
        x: (this.fill && (0.5 - math.random()) * this.T.w) || 0,
        y: (this.fill && (0.5 - math.random()) * this.T.h) || 0,
      };
      if (this.fill && this.T.r < 0.1 && this.T.r > -0.1) {
        let newer_offset = {
          x: math.sin(this.T.r) * new_offset.y + math.cos(this.T.r) * new_offset.x,
          y: math.sin(this.T.r) * new_offset.x + math.cos(this.T.r) * new_offset.y,
        };
      ...
      table.insert(this.particles, {
        ...
        age: 0,
        velocity:
          this.speed * (this.vel_variation * math.random() + (1 - this.vel_variation)) * 0.7,
        r_vel: 0.2 * (0.5 - math.random()),
        e_prev: 0,
        e_curr: 0,
        scale: 0,
        visible_scale: 0,
        time: G.TIMERS[this.timer_type],
        colour: pseudorandom_element(this.colours)[0] ?? [NaN, NaN, NaN, NaN],
        offset: new_offset,
      });
  ...
  move(dt) {
    ...
    for (let i = this.particles.length - 1; i >= 0; i += -1) {
      ...
      this.particles[i].e_curr = math.min(
        2 *
          math.min(
            (this.particles[i].age / this.lifespan) * this.scale,
            this.scale * ((this.lifespan - this.particles[i].age) / this.lifespan)
          ),
        this.scale
      );
      this.particles[i].e_vel =
        (this.particles[i].e_curr - this.particles[i].e_prev) * this.scale * dt +
        (1 - this.scale * dt) * (this.particles[i].e_vel ?? 1);
      this.particles[i].scale = this.particles[i].scale + (this.particles[i].e_vel ?? 1);
      this.particles[i].scale = math.min(
        2 *
          math.min(
            (this.particles[i].age / this.lifespan) * this.scale,
            this.scale * ((this.lifespan - this.particles[i].age) / this.lifespan)
      ...
      if (this.particles[i].scale < 0) {
        ...
        this.particles[i].offset.x =
          this.particles[i].offset.x +
          this.particles[i].velocity * math.sin(this.particles[i].dir) * dt;
        this.particles[i].offset.y =
          this.particles[i].offset.y +
          this.particles[i].velocity * math.cos(this.particles[i].dir) * dt;
        this.particles[i].facing = this.particles[i].facing + this.particles[i].r_vel * dt;
        this.particles[i].velocity = math.max(
          0,
          this.particles[i].velocity - this.particles[i].velocity * 0.07 * dt
        );
      }
    }
  }
    ...
    for (const [k, w] of pairs(this.particles)) {
      ...
      if (v.draw) {
        ...
        love.graphics.setColor(
          ...
          v.colour[1],
          v.colour[2],
          v.colour[3] * alpha * (1 - this.fade_alpha)
        );
