...
class Moveable extends LuaNode {
  ...
  constructor(X?: { T: TransformValue } | number, Y?: number, W?: number, H?: number) {
    let args: { T: TransformValue | TransformArray } = (typeof X === "object" && X) || {
      T: [X || 0, Y || 0, W || 0, H || 0],
    };
    super(args);
    this.VT = {
  ...
  set_alignment(args: {
    ...
    if (args.major) {
      this.set_role({
        role_type: "Minor",
        major: args.major,
        xy_bond: args.bond || args.xy_bond || "Weak",
        wh_bond: args.wh_bond || this.role.wh_bond,
        r_bond: args.r_bond || this.role.r_bond,
        scale_bond: args.scale_bond || this.role.scale_bond,
      });
    }
    this.alignment.type = args.type || this.alignment.type;
    if (
      (args.offset && typeof args.offset === "object" && !(args.offset.y && args.offset.x)) ||
      typeof args.offset !== "object"
    ) {
  ...
  align_to_major() {
    if (this.alignment.type !== this.alignment.prev_type) {
      this.alignment.type_list = {
        a: this.alignment.type === "a",
        m: string.find(this.alignment.type, "m"),
        c: string.find(this.alignment.type, "c"),
        b: string.find(this.alignment.type, "b"),
        t: string.find(this.alignment.type, "t"),
        l: string.find(this.alignment.type, "l"),
        r: string.find(this.alignment.type, "r"),
        i: string.find(this.alignment.type, "i"),
      };
    }
    ...
    if (this.alignment.type_list.m) {
      this.role.offset.x =
        0.5 * this.role.major.T.w -
        this.Mid.T.w / 2 +
        this.alignment.offset.x -
        this.Mid.T.x +
        this.T.x;
    }
    if (this.alignment.type_list.c) {
      this.role.offset.y =
        0.5 * this.role.major.T.h -
        this.Mid.T.h / 2 +
        this.alignment.offset.y -
        this.Mid.T.y +
        this.T.y;
    }
    ...
    [this.alignment.prev_offset.x, this.alignment.prev_offset.y] = [
      this.alignment.offset.x,
      this.alignment.offset.y,
    ];
  }
  ...
  drag(offset: Position2D) {
    if (this.states.drag.can || offset) {
      ...
      [_t.x, _t.y] = [
        this.container.T.w / 2 - this.container.T.x,
        this.container.T.h / 2 - this.container.T.y,
      ];
      point_translate(_p, _t);
      if (!offset) {
  ...
  juice_up(amount: number, rot_amt: any) {
    ...
    this.juice = {
      ...
      scale_amt: amount,
      r: 0,
      r_amt: rot_amt || pseudorandom_element([0.6 * amount, -0.6 * amount]) || 0,
  ...
  move_juice(dt: number) {
    if (this.juice && !this.juice.handled_elsewhere) {
      if (this.juice.end_time < G.TIMERS.REAL) {
        ...
        this.juice.scale =
          this.juice.scale_amt *
          math.sin(50.8 * (G.TIMERS.REAL - this.juice.start_time)) *
          math.max(
            0,
            ((this.juice.end_time - G.TIMERS.REAL) /
              (this.juice.end_time - this.juice.start_time)) ^
              3
          );
        this.juice.r =
          this.juice.r_amt *
          math.sin(40.8 * (G.TIMERS.REAL - this.juice.start_time)) *
          math.max(
            0,
            ((this.juice.end_time - G.TIMERS.REAL) /
              (this.juice.end_time - this.juice.start_time)) ^
  ...
  move(dt: number) {
    ...
    if (this.role.role_type === "Glued") {
      ...
      if (this.role.major.FRAME.MOVE < G.FRAMES.MOVE) {
        this.role.major.move(dt);
      }
  ...
  move_with_major(dt: number) {
    ...
    if (this.role.r_bond === "Weak") {
      [MWM.rotated_offset.x, MWM.rotated_offset.y] = [
        this.role.offset.x + major_tab.offset.x,
        this.role.offset.y + major_tab.offset.y,
      ];
    } else {
      if (major_tab.major.VT.r < 0.0001 && major_tab.major.VT.r > -0.0001) {
        ...
        [MWM.angles.cos, MWM.angles.sin] = [
          math.cos(major_tab.major.VT.r),
          math.sin(major_tab.major.VT.r),
        ];
        [MWM.WH.w, MWM.WH.h] = [
          -this.T.w / 2 + major_tab.major.T.w / 2,
          -this.T.h / 2 + major_tab.major.T.h / 2,
        ];
        [MWM.offs.x, MWM.offs.y] = [
          this.role.offset.x + major_tab.offset.x - MWM.WH.w,
          this.role.offset.y + major_tab.offset.y - MWM.WH.h,
        ];
    ...
    if (this.role.scale_bond === "Strong") {
      this.VT.scale =
        this.T.scale * (major_tab.major.VT.scale / major_tab.major.T.scale) +
  ...
  move_xy(dt: number) {
    if (
      ...
      this.velocity.x =
        G.exp_times.xy * this.velocity.x + (1 - G.exp_times.xy) * (this.T.x - this.VT.x) * 35 * dt;
      this.velocity.y =
        G.exp_times.xy * this.velocity.y + (1 - G.exp_times.xy) * (this.T.y - this.VT.y) * 35 * dt;
      if (
        this.velocity.x * this.velocity.x + this.velocity.y * this.velocity.y >
        (G.exp_times.max_vel ?? 0) * (G.exp_times.max_vel ?? 0)
      ) {
        let actual_vel = math.sqrt(
          this.velocity.x * this.velocity.x + this.velocity.y * this.velocity.y
        );
  ...
  move_scale(dt: any) {
    let des_scale =
      this.T.scale +
      ((this.zoom && ((this.states.drag.is && 0.1) || 0) + ((this.states.hover.is && 0.05) || 0)) ||
        0) +
      ((this.juice && this.juice.scale) || 0);
    if (des_scale !== this.VT.scale || math.abs(this.velocity.scale) > 0.001) {
      this.STATIONARY = false;
      this.velocity.scale =
        G.exp_times.scale * this.velocity.scale +
        (1 - G.exp_times.scale) * (des_scale - this.VT.scale);
      this.VT.scale = this.VT.scale + this.velocity.scale;
    }
  }
  move_wh(dt: number) {
    if (
      (this.T.w !== this.VT.w && !this.pinch.x) ||
      (this.T.h !== this.VT.h && !this.pinch.y) ||
      (this.VT.w > 0 && this.pinch.x) ||
      (this.VT.h > 0 && this.pinch.y)
    ) {
      this.STATIONARY = false;
  ...
  set_role(args: {
    ...
    this.role = {
      role_type: args.role_type || this.role.role_type,
      offset: args.offset || this.role.offset,
      major: args.major || this.role.major,
      xy_bond: args.xy_bond || this.role.xy_bond,
      wh_bond: args.wh_bond || this.role.wh_bond,
      r_bond: args.r_bond || this.role.r_bond,
      scale_bond: args.scale_bond || this.role.scale_bond,
      draw_major: args.draw_major || this.role.draw_major,
    };
  ...
  get_major() {
    if (
      this.role.role_type !== "Major" &&
      this.role.major !== this &&
      this.role.xy_bond !== "Weak" &&
      this.role.r_bond !== "Weak"
    ) {
      if (!this.FRAME.MAJOR || G.REFRESH_FRAME_MAJOR_CACHE) {
        ...
        [this.FRAME.MAJOR.offset.x, this.FRAME.MAJOR.offset.y] = [
          major.offset.x + this.role.offset.x + this.layered_parallax.x,
          major.offset.y + this.role.offset.y + this.layered_parallax.y,
        ];
      }
      return this.FRAME.MAJOR;
    } else {
      this.ARGS.get_major = this.ARGS.get_major || {
        major: undefined,
...
var MWM: {
  ...
  rotated_offset: { x: NaN, y: NaN },
  angles: { sin: NaN, cos: NaN },
  WH: { w: NaN, h: NaN },
  offs: { x: NaN, y: NaN },
};
