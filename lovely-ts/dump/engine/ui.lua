...
UIBox = __TS__Class()
UIBox.name = "UIBox"
__TS__ClassExtends(UIBox, Moveable)
function UIBox.prototype.____constructor(self, args)
    Moveable.prototype.____constructor(self, args.T)
    ...
    if args.config then
        ...
        self:set_role({xy_bond = args.config.xy_bond, r_bond = args.config.r_bond, wh_bond = args.config.wh_bond or "Weak", scale_bond = args.config.scale_bond or "Weak"})
    ...
    self:align_to_major()
    local ____temp_0 = {self.T.x, self.T.y}
    self.VT.x = ____temp_0[1]
    self.VT.y = ____temp_0[2]
    local ____temp_1 = {self.T.w, self.T.h}
    self.VT.w = ____temp_1[1]
    self.VT.h = ____temp_1[2]
...
function UIBox.prototype.get_UIE_by_ID(self, id, node)
    if not node then
        node = self.UIRoot
    end
...
function UIBox.prototype.calculate_xywh(self, node, _T, recalculate, _scale)
    node.ARGS.xywh_node_trans = node.ARGS.xywh_node_trans or ({})
    local _nt = node.ARGS.xywh_node_trans
    local _ct = {}
    local ____temp_2 = {0, 0, 0, 0}
    _ct.x = ____temp_2[1]
    _ct.y = ____temp_2[2]
    _ct.w = ____temp_2[3]
    _ct.h = ____temp_2[4]
    ...
    if node.UIT == G.UIT.B or node.UIT == G.UIT.T or node.UIT == G.UIT.O then
        local ____temp_3 = {_T.x, _T.y, node.config.w or node.config.object and node.config.object.T.w, node.config.h or node.config.object and node.config.object.T.h}
        _nt.x = ____temp_3[1]
        _nt.y = ____temp_3[2]
        _nt.w = ____temp_3[3]
        _nt.h = ____temp_3[4]
        if node.UIT == G.UIT.T then
            ...
            if node.config.ref_table and node.config.ref_value then
                ...
                if node.config.func and not recalculate then
                    local ____self_4 = G.FUNCS
                    ____self_4[node.config.func](____self_4, node)
                end
            end
            if not node.config.text then
            ...
            local ____temp_5 = {_T.x, _T.y, tx / (G.TILESIZE * G.TILESCALE), ty / (G.TILESIZE * G.TILESCALE)}
            _nt.x = ____temp_5[1]
            _nt.y = ____temp_5[2]
            _nt.w = ____temp_5[3]
            _nt.h = ____temp_5[4]
        ...
        do
            ...
            while i <= 2 do
                if i == 1 or i == 2 and (node.config.maxw and _ct.w > node.config.maxw or node.config.maxh and _ct.h > node.config.maxh) then
                    ...
                    local ____temp_6 = {_T.x, _T.y, node.config.minw or 0, node.config.minh or 0}
                    _nt.x = ____temp_6[1]
                    _nt.y = ____temp_6[2]
                    _nt.w = ____temp_6[3]
                    _nt.h = ____temp_6[4]
                    if node.UIT == G.UIT.ROOT then
                        local ____temp_7 = {0, 0, node.config.minw or 0, node.config.minh or 0}
                        _nt.x = ____temp_7[1]
                        _nt.y = ____temp_7[2]
                        _nt.w = ____temp_7[3]
                        _nt.h = ____temp_7[4]
                    end
                    local ____temp_8 = {_nt.x + padding, _nt.y + padding, 0, 0}
                    _ct.x = ____temp_8[1]
                    _ct.y = ____temp_8[2]
                    _ct.w = ____temp_8[3]
                    _ct.h = ____temp_8[4]
                    ...
                    for k, v in ipairs(node.children) do
                        if getmetatable(v) == UIElement then
                            if v.config and v.config.scale then
                                v.config.scale = v.config.scale * fac
                            end
                            _tw, _th = __TS__Unpack(self:calculate_xywh(v, _ct, recalculate, fac))
                            if _th and _tw then
                                if v.UIT == G.UIT.R then
                                    _ct.h = _ct.h + _th + padding
                                    _ct.y = _ct.y + _th + padding
                                    if _tw + padding > _ct.w then
                                        _ct.w = _tw + padding
                                    end
...
function UIBox.prototype.remove_group(self, node, group)
    node = node or self.UIRoot
    for k, v in pairs(node.children) do
        if self:remove_group(v, group) then
            node.children[k] = nil
        end
    end
    ...
    if not node.parent or true then
        ...
        self.UIRoot:set_wh()
        self.UIRoot:set_alignment()
    end
end
function UIBox.prototype.get_group(self, node, group, ingroup)
    node = node or self.UIRoot
    ingroup = ingroup or ({})
    for k, v in pairs(node.children) do
...
function UIBox.prototype.set_parent_child(self, node, parent)
    local UIE = UIElement(
        _G,
        parent,
        self,
...
function UIBox.prototype.remove(self)
    if self == G.OVERLAY_MENU then
...
function UIBox.prototype.draw(self)
...
function UIBox.prototype.recalculate(self)
    self:calculate_xywh(self.UIRoot, self.T, true)
    self.UIRoot:set_wh()
    self.UIRoot:set_alignments()
    self.T.w = self.UIRoot.T.w
...
function UIBox.prototype.move(self, dt)
    Moveable:move(self, dt)
    Moveable:move(self.UIRoot, dt)
end
function UIBox.prototype.drag(self, offset)
    Moveable:drag(self, offset)
    Moveable:move(self.UIRoot, dt)
end
function UIBox.prototype.add_child(self, node, parent)
    self:set_parent_child(node, parent)
    self:recalculate()
end
function UIBox.prototype.set_container(self, container)
    self.UIRoot:set_container(container)
    Node:set_container(self, container)
end
function UIBox.prototype.print_topology(self, indent)
    local box_str = "| UIBox | - ID:" .. tostring(self.ID) .. " w/h:" .. tostring(self.T.w) .. "/" .. tostring(self.T.h)
    local indent = indent or 0
    box_str = box_str .. self.UIRoot:print_topology(indent)
    return box_str
end
UIElement = __TS__Class()
UIElement.name = "UIElement"
__TS__ClassExtends(UIElement, Moveable)
function UIElement.prototype.____constructor(self, parent, new_UIBox, new_UIT, config)
    Moveable.prototype.____constructor(self)
    self.parent = parent
    self.UIT = new_UIT
...
function UIElement.prototype.set_values(self, _T, recalculate)
    ...
    if self.config.collideable then
        self.states.collide.can = true
    end
    ...
    if self.config.juice then
        ...
        if self.UIT == G.UIT.O then
            self.config.object:juice_up(0.5)
        end
        if self.UIT == G.UIT.B then
            self:juice_up()
        end
        if self.UIT == G.UIT.C then
            self:juice_up()
        end
        if self.UIT == G.UIT.R then
            self:juice_up()
        end
    ...
    if self.config and self.config.func and ((self.config.button_UIE or self.config.button) and self.config.func ~= "set_button_pip" or self.config.insta_func) then
        local ____self_9 = G.FUNCS
        ____self_9[self.config.func](____self_9, self)
    end
end
function UIElement.prototype.print_topology(self, indent)
    local UIT = "????"
    ...
    local box_str = "\\n" .. string.rep("  ", indent) .. "| " .. UIT .. " | - ID:" .. tostring(self.ID) .. " w/h:" .. tostring(self.T.w) .. "/" .. tostring(self.T.h)
    if UIT == "O" then
        box_str = box_str .. " OBJ:" .. (getmetatable(self.config.object) == CardArea and "CardArea" or getmetatable(self.config.object) == Card and "Card" or getmetatable(self.config.object) == UIBox and "UIBox" or getmetatable(self.config.object) == Particles and "Particles" or getmetatable(self.config.object) == DynaText and "DynaText" or getmetatable(self.config.object) == Sprite and "Sprite" or getmetatable(self.config.object) == AnimatedSprite and "AnimatedSprite" or "OTHER")
    elseif UIT == "T" then
        box_str = box_str .. " TEXT:" .. tostring(self.config.text or "REF")
    end
    for k, v in ipairs(self.children) do
        if v.print_topology then
            box_str = box_str .. tostring(v:print_topology(indent + 1))
        end
    end
    return box_str
end
function UIElement.prototype.initialize_VT(self)
    self:move_with_major(0)
    self:calculate_parrallax()
    for _, v in pairs(self.children) do
        if v.initialize_VT then
            v:initialize_VT()
        end
    end
    local ____temp_10 = {self.T.w, self.T.h}
    self.VT.w = ____temp_10[1]
    self.VT.h = ____temp_10[2]
    if self.UIT == G.UIT.T then
        self:update_text()
    end
    if self.config.object then
...
function UIElement.prototype.juice_up(self, amount, rot_amt)
    if self.UIT == G.UIT.O then
        if self.config.object then
            self.config.object:juice_up(amount, rot_amt)
        end
    else
        Moveable:juice_up(self, amount, rot_amt)
    end
end
function UIElement.prototype.can_drag(self)
    if self.states.drag.can then
        return self
    end
    return self.UIBox:can_drag()
end
function UIElement.prototype.draw(self)
end
function UIElement.prototype.draw_children(self, layer)
    if self.states.visible then
        for k, v in pairs(self.children) do
            if not v.config.draw_layer and k ~= "h_popup" and k ~= "alert" then
                if v.draw_this and not v.config.draw_after then
                    v:draw_this()
                else
                    v:draw()
                end
                ...
                if v.draw_this and v.config.draw_after then
                    v:draw_this()
                else
                    v:draw()
                end
...
function UIElement.prototype.set_wh(self)
    ...
    if ({next(self.children)}) == nil or self.config.no_fill then
        ...
        for k, w in pairs(self.children) do
            if w.set_wh then
                local _cw, _ch = __TS__Unpack(w:set_wh())
                if _cw and _ch then
                    if _cw > _max_w then
                        _max_w = _cw
                    end
        ...
        for k, w in pairs(self.children) do
            if w.UIT == G.UIT.R then
                w.T.w = _max_w
            end
            if w.UIT == G.UIT.C then
                w.T.h = _max_h
            end
        end
    end
...
function UIElement.prototype.align(self, x, y)
...
function UIElement.prototype.set_alignments(self)
    for k, v in pairs(self.children) do
        ...
        if v.set_alignments then
            v:set_alignments()
        end
    end
end
function UIElement.prototype.update_text(self)
...
function UIElement.prototype.update_object(self)
...
function UIElement.prototype.draw_this(self)
    ...
    if self.config.button or self.config.button_UIE then
        ...
        if self.config.button and (self.last_clicked and self.last_clicked > G.TIMERS.REAL - 0.1 or self.config.button and (self.states.hover.is or self.states.drag.is) and G.CONTROLLER.is_cursor_down) then
        ...
        if self.config.button_UIE and not self.config.button_UIE.config.button then
            button_active = false
        end
    end
    if self.config.colour[4] > 0.01 then
        ...
        elseif self.UIT == G.UIT.B or self.UIT == G.UIT.C or self.UIT == G.UIT.R or self.UIT == G.UIT.ROOT then
            ...
            if self.config.shadow and G.SETTINGS.GRAPHICS.shadows == "On" then
                ...
                if self.config.r and self.VT.w > 0.01 then
                    ...
                    love.graphics.rectangle(
                        "fill",
                        -self.shadow_parrallax.x * parallax_dist,
                        -self.shadow_parrallax.y * parallax_dist,
                        self.VT.w * G.TILESIZE,
                        self.VT.h * G.TILESIZE
                    )
                end
            ...
            if self.config.emboss then
                love.graphics.setColor(darken(_G, self.config.colour, self.states.hover.is and 0.5 or 0.3, true))
            ...
            self.ARGS.button_colours[1] = self.config.button_delay and mix_colours(_G, self.config.colour, G.C.L_BLACK, 0.5) or self.config.colour
        ...
        elseif self.UIT == G.UIT.O and self.config.object then
            if self.config.focus_with_object and self.config.object.states.focus.is then
                ...
                local lw = 50 * bit.bxor(
                    math.max(0, self.object_focus_timer - G.TIMERS.REAL + 0.3),
                    2
                )
                prep_draw(_G, self, 1)
                love.graphics.scale(1 / G.TILESIZE)
                love.graphics.setLineWidth(lw + 1.5)
                love.graphics.setColor(adjust_alpha(_G, G.C.WHITE, 0.2 * lw, true))
                self:draw_pixellated_rect("fill", parallax_dist)
                love.graphics.setColor(self.config.colour[4] > 0 and mix_colours(_G, G.C.WHITE, self.config.colour, 0.8) or G.C.WHITE)
    ...
    if self.config.outline and self.config.outline_colour[4] > 0.01 then
        if self.config.outline then
            ...
            if self.config.line_emboss then
                love.graphics.setColor(darken(_G, self.config.outline_colour, self.states.hover.is and 0.5 or 0.3, true))
    ...
    if self.states.focus.is then
        ...
        local lw = 50 * bit.bxor(
            math.max(0, self.focus_timer - G.TIMERS.REAL + 0.3),
            2
        )
        prep_draw(_G, self, 1)
        love.graphics.scale(1 / G.TILESIZE)
    ...
    if self.config.chosen then
        ...
        if self.config.shadow and G.SETTINGS.GRAPHICS.shadows == "On" then
            ...
            love.graphics.polygon(
                ...
                get_chosen_triangle_from_rect(
                    _G,
                    self.layered_parallax.x - self.shadow_parrallax.x * parallax_dist * 0.5,
                    self.layered_parallax.y - self.shadow_parrallax.y * parallax_dist * 0.5,
                    self.VT.w * G.TILESIZE,
                    self.VT.h * G.TILESIZE,
                    self.config.chosen == "vert"
                )
            )
        end
        love.graphics.pop()
        ...
        love.graphics.polygon(
            ...
            get_chosen_triangle_from_rect(
                _G,
                self.layered_parallax.x,
                self.layered_parallax.y,
                self.VT.w * G.TILESIZE,
                self.VT.h * G.TILESIZE,
                self.config.chosen == "vert"
            )
...
function UIElement.prototype.draw_pixellated_rect(self, _type, _parallax, _emboss, _progress)
    if not self.pixellated_rect or self.pixellated_rect[_type].vertices.length < 1 or _parallax ~= self.pixellated_rect.parallax or self.pixellated_rect.w ~= self.VT.w or self.pixellated_rect.h ~= self.VT.h or self.pixellated_rect.sw ~= self.shadow_parrallax.x or self.pixellated_rect.sh ~= self.shadow_parrallax.y or self.pixellated_rect.progress ~= (_progress or 1) then
        ...
        local res = self.config.res or math.min(
            self.VT.w,
            self.VT.h + math.abs(ext_up) / G.TILESIZE
        ) > 3.5 and 0.8 or math.min(
            self.VT.w,
            self.VT.h + math.abs(ext_up) / G.TILESIZE
        ) > 0.3 and 0.6 or 0.15
...
function UIElement.prototype.update(self, dt)
    ...
    if self.config.button_delay then
        ...
        if G.TIMERS.REAL >= self.config.button_delay_end then
            self.config.button_delay = nil
        end
    end
    ...
    if self.config and self.config.func then
        ...
        local ____self_11 = G.FUNCS
        ____self_11[self.config.func](____self_11, self)
    end
    if self.UIT == G.UIT.T then
        self:update_text()
    end
...
function UIElement.prototype.collides_with_point(self, cursor_trans)
...
function UIElement.prototype.click(self)
    if self.config.button and (not self.last_clicked or self.last_clicked + 0.1 < G.TIMERS.REAL) and self.states.visible and not self.under_overlay and not self.disable_button then
        ...
        local ____self_12 = G.FUNCS
        ____self_12[self.config.button](____self_12, self)
        ...
        if self.config.choice then
            ...
            for k, v in pairs(choices) do
                if v.config and v.config.choice then
                    v.config.chosen = false
                end
            end
            self.config.chosen = chosen_temp or true
        end
        play_sound(_G, "button", 1, 0.3)
...
function UIElement.prototype.put_focused_cursor(self)
...
function UIElement.prototype.remove(self)
    if self.config and self.config.object then
...
function UIElement.prototype.hover(self)
    if self.config and self.config.on_demand_tooltip then
        self.config.h_popup = create_popup_UIBox_tooltip(_G, self.config.on_demand_tooltip)
    ...
    if self.config.tooltip then
        self.config.h_popup = create_popup_UIBox_tooltip(_G, self.config.tooltip)
    ...
    if self.config.detailed_tooltip and G.CONTROLLER.HID.pointer then
        self.config.h_popup = create_UIBox_detailed_tooltip(_G, self.config.detailed_tooltip)
        self.config.h_popup_config = {align = "tm", offset = {x = 0, y = -0.1}, parent = self}
    end
    Node:hover(self)
end
function UIElement.prototype.stop_hover(self)
    Node:stop_hover(self)
    if self.config and self.config.on_demand_tooltip then
        self.config.h_popup = nil
    end
end
function UIElement.prototype.release(self, other)
    if self.parent then
        self.parent:release(other)
    end
end
function is_UI_containter(self, node)
