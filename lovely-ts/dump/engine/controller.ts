...
class Controller extends LuaObject {
    ...
    cursor_down: { T: TransformValue|Position2D; target?: LuaNode; time: number; handled: boolean; prev_target?: LuaNode };
    cursor_up: { T: TransformValue|Position2D; target?: LuaNode; time: number; handled: boolean; prev_target?: LuaNode };
    cursor_hover: { T: TransformValue|Position2D; target?: LuaNode; time: number; handled: boolean; prev_target?: LuaNode };
    ...
    released_buttons: [];
    interrupt: {
        stack?: any; focus: boolean; 
};
    locks: {};
    locked?: boolean;
    axis_buttons: { l_stick: { current: string; previous: string; }; r_stick: { current: string; previous: string; }; l_trig: { current: string; previous: string; }; r_trig: { current: string; previous: string; }; };
    axis_cursor_speed: number;
    button_registry: {};
    snap_cursor_to: undefined;
    cursor_context: { layer: number; stack: {}; };
    cardarea_context: {};
    HID: { last_type: string; dpad: boolean; pointer: boolean; touch: boolean; controller: boolean; mouse: boolean; axis_cursor: boolean; };
    GAMEPAD: {
        temp_console: string | number | symbol; object: undefined; mapping: undefined; name: undefined; 
};
    ...
    constructor() {
        ...
        this.axis_buttons = { l_stick: { current: "", previous: "" }, r_stick: { current: "", previous: "" }, l_trig: { current: "", previous: "" }, r_trig: { current: "", previous: "" } };
        ...
        this.HID = { last_type: "", dpad: false, pointer: true, touch: false, controller: false, mouse: true, axis_cursor: false };
        this.GAMEPAD = { object: undefined, mapping: undefined, name: undefined };
        this.GAMEPAD_CONSOLE = "";
        this.keyboard_controller = { getGamepadMappingString: function () {
                return "balatro_kbm";
            ...
            if (this.GAMEPAD_CONSOLE !== this.GAMEPAD.temp_console) {
                ...
                for (const [k, v] of pairs(G.I.SPRITE)) {
                    if ((v as Sprite).atlas === G.ASSET_ATLAS["gamepad_ui"]) {
                        (v as Sprite).sprite_pos.y = G.CONTROLLER.GAMEPAD_CONSOLE === "Nintendo" && 2 || G.CONTROLLER.GAMEPAD_CONSOLE === "Playstation" && (G.F_PS4_PLAYSTATION_GLYPHS && 3 || 1) || 0;
    ...
    get_console_from_gamepad(_gamepad) {
        G.ARGS.gamepad_patterns = G.ARGS.gamepad_patterns || { Playstation: ["%f[%w]PS%d%f[%D]", "Sony%f[%W]", "Play[Ss]tation"], Nintendo: ["Wii%f[%L]", "%f[%u]S?NES%f[%U]", "%f[%l]s?nes%f[%L]", "%f[%u]Switch%f[%L]", "Joy[- ]Cons?%f[%L]"] };
    ...
    update(dt:number) {
        ...
        if (this.locks.frame_set) {
            ...
            G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 0.1, timer: "UPTIME", blocking: false, blockable: false, no_delete: true, func: function () {
        ...
        if (this.HID.controller) {
            if (this.cursor_context.stack[this.cursor_context.layer]) {
                ...
                this.snap_to({ node: _context.node && !_context.node.REMOVED && _context.node, T: _context.cursor_pos });
                this.interrupt.stack = _context.interrupt;
                this.cursor_context.stack[this.cursor_context.layer] = undefined;
            }
            if (this.dragging.prev_target && !this.dragging.target && getmetatable(this.dragging.prev_target) === Card && !this.dragging.prev_target.REMOVED) {
        ...
        if (!this.cursor_up.handled) {
            ...
            if (this.cursor_down.target) {
                if (!this.cursor_down.target?.click_timeout || this.cursor_down.target.click_timeout * G.SPEEDFACTOR > this.cursor_up.time - this.cursor_down.time) {
                    if (Vector_Dist(this.cursor_down.T as TransformValue, this.cursor_up.T as TransformValue) < G.MIN_CLICK_DIST) {
                        if (this.cursor_down.target.states.click.can) {
                            this.clicked.target = this.cursor_down.target;
                            this.clicked.handled = false;
                        }
                    }
                    else if (this.dragging.prev_target && this.cursor_up.target && this.cursor_up.target.states.release_on.can) {
        ...
        if (this.cursor_hover.target && this.cursor_hover.target.states.hover.can && (!this.HID.touch || this.is_cursor_down)) {
        ...
        else if ((this.cursor_hover.target === undefined || this.HID.touch && !this.is_cursor_down) && this.hovering.target) {
        ...
        if (this.hovering.target) {
            ...
            if (this.hovering.prev_target !== this.hovering.target) {
                if (this.hovering.target !== this.dragging.target && !this.HID.touch) {
                    ...
                    G.E_MANAGER.add_event(new GameEvent({ trigger: "after", blockable: false, blocking: false, delay: G.MIN_HOVER_TIME, func: function () {
    ...
    add_to_registry(node, registry) {
        ...
        table.insert(this.button_registry[registry], 1, { node: node, menu: !!G.OVERLAY_MENU || !!G.SETTINGS.paused });
    };
    process_registry() {
        for (const [_, registry] of pairs(this.button_registry)) {
            for (let i = 0; i < registry.length; i++) {
                if (registry[i].click && registry[i].node.click) {
                    if (registry[i].menu === !!G.OVERLAY_MENU && registry[i].node.T.x > -2 && registry[i].node.T.x < G.ROOM.T.w + 2 && registry[i].node.T.y > -2 && registry[i].node.T.y < G.ROOM.T.h + 2) {
    ...
    mod_cursor_context_layer(delta) {
        if (delta === 1) {
            let prev_cursor_context = { node: this.focused.target, cursor_pos: { x: G.CURSOR.T.x, y: G.CURSOR.T.y }, interrupt: this.interrupt.focus };
    ...
    snap_to(args) {
        this.snap_cursor_to = { node: args.node, T: args.T, type: args.node && "node" || "transform" };
    };
    save_cardarea_focus(_cardarea) {
        if (G[_cardarea]) {
            if (this.focused.target && this.focused.target.area && this.focused.target.area === G[_cardarea]) {
    ...
    recall_cardarea_focus(_cardarea) {
        ...
        if (_cardarea && (!this.focused.target || this.interrupt.focus || !this.interrupt.focus && this.focused.target.area && this.focused.target.area === _cardarea)) {
        ...
        if (this.HID.controller) {
            ...
            if (this.dragging.target && math.abs(l_stick_x) + math.abs(l_stick_y) > 0.1) {
                ...
                if (math.abs(l_stick_x) + math.abs(l_stick_y) > 0.5) {
                    ...
                    this.axis_buttons.l_stick.current = math.abs(l_stick_x) > math.abs(l_stick_y) && (l_stick_x > 0 && "dpright" || "dpleft") || (l_stick_y > 0 && "dpdown" || "dpup");
                }
            ...
            if (mag > G.DEADZONE) {
                ...
                r_stick_x = r_stick_x + (r_stick_x > 0 && -G.DEADZONE || 0) + (r_stick_x < 0 && G.DEADZONE || 0);
                r_stick_y = r_stick_y + (r_stick_y > 0 && -G.DEADZONE || 0) + (r_stick_y < 0 && G.DEADZONE || 0);
    ...
    button_press_update(button, dt) {
        ...
        if (this.button_registry[button] && this.button_registry[button][1] && !this.button_registry[button][1].node.under_overlay) {
            ...
            if (button === "a") {
                if (this.focused.target && this.focused.target.config.focus_args && this.focused.target.config.focus_args.type === "slider" && (!G.CONTROLLER.HID.mouse && !G.CONTROLLER.HID.axis_cursor)) { }
                else {
                    this.L_cursor_press();
                }
            }
    ...
    button_hold_update(button, dt) {
        ...
        if ((button === "dpleft" || button === "dpright" || button === "dpup" || button === "dpdown") && !this.no_holdcap) {
    ...
    key_press_update(key, dt) {
        ...
        if (this.text_input_hook) {
            if (key === "escape") {
                ...
                G.FUNCS.text_input_key({ e: this.text_input_hook, key: key, caps: this.held_keys["lshift"] || this.held_keys["rshift"] });
            }
            return;
        }
        if (key === "escape") {
        ...
        if (!_RELEASE_MODE) {
            if (key === "tab" && !G.debug_tools) {
                G.debug_tools = UIBox({ definition: create_UIBox_debug_tools(), config: { align: "cr", offset: { x: G.ROOM.T.x + 11, y: 0 }, major: G.ROOM_ATTACH, bond: "Weak" } });
            ...
            if (this.hovering.target && this.hovering.target.is(Card)) {
                ...
                if (G.OVERLAY_MENU) {
                    ...
                    if (key === "3") {
                        if (_card.ability.set === "Joker" && G.jokers && G.jokers.cards.length < G.jokers.config.card_limit) {
                            add_joker(_card.config.center.key);
                            _card.set_sprites(_card.config.center);
                        }
                        if (_card.ability.consumeable && G.consumeables && G.consumeables.cards.length < G.consumeables.config.card_limit) {
                            add_joker(_card.config.center.key);
                            _card.set_sprites(_card.config.center);
                        }
                    }
    ...
    get_cursor_collision(cursor_trans) {
        ...
        if (!G.DRAW_HASH[1] || cursor_trans.x - G.ROOM.T.x < -G.DRAW_HASH_BUFF || cursor_trans.x - G.ROOM.T.x > G.TILE_W + G.DRAW_HASH_BUFF || cursor_trans.y - G.ROOM.T.y < -G.DRAW_HASH_BUFF || cursor_trans.y - G.ROOM.T.y > G.TILE_H + G.DRAW_HASH_BUFF) {
            return;
        }
    ...
    set_cursor_hover() {
        ...
        if (this.interrupt.focus || this.locked && (!G.SETTINGS.paused || G.screenwipe) || this.locks.frame || this.COYOTE_FOCUS) {
    ...
    queue_R_cursor_press(x, y) {
        ...
        if (!G.SETTINGS.paused && G.hand && G.hand.highlighted[1]) {
            if (G.play && G.play.cards.length > 0 || this.locked || this.locks.frame || G.GAME.STOP_USE && G.GAME.STOP_USE > 0) {
                return;
            }
    ...
    L_cursor_press(x, y) {
        ...
        let press_node = this.HID.touch && this.cursor_hover.target || this.hovering.target || this.focused.target;
        if (press_node) {
            this.cursor_down.target = press_node.states.click.can && press_node || press_node.can_drag() || undefined;
        }
    ...
    is_node_focusable(node) {
        ...
        if (!node.REMOVED && !node.under_overlay && (node.states.hover.can && !this.dragging.target || this.dragging.target === node) && !!node.created_on_pause === !!G.SETTINGS.paused && node.states.visible && (!node.UIBox || node.UIBox.states.visible)) {
            if (this.screen_keyboard) {
                ...
                if (node.is(Card) && (node.facing === "front" || node.area === G.hand || node.area === G.jokers || node === G.deck) && node.states.hover.can && !node.jimbo) {
                    ret_val = true;
                }
    ...
    update_focus(dir) {
        this.focused.prev_target = this.focused.target;
        if (!this.HID.controller || this.interrupt.focus || this.locked && (!G.SETTINGS.paused || G.screenwipe)) {
            if (this.focused.target) {
                this.focused.target.states.focus.is = false;
            }
            this.focused.target = undefined;
        ...
        if (this.focused.target) {
            this.focused.target.states.focus.is = false;
            if (!this.is_node_focusable(this.focused.target) || !this.focused.target.collides_with_point(G.CURSOR.T) || this.HID.axis_cursor) {
                this.focused.target = undefined;
            }
        }
        ...
        if (G.ARGS.focusables.length > 0) {
            if (dir) {
                if ((dir === "L" || dir === "R") && this.focused.target && this.focused.target.is(Card) && this.focused.target.area === G.hand && G.hand) {
                    ...
                    if (this.focused.target) {
                        ...
                        if (this.focused.target.config.focus_args && this.focused.target.config.focus_args.funnel_to) {
                        ...
                        [G.ARGS.focus_cursor_pos.x, G.ARGS.focus_cursor_pos.y] = this.hovering.target.put_focused_cursor();
                    ...
                    for (const [_, v] of pairs(G.ARGS.focusables)) {
                        if (v !== this.hovering.target && v !== this.focused.target) {
                            ...
                            if (eligible) {
                                G.ARGS.focus_list[G.ARGS.focus_list.length + 1] = { node: v, dist: math.abs(G.ARGS.focus_vec.x) + math.abs(G.ARGS.focus_vec.y) };
                            }
                        }
                    }
        ...
        if (G.ARGS.focus_list[1]) {
            if (G.ARGS.focus_list[1].node.config && G.ARGS.focus_list[1].node.config.focus_args && G.ARGS.focus_list[1].node.config.focus_args.funnel_from) {
    ...
    capture_focused_input(button, input_type, dt) {
        ...
        if (input_type === "press" && (button === "dpleft" || button === "dpright") && focused && this.dragging.target && (this.held_button_times["a"] && this.held_button_times["a"] < 0.12) && focused.area && focused.area.can_highlight(focused)) {
            this.L_cursor_release();
        ...
        else if (input_type === "press" && focused && focused.area && focused === this.dragging.target) {
        ...
        if (G.OVERLAY_MENU && !this.screen_keyboard && input_type === "press" && G.OVERLAY_MENU.get_UIE_by_ID("tab_shoulders") && (button === "leftshoulder" || button === "rightshoulder")) {
            focused = G.OVERLAY_MENU.get_UIE_by_ID("tab_shoulders");
            extern_button = true;
        }
        if (G.OVERLAY_MENU && !this.screen_keyboard && input_type === "press" && G.OVERLAY_MENU.get_UIE_by_ID("cycle_shoulders") && (button === "leftshoulder" || button === "rightshoulder")) {
        ...
        if (focused && focused.config.focus_args) {
            if (focused.config.focus_args.type === "cycle" && input_type === "press") {
                if (extern_button && button === "leftshoulder" || !extern_button && button === "dpleft") {
                    focused.children[1].click();
                    ret = true;
                }
                if (extern_button && button === "rightshoulder" || !extern_button && button === "dpright") {
                    focused.children[3].click();
                    ret = true;
                }
            }
            if (focused.config.focus_args.type === "tab" && input_type === "press") {
                let proto_choices = focused.UIBox.get_group(undefined, focused.children[1].children[1].config.group);
                let choices = {};
                ...
                for (const [k, v] of ipairs(choices)) {
                    if (v.config.chosen) {
                        ...
                        else if (extern_button && button === "rightshoulder" || !extern_button && button === "dpright") {
