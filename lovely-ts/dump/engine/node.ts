...
class LuaNode extends LuaObject {
  ...
  ARGS: {
    ...
    collides_with_point_rotation?:
      | { cos: number; sin: number }
      | { sin: undefined; cos: undefined };
  ...
  states: {
    ...
    release_on: { can: boolean; is: boolean };
  };
  container: LuaNode;
  ...
  set_offset(point: { x: number; y: number }, type: string) {
    ...
    this.ARGS.set_offset_translation = this.ARGS.set_offset_translation || {
      x: undefined,
      y: undefined,
    };
    ...
    if (this.config && this.config.d_popup) {
      if (this.children) {
        if (!this.children.d_popup) {
          this.children.d_popup = new UIBox({
            definition: this.config.d_popup,
            config: this.config.d_popup_config,
          });
  ...
  hover() {
    ...
    if (this.config && this.config.h_popup) {
      if (!this.children.h_popup) {
        ...
        this.children.h_popup = new UIBox({
          definition: this.config.h_popup,
          config: this.config.h_popup_config,
        });
        this.children.h_popup.states.collide.can = false;
  ...
  put_focused_cursor() {
    return [
      (this.T.x + this.T.w / 2 + this.container.T.x) * G.TILESCALE * G.TILESIZE,
      (this.T.y + this.T.h / 2 + this.container.T.y) * G.TILESCALE * G.TILESIZE,
    ];
  }
  ...
  translate_container() {
    if (this.container && this.container !== this) {
      love.graphics.translate(
        this.container.T.w * G.TILESCALE * G.TILESIZE * 0.5,
        this.container.T.h * G.TILESCALE * G.TILESIZE * 0.5
      );
      love.graphics.rotate(this.container.T.r);
      love.graphics.translate(
        -this.container.T.w * G.TILESCALE * G.TILESIZE * 0.5 +
          this.container.T.x * G.TILESCALE * G.TILESIZE,
        -this.container.T.h * G.TILESCALE * G.TILESIZE * 0.5 +
          this.container.T.y * G.TILESCALE * G.TILESIZE
      );
    }
  }
  remove() {
  ...
  fast_mid_dist(other_node: LuaNode) {
    return (
      math.sqrt(other_node.T.x + 0.5 * other_node.T.w - (this.T.x + this.T.w)) ** 2 +
      (other_node.T.y + 0.5 * other_node.T.h - (this.T.y + this.T.h) ** 2)
    );
  }
