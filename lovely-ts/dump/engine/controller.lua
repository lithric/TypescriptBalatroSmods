...
Controller = __TS__Class()
Controller.name = "Controller"
__TS__ClassExtends(Controller, LuaObject)
function Controller.prototype.____constructor(self)
    ...
    self.keyboard_controller = {
        getGamepadMappingString = function(self)
            return "balatro_kbm"
        end,
        getGamepadAxis = function(self)
            return 0
        end
    }
    self.is_cursor_down = false
end
function Controller.prototype.set_gamepad(self, _gamepad)
...
function Controller.prototype.get_console_from_gamepad(self, _gamepad)
    G.ARGS.gamepad_patterns = G.ARGS.gamepad_patterns or ({Playstation = {"%f[%w]PS%d%f[%D]", "Sony%f[%W]", "Play[Ss]tation"}, Nintendo = {
        "Wii%f[%L]",
        "%f[%u]S?NES%f[%U]",
        "%f[%l]s?nes%f[%L]",
        "%f[%u]Switch%f[%L]",
...
function Controller.prototype.set_HID_flags(self, HID_type, button)
...
function Controller.prototype.set_cursor_position(self)
    if self.HID.mouse or self.HID.touch then
        ...
        local ____temp_0 = {love.mouse.getPosition()}
        self.cursor_position.x = ____temp_0[1]
        self.cursor_position.y = ____temp_0[2]
...
function Controller.prototype.update(self, dt)
    self.locked = false
    ...
    if self.overlay_timer > 1.5 then
        self.locks.frame = nil
    end
    self:cull_registry()
    ...
    if not G.screenwipe then
        for k, v in pairs(self.pressed_keys) do
            if v then
                self:key_press_update(k, dt)
            end
        end
        for k, v in pairs(self.held_keys) do
            if v then
                self:key_hold_update(k, dt)
            end
        end
        for k, v in pairs(self.released_keys) do
            if v then
                self:key_release_update(k, dt)
            end
        end
        for k, v in pairs(self.pressed_buttons) do
            if v then
                self:button_press_update(k, dt)
            end
        end
        for k, v in pairs(self.held_buttons) do
            if v then
                self:button_hold_update(k, dt)
            end
        end
        for k, v in pairs(self.released_buttons) do
            if v then
                self:button_release_update(k, dt)
            end
        end
    end
    self.frame_buttonpress = false
    self.pressed_keys = EMPTY(_G, self.pressed_keys)
    self.released_keys = EMPTY(_G, self.released_keys)
    self.pressed_buttons = EMPTY(_G, self.pressed_buttons)
    self.released_buttons = EMPTY(_G, self.released_buttons)
    ...
    if not self.cursor_down.handled then
        local ____opt_1 = self.cursor_down.target
        if ____opt_1 and ____opt_1.states.drag.can then
    ...
    if not self.cursor_up.handled then
        ...
        if self.cursor_down.target then
            local ____opt_3 = self.cursor_down.target
            if not (____opt_3 and ____opt_3.click_timeout) or self.cursor_down.target.click_timeout * G.SPEEDFACTOR > self.cursor_up.time - self.cursor_down.time then
                if Vector_Dist(_G, self.cursor_down.T, self.cursor_up.T) < G.MIN_CLICK_DIST then
    ...
    if not self.clicked.handled then
        local ____opt_5 = self.clicked.target
    ...
    if not self.released_on.handled and self.dragging.prev_target then
        ...
        local ____opt_7 = self.released_on.target
        if ____opt_7 ~= nil then
            ____opt_7:release(self.dragging.prev_target)
        end
...
function Controller.prototype.cull_registry(self)
    for k, registry in pairs(self.button_registry) do
        do
            local i = registry.length - 1
            while i >= 0 do
...
function Controller.prototype.add_to_registry(self, node, registry)
...
function Controller.prototype.process_registry(self)
    for _, registry in pairs(self.button_registry) do
        do
            local i = 0
            while i < registry.length do
                if registry[i].click and registry[i].node.click then
                    if registry[i].menu == not not G.OVERLAY_MENU and registry[i].node.T.x > -2 and registry[i].node.T.x < G.ROOM.T.w + 2 and registry[i].node.T.y > -2 and registry[i].node.T.y < G.ROOM.T.h + 2 then
...
function Controller.prototype.mod_cursor_context_layer(self, delta)
    if delta == 1 then
...
function Controller.prototype.snap_to(self, args)
    self.snap_cursor_to = {node = args.node, T = args.T, type = args.node and "node" or "transform"}
end
function Controller.prototype.save_cardarea_focus(self, _cardarea)
    if G[_cardarea] then
...
function Controller.prototype.recall_cardarea_focus(self, _cardarea)
    local ca_string = nil
    if type(_cardarea) == "string" then
        ca_string = _cardarea
        _cardarea = G[_cardarea]
    end
    if _cardarea and (not self.focused.target or self.interrupt.focus or not self.interrupt.focus and self.focused.target.area and self.focused.target.area == _cardarea) then
        if ca_string and self.cardarea_context[ca_string] then
            do
                local i = self.cardarea_context[ca_string]
                while i >= 0 do
    ...
    if ca_string then
        self.cardarea_context[ca_string] = nil
    end
end
function Controller.prototype.update_cursor(self, hard_set_T)
    if hard_set_T then
    ...
    if self.focused.target then
        local ____temp_9 = self.focused.target:put_focused_cursor()
        self.cursor_position.x = ____temp_9[1]
        self.cursor_position.y = ____temp_9[2]
        G.CURSOR.T.x = self.cursor_position.x / (G.TILESCALE * G.TILESIZE)
        G.CURSOR.T.y = self.cursor_position.y / (G.TILESCALE * G.TILESIZE)
        G.CURSOR.VT.x = G.CURSOR.T.x
        G.CURSOR.VT.y = G.CURSOR.T.y
...
function Controller.prototype.handle_axis_buttons(self)
...
function Controller.prototype.update_axis(self, dt)
    ...
    if self.HID.controller then
        ...
        if self.dragging.target and math.abs(l_stick_x) + math.abs(l_stick_y) > 0.1 then
            ...
            if math.abs(l_stick_x) < 0.1 then
                l_stick_x = 0
            end
            if math.abs(l_stick_y) < 0.1 then
                l_stick_y = 0
            end
            ...
            if math.abs(l_stick_x) + math.abs(l_stick_y) > 0.5 then
                ...
                self.axis_buttons.l_stick.current = math.abs(l_stick_x) > math.abs(l_stick_y) and (l_stick_x > 0 and "dpright" or "dpleft") or (l_stick_y > 0 and "dpdown" or "dpup")
        ...
        if mag > G.DEADZONE then
            axis_interpretation = "axis_cursor"
            if math.abs(r_stick_x) < G.DEADZONE then
                r_stick_x = 0
            end
            if math.abs(r_stick_y) < G.DEADZONE then
                r_stick_y = 0
            end
    ...
    if axis_interpretation then
        self.interrupt.focus = false
    end
    return axis_interpretation
end
function Controller.prototype.button_press_update(self, button, dt)
    ...
    if self.button_registry[button] and self.button_registry[button][1] and not self.button_registry[button][1].node.under_overlay then
        ...
        if button == "a" then
            if self.focused.target and self.focused.target.config.focus_args and self.focused.target.config.focus_args.type == "slider" and (not G.CONTROLLER.HID.mouse and not G.CONTROLLER.HID.axis_cursor) then
            else
                self:L_cursor_press()
            end
        end
...
function Controller.prototype.button_hold_update(self, button, dt)
    if self.locked and not G.SETTINGS.paused or self.locks.frame or self.frame_buttonpress then
        return
    end
    self.frame_buttonpress = true
...
function Controller.prototype.button_release_update(self, button, dt)
    if not self.held_button_times[button] then
        return
    end
...
function Controller.prototype.key_press_update(self, key, dt)
    ...
    if self.text_input_hook then
        ...
        elseif key == "capslock" then
            ...
            G.FUNCS:text_input_key({e = self.text_input_hook, key = key, caps = self.held_keys.lshift or self.held_keys.rshift})
        end
        return
    end
    if key == "escape" then
    ...
    if not _RELEASE_MODE then
        if key == "tab" and not G.debug_tools then
            G.debug_tools = UIBox(
                _G,
                {
                    definition = create_UIBox_debug_tools(_G),
        ...
        if self.hovering.target and self.hovering.target:is(Card) then
            ...
            if G.OVERLAY_MENU then
                if key == "1" then
                    unlock_card(_G, _card.config.center)
                    _card:set_sprites(_card.config.center)
                end
                if key == "2" then
                    unlock_card(_G, _card.config.center)
                    discover_card(_G, _card.config.center)
                    _card:set_sprites(_card.config.center)
                end
                if key == "3" then
                    if _card.ability.set == "Joker" and G.jokers and G.jokers.cards.length < G.jokers.config.card_limit then
                        add_joker(_G, _card.config.center.key)
                        _card:set_sprites(_card.config.center)
                    end
                    if _card.ability.consumeable and G.consumeables and G.consumeables.cards.length < G.consumeables.config.card_limit then
                        add_joker(_G, _card.config.center.key)
                        _card:set_sprites(_card.config.center)
                    end
                end
            ...
            if key == "q" then
                if _card.ability.set == "Joker" or _card.playing_card or _card.area then
                    ...
                    local _edition = G.P_CENTER_POOLS.Edition[found_index + 1].key
        ...
        if key == "v" then
            if not G.prof then
                G.prof = require("engine.engine.profile")
                G.prof:start()
            else
                G.prof:stop()
                print(G.prof:report())
...
function Controller.prototype.key_hold_update(self, key, dt)
    if self.locked and not G.SETTINGS.paused or self.locks.frame or self.frame_buttonpress then
        return
    end
    if self.held_key_times[key] then
        ...
        if key == "r" and not G.SETTINGS.paused then
            if self.held_key_times[key] > 0.7 then
                ...
                local ____temp_10 = {nil, nil}
                G.forced_seed = ____temp_10[1]
                G.setup_seed = ____temp_10[2]
...
function Controller.prototype.key_release_update(self, key, dt)
    for _, keybind in pairs(SMODS.Keybinds) do
...
function Controller.prototype.key_press(self, key)
    self.pressed_keys[key] = true
    self.held_keys[key] = true
end
function Controller.prototype.key_release(self, key)
    self.held_keys[key] = nil
    self.released_keys[key] = true
end
function Controller.prototype.button_press(self, button)
    self.pressed_buttons[button] = true
    self.held_buttons[button] = true
end
function Controller.prototype.button_release(self, button)
    self.held_buttons[button] = nil
    self.released_buttons[button] = true
end
function Controller.prototype.get_cursor_collision(self, cursor_trans)
    self.collision_list = EMPTY(_G, self.collision_list)
    self.nodes_at_cursor = EMPTY(_G, self.nodes_at_cursor)
    if self.COYOTE_FOCUS then
        return
    end
    if self.dragging.target then
        self.dragging.target.states.collide.is = true
        self.nodes_at_cursor[#self.nodes_at_cursor + 1 + 1] = self.dragging.target
        self.collision_list[#self.collision_list + 1 + 1] = self.dragging.target
    end
    if not G.DRAW_HASH[2] or cursor_trans.x - G.ROOM.T.x < -G.DRAW_HASH_BUFF or cursor_trans.x - G.ROOM.T.x > G.TILE_W + G.DRAW_HASH_BUFF or cursor_trans.y - G.ROOM.T.y < -G.DRAW_HASH_BUFF or cursor_trans.y - G.ROOM.T.y > G.TILE_H + G.DRAW_HASH_BUFF then
        return
    end
    local DRAW_HASH_SQUARE = G.DRAW_HASH
    do
        local i = #DRAW_HASH_SQUARE
        while i <= 1 do
            local v = DRAW_HASH_SQUARE[i + 1]
            if v:collides_with_point(cursor_trans) and not v.REMOVED then
                self.nodes_at_cursor[#self.nodes_at_cursor + 1 + 1] = v
                if v.states.collide.can then
                    v.states.collide.is = true
                    self.collision_list[#self.collision_list + 1 + 1] = v
                end
            end
            i = i + -1
        end
...
function Controller.prototype.set_cursor_hover(self)
    self.cursor_hover.T = self.cursor_hover.T or ({})
    local ____temp_11 = {G.CURSOR.T.x, G.CURSOR.T.y}
    self.cursor_hover.T.x = ____temp_11[1]
    self.cursor_hover.T.y = ____temp_11[2]
    self.cursor_hover.time = G.TIMERS.TOTAL
    self.cursor_hover.prev_target = self.cursor_hover.target
    self.cursor_hover.target = nil
    if self.interrupt.focus or self.locked and (not G.SETTINGS.paused or G.screenwipe) or self.locks.frame or self.COYOTE_FOCUS then
    ...
    if self.cursor_hover.target ~= self.cursor_hover.prev_target then
        self.cursor_hover.handled = false
    end
end
function Controller.prototype.queue_L_cursor_press(self, x, y)
    if self.locks.frame then
        return
    end
    if G.STATE == G.STATES.SPLASH then
...
function Controller.prototype.queue_R_cursor_press(self, x, y)
    if self.locks.frame then
        return
    end
    if not G.SETTINGS.paused and G.hand and G.hand.highlighted[1] then
        if G.play and G.play.cards.length > 0 or self.locked or self.locks.frame or G.GAME.STOP_USE and G.GAME.STOP_USE > 0 then
            return
        end
        G.hand:unhighlight_all()
    end
end
function Controller.prototype.L_cursor_press(self, x, y)
    x = x or self.cursor_position.x
    y = y or self.cursor_position.y
    if self.locked and (not G.SETTINGS.paused or G.screenwipe) or self.locks.frame then
        return
    end
...
function Controller.prototype.L_cursor_release(self, x, y)
    x = x or self.cursor_position.x
    y = y or self.cursor_position.y
    if self.locked and (not G.SETTINGS.paused or G.screenwipe) or self.locks.frame then
        return
...
function Controller.prototype.is_node_focusable(self, node)
    local ret_val = false
    if node.T.y > G.ROOM.T.h + 3 then
        return false
    end
    if not node.REMOVED and not node.under_overlay and (node.states.hover.can and not self.dragging.target or self.dragging.target == node) and not not node.created_on_pause == not not G.SETTINGS.paused and node.states.visible and (not node.UIBox or node.UIBox.states.visible) then
        if self.screen_keyboard then
            if node.UIBox and node.UIBox == self.screen_keyboard and node.config.button then
                ret_val = true
            end
        else
            if node:is(Card) and (node.facing == "front" or node.area == G.hand or node.area == G.jokers or node == G.deck) and node.states.hover.can and not node.jimbo then
                ret_val = true
            end
            if node.config and node.config.force_focus then
                ret_val = true
            end
...
function Controller.prototype.update_focus(self, dir)
    self.focused.prev_target = self.focused.target
    ...
    G.ARGS.focus_list = EMPTY(_G, G.ARGS.focus_list)
    G.ARGS.focusables = EMPTY(_G, G.ARGS.focusables)
    if self.focused.target then
        self.focused.target.states.focus.is = false
    ...
    if not dir and self.focused.target then
        ...
        G.ARGS.focusables[G.ARGS.focusables.length + 1] = self.focused.target
    else
        if not dir then
            for k, v in ipairs(self.nodes_at_cursor) do
                v.states.focus.can = false
                v.states.focus.is = false
                if G.ARGS.focusables.length == 0 and self:is_node_focusable(v) then
    ...
    if G.ARGS.focusables.length > 0 then
        if dir then
            if (dir == "L" or dir == "R") and self.focused.target and self.focused.target:is(Card) and self.focused.target.area == G.hand and G.hand then
                ...
                if nu_rank > G.hand.cards.length then
                    nu_rank = 1
                end
                if nu_rank == 0 then
                ...
                local ____temp_12 = {G.CURSOR.T.x - G.ROOM.T.x, G.CURSOR.T.y - G.ROOM.T.y}
                G.ARGS.focus_cursor_pos.x = ____temp_12[1]
                G.ARGS.focus_cursor_pos.y = ____temp_12[2]
                if self.focused.target then
                    ...
                    local ____temp_13 = {_ft.T.x + 0.5 * _ft.T.w, _ft.T.y + 0.5 * _ft.T.h}
                    G.ARGS.focus_cursor_pos.x = ____temp_13[1]
                    G.ARGS.focus_cursor_pos.y = ____temp_13[2]
                elseif self.hovering.target and self.hovering.target.states.focus.can then
                    local ____temp_14 = self.hovering.target:put_focused_cursor()
                    G.ARGS.focus_cursor_pos.x = ____temp_14[1]
                    G.ARGS.focus_cursor_pos.y = ____temp_14[2]
                    ...
                    if v ~= self.hovering.target and v ~= self.focused.target then
                        ...
                        if v.config.focus_args and v.config.focus_args.nav then
                            if v.config.focus_args.nav == "wide" then
                                if G.ARGS.focus_vec.y > 0.1 and dir == "D" then
                                    eligible = true
                                elseif G.ARGS.focus_vec.y < -0.1 and dir == "U" then
                                    eligible = true
                                elseif math.abs(G.ARGS.focus_vec.y) < v.T.h / 2 then
                                    eligible = true
                                end
                            elseif v.config.focus_args.nav == "tall" then
                                if G.ARGS.focus_vec.x > 0.1 and dir == "R" then
                                    eligible = true
                                elseif G.ARGS.focus_vec.x < -0.1 and dir == "L" then
                                    eligible = true
                                elseif math.abs(G.ARGS.focus_vec.x) < v.T.w / 2 then
                                    eligible = true
                                end
                            end
                        elseif math.abs(G.ARGS.focus_vec.x) > math.abs(G.ARGS.focus_vec.y) then
                            if G.ARGS.focus_vec.x > 0 and dir == "R" then
                                eligible = true
                            elseif G.ARGS.focus_vec.x < 0 and dir == "L" then
                                eligible = true
                            end
                        else
                            if G.ARGS.focus_vec.y > 0 and dir == "D" then
                                eligible = true
                            elseif G.ARGS.focus_vec.y < 0 and dir == "U" then
                                eligible = true
                            end
                        end
                        if eligible then
                ...
                if G.ARGS.focus_list.length < 1 then
                    if self.focused.target then
            ...
            if self.focused.target then
                G.ARGS.focus_list[G.ARGS.focus_list.length + 1] = {node = self.focused.target, dist = 0}
            else
...
function Controller.prototype.capture_focused_input(self, button, input_type, dt)
    local ret = false
    local focused = self.focused.target
    local extern_button = false
    self.no_holdcap = nil
    if input_type == "press" and (button == "dpleft" or button == "dpright") and focused and self.dragging.target and (self.held_button_times.a and self.held_button_times.a < 0.12) and focused.area and focused.area:can_highlight(focused) then
        self:L_cursor_release()
    ...
    elseif input_type == "press" and focused and focused.area and focused == self.dragging.target then
        ...
        if button == "dpleft" and focused.rank > 1 then
            ...
            self:update_cursor()
        elseif button == "dpright" and focused.rank < focused.area.cards.length then
            focused.rank = focused.rank + 1
            focused.area.cards[focused.rank].rank = focused.rank - 1
            table.sort(
                focused.area.cards,
        ...
        if focused.config.focus_args.type == "tab" and input_type == "press" then
            ...
            for k, v in ipairs(choices) do
                if v.config.chosen then
                    if extern_button and button == "leftshoulder" or not extern_button and button == "dpleft" then
                        local next_i = k ~= 1 and k - 1 or choices.length
                    ...
                    elseif extern_button and button == "rightshoulder" or not extern_button and button == "dpright" then
                        local next_i = k ~= choices.length and k + 1 or 1
                        if focused.config.focus_args.no_loop and next_i < k then
                            ret = nil
                        else
                            choices[next_i]:click()
        ...
        if focused.config.focus_args.type == "slider" then
            if button == "dpleft" then
                ...
                if input_type == "hold" and self.held_button_times[button] > 0.2 then
                    G.FUNCS:slider_descreet(focused.children[2], -dt * self.held_button_times[button] * 0.6)
                end
                if input_type == "press" then
                    G.FUNCS:slider_descreet(focused.children[2], -0.01)
                end
                ret = true
            end
            if button == "dpright" then
...
function Controller.prototype.navigate_focus(self, dir)
