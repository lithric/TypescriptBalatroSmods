...
class UIBox extends Moveable {
    ...
    constructor(args) {
        ...
        if (args.config) {
            ...
            this.set_alignment({ major: args.config.major, type: args.config.align || args.config.type || "", bond: args.config.bond || "Strong", offset: args.config.offset || { x: 0, y: 0 }, lr_clamp: args.config.lr_clamp });
            this.set_role({ xy_bond: args.config.xy_bond, r_bond: args.config.r_bond, wh_bond: args.config.wh_bond || "Weak", scale_bond: args.config.scale_bond || "Weak" });
    ...
    calculate_xywh(node, _T, recalculate, _scale) {
        ...
        if (node.UIT === G.UIT.B || node.UIT === G.UIT.T || node.UIT === G.UIT.O) {
            [_nt.x, _nt.y, _nt.w, _nt.h] = [_T.x, _T.y, node.config.w || node.config.object && node.config.object.T.w, node.config.h || node.config.object && node.config.object.T.h];
            if (node.UIT === G.UIT.T) {
                ...
                let tx = node.config.lang.font.FONT.getWidth(node.config.text) * node.config.lang.font.squish * scale * G.TILESCALE * node.config.lang.font.FONTSCALE;
                let ty = node.config.lang.font.FONT.getHeight() * scale * G.TILESCALE * node.config.lang.font.FONTSCALE * node.config.lang.font.TEXT_HEIGHT_SCALE;
                if (node.config.vert) {
                    let thunk = tx;
                    tx = ty;
                    ty = thunk;
            ...
            for (let i = 1; i <= 2; i++) {
                if (i === 1 || i === 2 && (node.config.maxw && _ct.w > node.config.maxw || node.config.maxh && _ct.h > node.config.maxh)) {
                    let fac = _scale || 1;
                    if (i === 2) {
    ...
    set_parent_child(node, parent) {
        ...
        if ((node.n && node.n === G.UIT.C || node.n === G.UIT.R || node.n === G.UIT.ROOT) && node.nodes) {
    ...
    recalculate() {
        ...
        G.REFRESH_FRAME_MAJOR_CACHE = G.REFRESH_FRAME_MAJOR_CACHE > 1 && G.REFRESH_FRAME_MAJOR_CACHE - 1 || undefined;
    };
    move(dt) {
...
class UIElement extends Moveable {
    ...
    set_values(_T, recalculate) {
        ...
        this.set_role({ role_type: "Minor", major: this.UIBox, offset: { x: _T.x, y: _T.y }, wh_bond: "Weak", scale_bond: "Weak" });
        ...
        if (this.UIT === G.UIT.O && !this.config.no_role) {
            this.config.object.set_role(this.config.role || { role_type: "Minor", major: this, xy_bond: "Strong", wh_bond: "Weak", scale_bond: "Weak" });
        }
        ...
        if (this.config && this.config.func && ((this.config.button_UIE || this.config.button) && this.config.func !== "set_button_pip" || this.config.insta_func)) {
    ...
    print_topology(indent) {
        ...
        let box_str = "\\n" + (string.rep("  ", indent) + ("| " + (UIT + (" | - ID:" + (this.ID + (" w/h:" + (this.T.w + ("/" + this.T.h))))))));
        if (UIT === "O") {
            box_str = box_str + (" OBJ:" + (getmetatable(this.config.object) === CardArea && "CardArea" || getmetatable(this.config.object) === Card && "Card" || getmetatable(this.config.object) === UIBox && "UIBox" || getmetatable(this.config.object) === Particles && "Particles" || getmetatable(this.config.object) === DynaText && "DynaText" || getmetatable(this.config.object) === Sprite && "Sprite" || getmetatable(this.config.object) === AnimatedSprite && "AnimatedSprite" || "OTHER"));
        }
        else if (UIT === "T") {
    ...
    update_text() {
        if (this.config && this.config.text && !this.config.text_drawable) {
            ...
            this.config.text_drawable = love.graphics.newText(this.config.lang.font.FONT, [G.C.WHITE, this.config.text]);
        }
        if (this.config.ref_table && this.config.ref_table[this.config.ref_value] !== this.config.prev_value) {
            this.config.text = tostring(this.config.ref_table[this.config.ref_value]);
            this.config.text_drawable.set(this.config.text);
            if (!this.config.no_recalc && this.config.prev_value && string.len(this.config.prev_value) !== string.len(this.config.text)) {
    ...
    update_object() {
        if (this.config.ref_table && this.config.ref_value && this.config.ref_table[this.config.ref_value] !== this.config.object) {
        ...
        if (this.config.object && this.config.object.ui_object_updated) {
            ...
            if (this.config.object.non_recalc) {
                ...
                this.align(this.parent.T.x - this.config.object.T.x, this.parent.T.y - this.config.object.T.y);
    ...
    draw_this() {
        ...
        if (this.config.force_focus || this.config.force_collision || this.config.button_UIE || this.config.button || this.states.collide.can) {
            add_to_drawhash(this);
        }
        ...
        if (this.config.button || this.config.button_UIE) {
            this.layered_parallax.x = (this.parent && this.parent !== this.UIBox && this.parent.layered_parallax.x || 0) + (this.config.shadow && 0.4 * this.shadow_parrallax.x || 0) / G.TILESIZE;
            this.layered_parallax.y = (this.parent && this.parent !== this.UIBox && this.parent.layered_parallax.y || 0) + (this.config.shadow && 0.4 * this.shadow_parrallax.y || 0) / G.TILESIZE;
            if (this.config.button && (this.last_clicked && this.last_clicked > G.TIMERS.REAL - 0.1 || this.config.button && (this.states.hover.is || this.states.drag.is) && G.CONTROLLER.is_cursor_down)) {
                this.layered_parallax.x = this.layered_parallax.x - parallax_dist * this.shadow_parrallax.x / G.TILESIZE * (this.config.button_dist || 1);
                this.layered_parallax.y = this.layered_parallax.y - parallax_dist * this.shadow_parrallax.y / G.TILESIZE * (this.config.button_dist || 1);
                parallax_dist = 0;
        ...
        if (this.config.colour[4] > 0.01) {
            if (this.UIT === G.UIT.T && this.config.scale) {
                ...
                this.ARGS.text_parallax.sx = -this.shadow_parrallax.x * 0.5 / (this.config.scale * this.config.lang.font.FONTSCALE);
                this.ARGS.text_parallax.sy = -this.shadow_parrallax.y * 0.5 / (this.config.scale * this.config.lang.font.FONTSCALE);
                if (this.config.button_UIE && button_active || !this.config.button_UIE && this.config.shadow && G.SETTINGS.GRAPHICS.shadows === "On") {
                    prep_draw(this, 0.97);
                    if (this.config.vert) {
                        love.graphics.translate(0, this.VT.h);
                        love.graphics.rotate(-math.pi / 2);
                    }
                    if ((this.config.shadow || this.config.button_UIE && button_active) && G.SETTINGS.GRAPHICS.shadows === "On") {
                        love.graphics.setColor(0, 0, 0, 0.3 * this.config.colour[4]);
                        love.graphics.draw(this.config.text_drawable, (this.config.lang.font.TEXT_OFFSET.x + (this.config.vert && -this.ARGS.text_parallax.sy || this.ARGS.text_parallax.sx)) * (this.config.scale || 1) * this.config.lang.font.FONTSCALE / G.TILESIZE, (this.config.lang.font.TEXT_OFFSET.y + (this.config.vert && this.ARGS.text_parallax.sx || this.ARGS.text_parallax.sy)) * (this.config.scale || 1) * this.config.lang.font.FONTSCALE / G.TILESIZE, 0, this.config.scale * this.config.lang.font.squish * this.config.lang.font.FONTSCALE / G.TILESIZE, this.config.scale * this.config.lang.font.FONTSCALE / G.TILESIZE);
                    }
                    love.graphics.pop();
                }
                prep_draw(this, 1);
                ...
                love.graphics.draw(this.config.text_drawable, this.config.lang.font.TEXT_OFFSET.x * this.config.scale * this.config.lang.font.FONTSCALE / G.TILESIZE, this.config.lang.font.TEXT_OFFSET.y * this.config.scale * this.config.lang.font.FONTSCALE / G.TILESIZE, 0, this.config.scale * this.config.lang.font.squish * this.config.lang.font.FONTSCALE / G.TILESIZE, this.config.scale * this.config.lang.font.FONTSCALE / G.TILESIZE);
                love.graphics.pop();
            }
            else if (this.UIT === G.UIT.B || this.UIT === G.UIT.C || this.UIT === G.UIT.R || this.UIT === G.UIT.ROOT) {
                prep_draw(this, 1);
                ...
                if (this.config.shadow && G.SETTINGS.GRAPHICS.shadows === "On") {
                    ...
                    if (this.config.r && this.VT.w > 0.01) {
                        ...
                        love.graphics.rectangle("fill", -this.shadow_parrallax.x * parallax_dist, -this.shadow_parrallax.y * parallax_dist, this.VT.w * G.TILESIZE, this.VT.h * G.TILESIZE);
                    }
                    love.graphics.scale(1 / 0.98);
                }
                love.graphics.scale(button_being_pressed && 0.985 || 1);
                if (this.config.emboss) {
                    love.graphics.setColor(darken(this.config.colour, this.states.hover.is && 0.5 || 0.3, true));
                ...
                this.ARGS.button_colours[1] = this.config.button_delay && mix_colours(this.config.colour, G.C.L_BLACK, 0.5) || this.config.colour;
                this.ARGS.button_colours[2] = (collided_button.config.hover && collided_button.states.hover.is || collided_button.last_clicked && collided_button.last_clicked > G.TIMERS.REAL - 0.1) && G.C.UI.HOVER || undefined;
                for (const [k, v] of ipairs(this.ARGS.button_colours)) {
                    ...
                    if (this.config.r && this.VT.w > 0.01) {
                        if (this.config.button_delay) {
                            ...
                            love.graphics.setColor(v);
                            this.draw_pixellated_rect("fill", parallax_dist, undefined, this.config.button_delay_progress);
                        }
                            ...
                            this.draw_pixellated_rect("fill", parallax_dist, undefined, this.config.progress_bar.ref_table[this.config.progress_bar.ref_value] / this.config.progress_bar.max);
                        }
                        else {
                            this.draw_pixellated_rect("fill", parallax_dist);
                        }
                ...
                if (this.config.focus_with_object && this.config.object.states.focus.is) {
                    ...
                    this.draw_pixellated_rect("fill", parallax_dist);
                    love.graphics.setColor(this.config.colour[4] > 0 && mix_colours(G.C.WHITE, this.config.colour, 0.8) || G.C.WHITE);
        ...
        if (this.config.outline && this.config.outline_colour[4] > 0.01) {
            if (this.config.outline) {
                ...
                if (this.config.line_emboss) {
                    love.graphics.setColor(darken(this.config.outline_colour, this.states.hover.is && 0.5 || 0.3, true));
        ...
        if (this.config.chosen) {
            ...
            if (this.config.shadow && G.SETTINGS.GRAPHICS.shadows === "On") {
                love.graphics.setColor(0, 0, 0, 0.3 * this.config.colour[4]);
                love.graphics.polygon("fill", get_chosen_triangle_from_rect(this.layered_parallax.x - this.shadow_parrallax.x * parallax_dist * 0.5, this.layered_parallax.y - this.shadow_parrallax.y * parallax_dist * 0.5, this.VT.w * G.TILESIZE, this.VT.h * G.TILESIZE, this.config.chosen === "vert"));
            }
            love.graphics.pop();
            prep_draw(this, 1);
            love.graphics.scale(1 / G.TILESIZE);
            ...
            love.graphics.setColor(this.config.colour);
            love.graphics.polygon("fill", get_chosen_triangle_from_rect(this.layered_parallax.x, this.layered_parallax.y, this.VT.w * G.TILESIZE, this.VT.h * G.TILESIZE, this.config.chosen === "vert"));
            love.graphics.pop();
        }
        this.draw_boundingrect();
    };
    draw_pixellated_rect(_type, _parallax, _emboss, _progress) {
        if (!this.pixellated_rect || this.pixellated_rect[_type].vertices.length < 1 || _parallax !== this.pixellated_rect.parallax || this.pixellated_rect.w !== this.VT.w || this.pixellated_rect.h !== this.VT.h || this.pixellated_rect.sw !== this.shadow_parrallax.x || this.pixellated_rect.sh !== this.shadow_parrallax.y || this.pixellated_rect.progress !== (_progress || 1)) {
            this.pixellated_rect = { w: this.VT.w, h: this.VT.h, sw: this.shadow_parrallax.x, sh: this.shadow_parrallax.y, progress: _progress || 1, fill: { vertices: {} }, shadow: { vertices: {} }, line: { vertices: {} }, emboss: { vertices: {} }, line_emboss: { vertices: {} }, parallax: _parallax };
            let ext_up = this.config.ext_up && this.config.ext_up * G.TILESIZE || 0;
            let res = this.config.res || math.min(this.VT.w, this.VT.h + math.abs(ext_up) / G.TILESIZE) > 3.5 && 0.8 || math.min(this.VT.w, this.VT.h + math.abs(ext_up) / G.TILESIZE) > 0.3 && 0.6 || 0.15;
            let [totw, toth, subw, subh] = [this.VT.w * G.TILESIZE, (this.VT.h + math.abs(ext_up) / G.TILESIZE) * G.TILESIZE, this.VT.w * G.TILESIZE - 4 * res, (this.VT.h + math.abs(ext_up) / G.TILESIZE) * G.TILESIZE - 4 * res];
            let vertices = [subw / 2, subh / 2 - ext_up, 0, 4 * res - ext_up, 1 * res, 4 * res - ext_up, 1 * res, 2 * res - ext_up, 2 * res, 2 * res - ext_up, 2 * res, 1 * res - ext_up, 4 * res, 1 * res - ext_up, 4 * res, 0 * res - ext_up, subw, 0 * res - ext_up, subw, 1 * res - ext_up, subw + 2 * res, 1 * res - ext_up, subw + 2 * res, 2 * res - ext_up, subw + 3 * res, 2 * res - ext_up, subw + 3 * res, 4 * res - ext_up, totw, 4 * res - ext_up, totw, subh - ext_up, subw + 3 * res, subh - ext_up, subw + 3 * res, subh + 2 * res - ext_up, subw + 2 * res, subh + 2 * res - ext_up, subw + 2 * res, subh + 3 * res - ext_up, subw, subh + 3 * res - ext_up, subw, toth - ext_up, 4 * res, toth - ext_up, 4 * res, subh + 3 * res - ext_up, 2 * res, subh + 3 * res - ext_up, 2 * res, subh + 2 * res - ext_up, 1 * res, subh + 2 * res - ext_up, 1 * res, subh - ext_up, 0, subh - ext_up, 0, 4 * res - ext_up];
            for (const [k, v] of ipairs(vertices)) {
                ...
                if (k > 4) {
                    ...
                    if (_emboss) {
                        this.pixellated_rect.line_emboss.vertices[k - 4] = v + (k % 2 === 0 && -_emboss * this.shadow_parrallax.y || -0.7 * _emboss * this.shadow_parrallax.x);
                    }
                }
                if (k % 2 === 0) {
        ...
        love.graphics.polygon((_type === "line" || _type === "line_emboss") && "line" || "fill", this.pixellated_rect[_type].vertices);
    };
    update(dt) {
        ...
        if (this.config.button_delay) {
            ...
            this.config.button_delay_progress = (G.TIMERS.REAL - this.config.button_delay_start) / this.config.button_delay;
    ...
    click() {
        if (this.config.button && (!this.last_clicked || this.last_clicked + 0.1 < G.TIMERS.REAL) && this.states.visible && !this.under_overlay && !this.disable_button) {
    ...
    hover() {
        if (this.config && this.config.on_demand_tooltip) {
            ...
            this.config.h_popup_config = { align: this.T.y > G.ROOM.T.h / 2 && "tm" || "bm", offset: { x: 0, y: this.T.y > G.ROOM.T.h / 2 && -0.1 || 0.1 }, parent: this };
        }
