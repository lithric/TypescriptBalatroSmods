...
class AnimatedSprite extends Sprite {
  ...
  set_sprite_pos(sprite_pos) {
    this.animation = {
      x: (sprite_pos && sprite_pos.x) || 0,
      y: (sprite_pos && sprite_pos.y) || 0,
      frames: this.atlas.frames,
      current: 0,
      w: this.scale.x,
      h: this.scale.y,
    };
    ...
    this.sprite = love.graphics.newQuad(
      0,
      this.animation.h * this.animation.y,
      this.animation.w,
      this.animation.h,
      this.image_dims[0],
      this.image_dims[1]
  ...
  animate() {
    let new_frame =
      math.floor(G.ANIMATION_FPS * (G.TIMERS.REAL - this.offset_seconds)) %
    ...
    if (this.float) {
      ...
      this.offset.y =
        -(1 + 0.3 * math.sin(0.666 * G.TIMERS.REAL + this.T.y)) * this.shadow_parrallax.y;
      this.offset.x =
        -(0.7 + 0.2 * math.sin(0.666 * G.TIMERS.REAL + this.T.x)) * this.shadow_parrallax.x;
    }
  }
  remove() {
