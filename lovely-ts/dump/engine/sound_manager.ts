    ...
    if (extension === ".ogg") {
      ...
      let s: SoundHandler = {
        sound: love.audio.newSource(
          ...
          (string.find(sound_code, "music") && "stream") || "static"
        ),
...
function MODULATE(args: SoundOptions): void {
  ...
  for (const [k, v] of pairs(SOURCES)) {
    ...
    for (const [_, s] of pairs(v)) {
      if (
        (s as SoundHandler).sound &&
        (s as SoundHandler).sound.isPlaying() &&
        (s as SoundHandler).original_volume
      ) {
