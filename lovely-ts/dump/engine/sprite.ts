...
class Sprite extends Moveable {
  ...
  constructor(
    ...
    W: number,
    H: number,
    new_sprite_atlas: any,
    sprite_pos: Position2D
  ) {
    super(X, Y, W, H);
    this.CT = this.VT;
  ...
  set_sprite_pos(sprite_pos: { v?: number; y: number; x: number }) {
    ...
    this.sprite = love.graphics.newQuad(
      this.sprite_pos.x * this.atlas.px,
      this.sprite_pos.y * this.atlas.py,
      this.scale.x,
      this.scale.y,
      this.atlas.image.getDimensions()
    );
    this.image_dims = {};
  ...
  define_draw_steps(draw_step_definitions: DrawStepDefinition[]) {
    ...
    for (const [k, v] of ipairs(draw_step_definitions)) {
      this.draw_steps[this.draw_steps.length + 1] = {
        shader: v.shader || "dissolve",
        shadow_height: v.shadow_height || undefined,
        send: v.send || undefined,
        no_tilt: v.no_tilt || undefined,
        other_obj: v.other_obj || undefined,
        ms: v.ms || undefined,
        mr: v.mr || undefined,
        mx: v.mx || undefined,
        my: v.my || undefined,
  ...
  draw_shader(
      ...
      if (_send) {
        for (const [k, v] of ipairs(_send)) {
          G.SHADERS[_shader].send(
            v.name,
            v.val || (v.func && v.func()) || v.ref_table[v.ref_value]
          );
        }
      }
    }
    ...
    if (!custom_shader && _shader !== "vortex") {
      ...
      this.ARGS.prep_shader.cursor_pos[1] =
        (_draw_major.tilt_var && _draw_major.tilt_var.mx * G.CANV_SCALE) ||
        G.CONTROLLER.cursor_position.x * G.CANV_SCALE;
      this.ARGS.prep_shader.cursor_pos[2] =
        (_draw_major.tilt_var && _draw_major.tilt_var.my * G.CANV_SCALE) ||
        G.CONTROLLER.cursor_position.y * G.CANV_SCALE;
      G.SHADERS[_shader || "dissolve"].send("mouse_screen_pos", this.ARGS.prep_shader.cursor_pos);
      G.SHADERS[_shader || "dissolve"].send(
        "screen_scale",
        G.TILESCALE * G.TILESIZE * (_draw_major.mouse_damping || 1) * G.CANV_SCALE
      );
      G.SHADERS[_shader || "dissolve"].send(
        "hovering",
        (((_shadow_height && !tilt_shadow) || _no_tilt) && 0) ||
          (_draw_major.hover_tilt || 0) * (tilt_shadow || 1)
      );
      G.SHADERS[_shader || "dissolve"].send("dissolve", Math.abs(_draw_major.dissolve || 0));
      G.SHADERS[_shader || "dissolve"].send(
        "time",
        (123.33412 * (_draw_major.ID / 1.14212 || 12.5123152)) % 3000
      );
      ...
      G.SHADERS[_shader || "dissolve"].send(
        "burn_colour_1",
        (_draw_major.dissolve_colours && _draw_major.dissolve_colours[1]) || G.C.CLEAR
      );
      G.SHADERS[_shader || "dissolve"].send(
        "burn_colour_2",
        (_draw_major.dissolve_colours && _draw_major.dissolve_colours[2]) || G.C.CLEAR
      );
      G.SHADERS[_shader || "dissolve"].send("shadow", !!_shadow_height);
      if (_send) {
        G.SHADERS[_shader || "dissolve"].send(
          (SMODS.Shaders[_shader || "dissolve"] &&
            SMODS.Shaders[_shader || "dissolve"].original_key) ||
            _shader,
          _send
        );
      }
  ...
  draw_this(overlay?: HexArray) {
    ...
    if (
      this.sprite_pos.x !== this.sprite_pos_copy.x ||
      this.sprite_pos.y !== this.sprite_pos_copy.y
    ) {
      this.set_sprite_pos(this.sprite_pos);
    }
    prep_draw(this, 1);
    ...
    if (this.video) {
      ...
      love.graphics.draw(
        ...
        0,
        0,
        this.VT.w / this.T.w / (this.video_dims.w / this.scale.x),
        this.VT.h / this.T.h / (this.video_dims.h / this.scale.y)
      );
    } else {
      love.graphics.draw(
        this.atlas.image,
