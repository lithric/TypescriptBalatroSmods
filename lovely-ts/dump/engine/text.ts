...
class DynaText extends Moveable {
  ...
  constructor(config) {
    ...
    this.string = (config.string && type(config.string) === "table" && config.string[1]) || [
      "HELLO WORLD",
    ];
    this.text_offset = {
      x: this.font.TEXT_OFFSET.x * this.scale + (this.config.x_offset || 0),
      y: this.font.TEXT_OFFSET.y * this.scale + (this.config.y_offset || 0),
    };
  ...
  update_text(first_pass?: any) {
    ...
    for (const [k, v] of ipairs(this.config.string)) {
      if ((type(v) === "table" && v.ref_table) || first_pass) {
        ...
        if (type(v) === "table" && (v.ref_table || v.string)) {
          new_string =
            (v.prefix || "") +
            (format_ui_value((v.ref_table && v.ref_table[v.ref_value]) || v.string) +
              (v.suffix || ""));
        ...
        if (old_string !== new_string || first_pass) {
          ...
          for (const [_, c] of utf8.chars(v)) {
            ...
            let let_tab = {
              letter: love.graphics.newText(this.font.FONT, c),
              char: c,
              scale: (old_letter && old_letter.scale) || part_scale,
            };
            this.strings[k].letters[current_letter] = let_tab;
            let tx =
              this.font.FONT.getWidth(c) *
                this.scale *
                part_scale *
                G.TILESCALE *
                this.font.FONTSCALE +
              2.7 * (this.config.spacing || 0) * G.TILESCALE * this.font.FONTSCALE;
            let ty =
              this.font.FONT.getHeight(c) *
              this.scale *
              part_scale *
              G.TILESCALE *
              this.font.FONTSCALE *
              this.font.TEXT_HEIGHT_SCALE;
            let_tab.offset = (old_letter && old_letter.offset) || { x: 0, y: 0 };
            let_tab.dims = {
              x: tx / (this.font.FONTSCALE * G.TILESCALE),
              y: ty / (this.font.FONTSCALE * G.TILESCALE),
            };
            let_tab.pop_in =
              (first_pass &&
                ((old_letter && old_letter.pop_in) || (this.config.pop_in && 0) || 1)) ||
              1;
    ...
    if (this.T) {
      if (
        (this.T.w !== this.config.W || this.T.h !== this.config.H) &&
        (!first_pass || this.reset_pop_in)
      ) {
  ...
  align_letters() {
    if (this.pop_cycle) {
      this.focused_string =
        (this.config.random_element && math.random(1, this.strings.length)) ||
        (this.focused_string === this.strings.length && 1) ||
        this.focused_string + 1;
    ...
    for (const [k, letter] of ipairs(this.strings[this.focused_string].letters)) {
      if (this.config.pop_out) {
        letter.pop_in = math.min(
          1,
          math.max(
            (this.config.min_cycle_time || 1) -
              ((G.TIMERS.REAL - this.pop_out_time) * this.config.pop_out) /
                (this.config.min_cycle_time || 1),
            0
          )
        );
        ...
        if (
          k === this.strings[this.focused_string].letters.length &&
          letter.pop_in <= 0 &&
        ...
        letter.pop_in = math.min(
          1,
          math.max(
            (G.TIMERS.REAL - this.config.pop_in - this.created_time) *
              this.string.length *
              this.pop_in_rate -
              k +
              1,
            (this.config.min_cycle_time === 0 && 1) || 0
          )
        );
        letter.pop_in = letter.pop_in * letter.pop_in;
        if (
          ...
          letter.pop_in > 0 &&
          !this.silent &&
          (this.string.length < 10 || k % 2 === 0)
        ) {
          if (
            this.T.x > G.ROOM.T.w + 2 ||
            this.T.y > G.ROOM.T.h + 2 ||
            this.T.x < -2 ||
            this.T.y < -2
          ) {
          } else {
            play_sound(
              ...
              0.45 +
                0.05 * math.random() +
                (0.3 / this.string.length) * k +
                (this.config.pitch_shift || 0)
            );
          }
        }
      ...
      if (this.config.rotate) {
        letter.r =
          ((this.config.rotate === 2 && -1) || 1) *
          ((0.2 * (-this.strings[this.focused_string].letters.length / 2 - 0.5 + k)) /
            this.strings[this.focused_string].letters.length +
            ((G.SETTINGS.reduced_motion && 0) || 1) * 0.02 * math.sin(2 * G.TIMERS.REAL + k));
      }
      if (this.config.pulse) {
        letter.scale =
          letter.scale +
          ((G.SETTINGS.reduced_motion && 0) || 1) *
            (1 / this.config.pulse.width) *
            this.config.pulse.amount *
            math.max(
              math.min(
                (this.config.pulse.start - G.TIMERS.REAL) * this.config.pulse.speed +
                  k +
                  this.config.pulse.width,
                (G.TIMERS.REAL - this.config.pulse.start) * this.config.pulse.speed -
                  k +
                  this.config.pulse.width +
                  2
              ),
        ...
        letter.r =
          ...
          ((G.SETTINGS.reduced_motion && 0) || 1) *
            (letter.scale - 1) *
            0.02 *
            (-this.strings[this.focused_string].letters.length / 2 - 0.5 + k);
        if (
          this.config.pulse.start >
          G.TIMERS.REAL +
            2 * this.config.pulse.speed * this.strings[this.focused_string].letters.length
        ) {
      ...
      if (this.config.quiver) {
        letter.scale =
          letter.scale + ((G.SETTINGS.reduced_motion && 0) || 1) * 0.1 * this.config.quiver.amount;
        letter.r =
          letter.r +
          ((G.SETTINGS.reduced_motion && 0) || 1) *
            0.3 *
            this.config.quiver.amount *
            (math.sin(41.12342 * G.TIMERS.REAL * this.config.quiver.speed + k * 1223.2) +
              math.cos(63.21231 * G.TIMERS.REAL * this.config.quiver.speed + k * 1112.2) *
                math.sin(36.1231 * G.TIMERS.REAL * this.config.quiver.speed) +
              math.cos(95.123 * G.TIMERS.REAL * this.config.quiver.speed + k * 1233.2) -
              math.sin(30.133421 * G.TIMERS.REAL * this.config.quiver.speed + k * 123.2));
      }
      if (this.config.float) {
        letter.offset.y =
          ((G.SETTINGS.reduced_motion && 0) || 1) *
            ...
            (2 +
              (this.font.FONTSCALE / G.TILESIZE) *
                2000 *
                math.sin(2.666 * G.TIMERS.REAL + 200 * k)) +
          60 * (letter.scale - 1);
      }
      if (this.config.bump) {
        letter.offset.y =
          ...
          math.max(
            0,
            (5 + this.bump_rate) * math.sin(this.bump_rate * G.TIMERS.REAL + 200 * k) -
              3 -
              this.bump_rate
          );
      }
  ...
  draw() {
    ...
    if (this.shadow) {
      ...
      love.graphics.translate(
        this.strings[this.focused_string].W_offset +
          (this.text_offset.x * this.font.FONTSCALE) / G.TILESIZE,
        this.strings[this.focused_string].H_offset +
          (this.text_offset.y * this.font.FONTSCALE) / G.TILESIZE
      );
      ...
      for (const [k, letter] of ipairs(this.strings[this.focused_string].letters)) {
        ...
        love.graphics.draw(
          letter.letter,
          (0.5 * (letter.dims.x - letter.offset.x) * this.font.FONTSCALE) / G.TILESIZE -
            (this.shadow_parrallax.x * this.scale) / G.TILESIZE,
          (0.5 * letter.dims.y * this.font.FONTSCALE) / G.TILESIZE -
            (this.shadow_parrallax.y * this.scale) / G.TILESIZE,
          letter.r || 0,
          (real_pop_in * this.scale * this.font.FONTSCALE) / G.TILESIZE,
          (real_pop_in * this.scale * this.font.FONTSCALE) / G.TILESIZE,
          (0.5 * letter.dims.x) / this.scale,
          (0.5 * letter.dims.y) / this.scale
        );
    ...
    [_shadow_norm.x, _shadow_norm.y] = [
      ((this.shadow_parrallax.x /
        math.sqrt(
          this.shadow_parrallax.y * this.shadow_parrallax.y +
            this.shadow_parrallax.x * this.shadow_parrallax.x
        )) *
        this.font.FONTSCALE) /
        G.TILESIZE,
    ...
    for (const [k, letter] of ipairs(this.strings[this.focused_string].letters)) {
      ...
      love.graphics.setColor(
        letter.prefix ||
          letter.suffix ||
          letter.colour ||
          this.colours[(k % this.colours.length) + 1]
      );
      love.graphics.draw(
        letter.letter,
        (0.5 * (letter.dims.x - letter.offset.x) * this.font.FONTSCALE) / G.TILESIZE +
          _shadow_norm.x,
        (0.5 * (letter.dims.y - letter.offset.y) * this.font.FONTSCALE) / G.TILESIZE +
          _shadow_norm.y,
        letter.r || 0,
        (real_pop_in * letter.scale * this.scale * this.font.FONTSCALE) / G.TILESIZE,
        (real_pop_in * letter.scale * this.scale * this.font.FONTSCALE) / G.TILESIZE,
        (0.5 * letter.dims.x) / this.scale,
        (0.5 * letter.dims.y) / this.scale
      );
