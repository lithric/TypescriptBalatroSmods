...
class Tag extends LuaObject {
      ...
      new GameEvent({
        ...
        func: function () {
          attention_text({
            ...
            hold: 0.3 / G.SETTINGS.GAMESPEED,
            cover: this.HUD_tag,
            cover_colour: _colour || G.C.GREEN,
            align: "cm",
          });
    ...
    if (this.name === "Orbital Tag") {
      if (G.orbital_hand) {
        ...
        if (
          ...
          G.GAME.orbital_choices[G.GAME.round_resets.ante][this.ability.blind_type]
        ) {
          this.ability.orbital_hand =
            G.GAME.orbital_choices[G.GAME.round_resets.ante][this.ability.blind_type];
        }
      }
    }
      ...
      if (_context.type === "eval") {
        ...
        if (this.name === "Orbital Tag") {
          update_hand_text(
            ...
            {
              handname: this.ability.orbital_hand,
              chips: G.GAME.hands[this.ability.orbital_hand].chips,
              mult: G.GAME.hands[this.ability.orbital_hand].mult,
              level: G.GAME.hands[this.ability.orbital_hand].level,
            }
          );
        ...
        if (this.name === "Juggle Tag") {
          ...
          G.GAME.round_resets.temp_handsize =
            (G.GAME.round_resets.temp_handsize || 0) + this.config.h_size;
          this.triggered = true;
          return true;
        }
        ...
        if (
          !_context.card.edition &&
          !_context.card.temp_edition &&
          _context.card.ability.set === "Joker"
        ) {
          let lock = this.ID;
          G.CONTROLLER.locks[lock] = true;
  ...
  generate_UI(_size) {
    ...
    let tag_sprite = new Sprite(
      0,
      0,
      _size * 1,
      _size * 1,
      G.ASSET_ATLAS[(!this.hide_ability && G.P_TAGS[this.key].atlas) || "tags"],
      (this.hide_ability && G.tag_undiscovered.pos) || this.pos
    );
    tag_sprite.T.scale = 1;
    tag_sprite_tab = {
      n: G.UIT.C,
      config: { align: "cm", ref_table: this, group: this.tally },
      nodes: [
        {
          n: G.UIT.O,
          config: {
    ...
    tag_sprite.hover = function (_this) {
      if (!G.CONTROLLER.dragging.target || G.CONTROLLER.using_touch) {
        if (!_this.hovering && _this.states.visible) {
          ...
          _this.config.h_popup_config = (_this.T.x > G.ROOM.T.w * 0.4 && {
            align: "cl",
            offset: { x: -0.1, y: 0 },
            parent: _this,
          }) || { align: "cr", offset: { x: 0.1, y: 0 }, parent: _this };
  ...
  get_uibox_table(tag_sprite, vars_only) {
    ...
    if (name_to_check === "Uncommon Tag") {
      ...
      loc_vars = [
        this.config.dollars_per_hand,
        this.config.dollars_per_hand * (G.GAME.hands_played || 0),
      ];
    } else if (name_to_check === "Garbage Tag") {
      loc_vars = [
        this.config.dollars_per_discard,
        this.config.dollars_per_discard * (G.GAME.unused_discards || 0),
      ];
      ...
      loc_vars = [
        (this.ability.orbital_hand === "[" + (localize("k_poker_hand") + "]") &&
          this.ability.orbital_hand) ||
          localize(this.ability.orbital_hand, "poker_hands"),
        this.config.levels,
      ];
    ...
    tag_sprite.ability_UIBox_table = generate_card_ui(
      ...
      undefined,
      loc_vars,
      (this.hide_ability && "Undiscovered") || "Tag",
      undefined,
      this.hide_ability,
      undefined,
      undefined,
  ...
  remove() {
    ...
    if (HUD_tag_key) {
      if (G.HUD_tags && G.HUD_tags[HUD_tag_key + 1]) {
        if (HUD_tag_key === 1) {
          G.HUD_tags[HUD_tag_key + 1].set_alignment({
            type: "bri",
            offset: { x: 0.7, y: 0 },
            xy_bond: "Weak",
            major: G.ROOM_ATTACH,
