...
class Card_Character extends Moveable {
    constructor(args) {
        ...
        this.children.card = new Card(this.T.x, this.T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, args.center || G.P_CENTERS.j_joker, { bypass_discovery_center: true });
        ...
        this.children.particles = new Particles(0, 0, 0, 0, { timer: 0.03, scale: 0.3, speed: 1.2, lifespan: 2, attach: this, colours: [G.C.RED, G.C.BLUE, G.C.ORANGE], fill: true });
        this.children.particles.static_rotation = true;
        this.children.particles.set_role({ role_type: "Minor", xy_bond: "Weak", r_bond: "Strong", major: this });
    ...
    add_button(button, func, colour, update_func, snap_to, yoff) {
        ...
        this.config.button_align = { align: "bm", offset: { x: 0, y: yoff || 0.3 }, major: this, parent: this };
        this.children.button = new UIBox({ definition: create_UIBox_character_button({ button: button, func: func, colour: colour, update_func: update_func, maxw: 3 }), config: this.config.button_align });
        if (snap_to) {
    ...
    add_speech_bubble(text_key, align, loc_vars) {
        ...
        this.config.speech_bubble_align = { align: align || "bm", offset: { x: 0, y: 0 }, parent: this };
        this.children.speech_bubble = new UIBox({ definition: G.UIDEF.speech_bubble(text_key, loc_vars), config: this.config.speech_bubble_align });
        this.children.speech_bubble.set_role({ role_type: "Minor", xy_bond: "Weak", r_bond: "Strong", major: this });
    ...
    say_stuff(n, not_first) {
        ...
        if (!not_first) {
            ...
            G.E_MANAGER.add_event(new GameEvent({ trigger: "after", blockable: false, blocking: false, delay: 0.13, func: function () {
