...
class Blind extends Moveable {
    constructor(X, Y, W, H) {
        ...
        this.children.animatedSprite = new AnimatedSprite(this.T.x, this.T.y, this.T.w, this.T.h, G.ANIMATION_ATLAS["blind_chips"], G.P_BLINDS.bl_small.pos);
    ...
    set_text() {
        if (this.config.blind) {
            if (this.disabled) {
                this.loc_name = this.name === "" && this.name || localize({ type: "name_text", key: this.config.blind.key, set: "Blind" });
                ...
                let target = { type: "raw_descriptions", key: this.config.blind.key, set: "Blind", vars: loc_vars || this.config.blind.vars };
                ...
                if (loc_target) {
                    ...
                    for (const [k, v] of ipairs(loc_target)) {
                        this.loc_debuff_text = this.loc_debuff_text + (v + (k <= loc_target.length && " " || ""));
    ...
    set_blind(blind, reset, silent) {
        if (!reset) {
            ...
            this.set_text();
            let obj = this.config.blind;
            this.children.animatedSprite.atlas = G.ANIMATION_ATLAS[obj.atlas] || G.ANIMATION_ATLAS["blind_chips"];
            ...
            this.chips = get_blind_amount(G.GAME.round_resets.ante) * this.mult * G.GAME.starting_params.ante_scaling;
            this.chip_text = number_format(this.chips);
            if (!blind) {
                this.chips = 0;
            }
            G.GAME.current_round.dollars_to_be_earned = this.dollars > 0 && string.rep(localize("$"), this.dollars) + "" || "";
            G.HUD_blind.alignment.offset.y = -10;
            G.HUD_blind.recalculate(false);
            if (blind && blind.name && blind.name !== "") {
                this.alert_debuff(true);
                G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 0.05, blockable: false, func: function () {
                        ...
                        G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 0.15, blockable: false, func: function () {
        ...
        if (this.name === "The Eye" && !reset) {
            this.hands = { ["Flush Five"]: false, ["Flush House"]: false, ["Five of a Kind"]: false, ["Straight Flush"]: false, ["Four of a Kind"]: false, ["Full House"]: false, ["Flush"]: false, ["Straight"]: false, ["Three of a Kind"]: false, ["Two Pair"]: false, ["Pair"]: false, ["High Card"]: false };
        }
        ...
        G.ARGS.spin.real = (G.SETTINGS.reduced_motion && 0 || 1) * (this.config.blind.boss && (this.config.blind.boss.showdown && 0.5 || 0.25) || 0);
    };
    alert_debuff(first) {
        if (this.loc_debuff_text && this.loc_debuff_text !== "") {
            ...
            G.E_MANAGER.add_event(new GameEvent({ blockable: false, blocking: false, func: function () {
                    ...
                    if (G.STATE === G.STATES.SELECTING_HAND) {
                        G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: G.SETTINGS.GAMESPEED * 0.05, blockable: false, func: function () {
                                play_sound("whoosh1", 0.55, 0.62);
                                for (let i = 1; i <= 4; i++) {
                                    let wait_time = 0.1 * (i - 1);
                                    G.E_MANAGER.add_event(new GameEvent({ blockable: false, trigger: "after", delay: G.SETTINGS.GAMESPEED * wait_time, func: function () {
                                            if (i === 1) {
                                                this.juice_up();
                                            }
                                ...
                                let hold_time = G.SETTINGS.GAMESPEED * (this.loc_debuff_text.length * 0.035 + 1.3);
                                let disp_text = this.get_loc_debuff_text();
                                attention_text({ scale: 0.7, text: disp_text, maxw: 12, hold: hold_time, align: "cm", offset: { x: 0, y: -1 }, major: G.play });
                                G.E_MANAGER.add_event(new GameEvent({ trigger: "after", delay: 1, blocking: false, blockable: false, func: function () {
                                        this.block_play = undefined;
                                        if (G.buttons) {
    ...
    get_loc_debuff_text() {
        ...
        let disp_text = (this.config.blind.name === "The Wheel" && G.GAME.probabilities.normal || "") + this.loc_debuff_text;
    ...
    defeat(silent) {
        ...
        this.juice_up();
        this.children.particles = Particles(0, 0, 0, 0, { timer_type: "TOTAL", timer: 0.01 * dissolve_time, scale: 0.1, speed: 1.5, lifespan: 0.7 * dissolve_time, attach: this, colours: this.dissolve_colours, fill: true });
        ...
        if (!silent) {
            for (let i = 1; i <= math.min(this.sound_pings || 3, 7); i++) {
                ...
                G.E_MANAGER.add_event(new GameEvent({ blockable: false, trigger: "after", delay: (0.15 - 0.01 * (this.sound_pings || 3)) * i * dissolve_time, func: function () {
        ...
        G.E_MANAGER.add_event(new GameEvent({ trigger: "ease", blockable: false, ref_table: this, ref_value: "dissolve", ease_to: 1, delay: 0.7 * dissolve_time, func: function (t) {
                return t;
            } }));
        G.E_MANAGER.add_event(new GameEvent({ trigger: "after", blockable: false, delay: 0.8 * dissolve_time, func: function () {
                G.HUD_blind.alignment.offset.y = -10;
                return true;
            } }));
        G.E_MANAGER.add_event(new GameEvent({ trigger: "after", blockable: false, delay: 0.95 * dissolve_time, func: function () {
    ...
    disable() {
        ...
        if (this.name === "The Wheel" || this.name === "The House" || this.name === "The Mark" || this.name === "The Fish") {
    ...
    stay_flipped(area, card) {
        if (!this.disabled) {
            ...
            if (area === G.hand) {
                if (this.name === "The Wheel" && pseudorandom(pseudoseed("wheel")) < G.GAME.probabilities.normal / 7) {
                    return true;
                }
                if (this.name === "The House" && G.GAME.current_round.hands_played === 0 && G.GAME.current_round.discards_used === 0) {
                    return true;
                }
    ...
    save() {
        let blindTable = { in_blind: this.in_blind, name: this.name, dollars: this.dollars, debuff: this.debuff, pos: this.pos, mult: this.mult, disabled: this.disabled, discards_sub: this.discards_sub, hands_sub: this.hands_sub, boss: this.boss, config_blind: "", chips: this.chips, chip_text: this.chip_text, hands: this.hands, only_hand: this.only_hand, triggered: this.triggered };
    ...
    load(blindTable) {
        ...
        G.ARGS.spin.real = (G.SETTINGS.reduced_motion && 0 || 1) * (this.config.blind.boss && (this.config.blind.boss.showdown && 1 || 0.3) || 0);
